...
| 9 | ⚰️ **RETIRED 2026-08-30** — ~~Envelope rollover + "left to assign" pot~~ (`leftover = budgeted + spent + prevLeftoverPos`). Shipped 2026-08-19, replaced by TOP-DOWN budgeting in `46bfbe4` on owner feedback. Rollover survives as one whole-budget switch; "left to assign" is gone by design — don't re-propose it | actual `budget/envelope.ts:52-66`; firefly `AvailableBudget.php` | M |
...
- **Phase 4 — Foundations (L):** materialized balance table + reverse calculator (T2 #1); then ~~envelope rollover~~ (retired, see T2 #9), Sharia X-Ray, AI tool-calling (T2 #9, #4, #7).
