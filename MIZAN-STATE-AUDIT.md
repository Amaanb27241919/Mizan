...
| `/api/cron/bot-signals` | `0 14 * * 1-5` (vercel.json, daily backstop) **+ GitHub Actions `*/15 * * * 1-5`** (`.github/workflows/cron-bot-signals.yml`) | Trading-bot strategy evaluation + signal generation/execution. Vercel Hobby caps crons at daily, so the every-15-min weekday cadence is driven externally via GitHub Actions hitting the endpoint with the `CRON_SECRET` bearer (public repo → unlimited Actions minutes). |
