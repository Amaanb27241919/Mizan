...
### Fixed
- **demo:** Make demo mode run with no network at all (`abe74f1`)
...
### Fixed
- **overview:** One return number, honestly labelled — and stop the chart inventing gains (`3bfd478`)
...
### Fixed
- **demo:** Stop the demo Overview figures regenerating on every page load (`71d98fa`)
...
### Added
- **admin:** Fold backlog buckets independently, hide the archives (`568f648`)
...
### Docs
- Record the notification feed and the user_state clobber fix (`1f23adf`)
- Update CLAUDE.md + BACKLOG; extract and test the history merge (`ff69f1d`)

## 2026-07-29

### Added
- **growth:** Widen activation window to 45 days, skip recently-active users (`8ac1447`)
...
### Maintenance
- Drop unused React default imports (automatic JSX runtime); lint 36->28 (`3253b9b`)
...
### Fixed
- **advisor:** Stop page crash on Send + sanitize upstream API errors (`0f6c252`)
...
### Docs
- **claude:** Sync to session — migration 025, speed-insights, ALERT_FROM warning (`1f8c6e2`)
...
### Fixed
- **bot:** Give semi-auto DCA buys a full-day approval window (was 60 min) (`a4e9728`)
...
### Docs
- **readme:** Correct demo mode — ~$435k (was ~$42M) + opt-in default (was 'defaults on') (`1199302`)
- Sync CLAUDE.md + state-audit to the 2026-07-15 work (demo $435k, RIA conformed) (`47d0f92`)
- **backlog:** O23 — record owner decision that beta keeps LIVE semi-auto (not paper) (`d60e9ef`)
- **backlog:** O23 conforming code shipped (890d147) — bot NL + Rebalancer now self-directed (`6391468`)
...
### Maintenance
- **zakat:** F1 correctness — add server↔lib nisab-formula drift guard (`5f3bd04`)
- **maintenance:** Close F6 (legacy flat file) + partial F5 (esbuild advisory) (`a262d8c`)
...
### Added
- **compliance:** Draw the read-only line at personalized advice (`1b842d7`)
