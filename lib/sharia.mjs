...
// ── Ethical / BDS overlay ────────────────────────────────────────────────────
// An OPTIONAL layer on top of the AAOIFI Sharia verdict. It NEVER changes the
// Sharia `status`; it only adds an `ethical` flag that the app applies when the
// user opts into the overlay (default off).
//
// ATTRIBUTION IS THE WHOLE DESIGN. Mizan does not assert that any company does
// anything. Every entry names the body that published the listing, so the app
// reports a third party's finding and the user can go read the source itself.
// Never reword an entry into Mizan's own voice, and never add a ticker without
// a citable listing behind it — a wrong ticker here is a false public claim
// about a real company, which is the one failure mode that actually matters.
//
// The sources are websites and PDFs, not machine-readable feeds, so this cannot
// be automated. Instead the list carries its own reconciliation date and the
// daily health sweep emails the owner once it goes stale (lib/anomaly.mjs →
// checkEthicalListReview). Reconciling means re-reading both sources, adding
// newly listed US-traded names, and REMOVING de-listed ones — OHCHR dropped 15
// enterprises in its 2023 pass for ceasing the listed activity, so staleness
// cuts both ways and leaving a name on after it is de-listed is the worse error.
export const ETHICAL_SOURCES = {
  ohchr: {
    key:     "ohchr",
    name:    "UN OHCHR settlement-business database",
    url:     "https://www.ohchr.org/en/business/bhr-database",
    version: "September 2025 update — 158 enterprises across 11 countries",
  },
  afsc: {
    key:     "afsc",
    name:    "AFSC \"Investigate\"",
    url:     "https://investigate.afsc.org/",
    version: "accessed 2026-08-20 — 200+ companies",
  },
};

/** ISO date this list was last reconciled against the sources above. */
export const ETHICAL_RECONCILED = "2026-08-20";

/** Cadence: re-read both sources twice a year. */
export const ETHICAL_REVIEW_DAYS = 180;

// `activity` is the listed conduct as the SOURCE characterises it, kept short
// enough for a tooltip. `sources` are keys into ETHICAL_SOURCES.
//
// KNOWN GAP (2026-08-20): the OHCHR database is majority Israeli-domiciled
// firms — banks (Leumi, Discount, Mizrahi Tefahot, FIBI), telecoms (Bezeq,
// Partner, Cellcom) and transport. Some carry US listings, but the name→ticker
// mapping was not verifiable to the standard above in this pass, so they are
// deliberately OMITTED rather than guessed. Adding them is an owner decision.
const ETHICAL_EXCLUSIONS = {
  CAT:  { sources: ["afsc"],          activity: "Heavy equipment cited in home-demolition campaigns" },
  HPQ:  { sources: ["afsc"],          activity: "IT and hardware contracts" },
  HPE:  { sources: ["afsc"],          activity: "IT and data-centre contracts" },
  MSI:  { sources: ["ohchr", "afsc"], activity: "Surveillance and communications systems" },
  BKNG: { sources: ["ohchr"],         activity: "Lists accommodations in occupied-territory settlements" },
  ABNB: { sources: ["ohchr"],         activity: "Lists accommodations in occupied-territory settlements" },
  EXPE: { sources: ["ohchr"],         activity: "Lists accommodations in occupied-territory settlements" },
  TRIP: { sources: ["ohchr"],         activity: "Lists accommodations in occupied-territory settlements" },
  CVX:  { sources: ["afsc"],          activity: "Resource-extraction operations" },
  PLTR: { sources: ["afsc"],          activity: "Surveillance and defence contracts" },
  MSFT: { sources: ["afsc"],          activity: "Technology and cloud contracts" },
  NVDA: { sources: ["afsc"],          activity: "Technology and hardware contracts" },
  LOGI: { sources: ["afsc"],          activity: "Technology and hardware contracts" },
};

/** Tickers currently on the list, sorted. For tests, docs and admin surfaces. */
export const ethicalExcludedTickers = () => Object.keys(ETHICAL_EXCLUSIONS).sort();

export function ethicalScreen(tk) {
  const t = String(tk || "").toUpperCase().trim();
  const e = ETHICAL_EXCLUSIONS[t];
  if (!e) {
    return { excluded: false, reason: null, activity: null, sources: [], list: "bds", reconciled: ETHICAL_RECONCILED };
  }
  const sources = e.sources.map((k) => ETHICAL_SOURCES[k]).filter(Boolean);
  // Composed so the string itself carries the attribution: any surface that
  // renders only `reason` (the pre-2026-08-20 shape) still says who listed it.
  const by = sources.map((s) => s.name).join(" and ");
  return {
    excluded:   true,
    reason:     by ? `${e.activity} — listed by ${by}` : e.activity,
    activity:   e.activity,
    sources,
    list:       "bds",
    reconciled: ETHICAL_RECONCILED,
  };
}

/** Days since the list was last reconciled against its sources. */
export function daysSinceEthicalReview(now = new Date(), iso = ETHICAL_RECONCILED) {
  const d = new Date(`${iso}T00:00:00Z`);
  if (Number.isNaN(d.getTime())) return Infinity;
  return Math.floor((now.getTime() - d.getTime()) / 86400000);
}

/** True once the reconciliation cadence has lapsed. */
export function isEthicalReviewDue(now = new Date(), iso = ETHICAL_RECONCILED) {
  return daysSinceEthicalReview(now, iso) >= ETHICAL_REVIEW_DAYS;
}
