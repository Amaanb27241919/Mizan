...
async function screenViaFinnhub(tk) {
  ...
  return verdictFromFundamentals(tk, {
    ...
    mc:     p.marketCapitalization || 0,
    debt:   metric.totalDebt || metric.longTermDebtAnnual || 0,
    cash:   metric.cashAndShortTermInvestmentsAnnual || metric.cashAndCashEquivalentsAnnual || 0,
    recv:   metric.netReceivablesAnnual || 0,
    assets: metric.totalAssetsAnnual || metric.totalAssetsTTM || 0,
    country: p.country, name: p.name,
    nonPermPct: undefined, // Finnhub free tier can't supply this
    source: "finnhub",
  });
}
...
export async function screenSymbol(tk) {
  ...
  if (!t) return { tk: t, status: "unknown", reason: "empty", source: activeShariaProvider() };
  if (CRYPTO_RE.test(t)) {
    return { tk: t, status: "halal", industry: "Cryptocurrency", source: activeShariaProvider(), nonPermPct: null,
             byStandard: Object.fromEntries(Object.keys(STANDARDS).map(k => [k, { pass: true, note: "crypto" }])), asOf: todayStr() };
  }
...
export async function screenBatch(symbols = []) {
  ...
  for (let i = 0; i < uniq.length; i += 4) {
    ...
    // 4 syms × 2 Finnhub calls = 8 req/batch; pause to stay under 60/min.
    if (ZOYA_API_KEY ? false : i + 4 < uniq.length) await new Promise(r => setTimeout(r, 1100));
  }
  return out;
}
