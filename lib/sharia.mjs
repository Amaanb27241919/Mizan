...
function evaluateAgainst(standard, { sector, debt, cash, recv, mc, assets, nonPermPct }) {
  ...
  const tests = [
    { rule: `Debt/${isAssets ? "Assets" : "MC"}`, pass: debtR < standard.debtMax, detail: `${debtR.toFixed(1)}%`, limit: standard.debtMax },
...
function verdictFromFundamentals(tk, { industry, mc, debt, cash, recv, assets, country, name, nonPermPct, source }) {
...
async function fhFundamentals(tk) {
  ...
  // Debt via the (still-provided) debt/equity ratio × book equity.
  ...
  let debt = (Number.isFinite(der) && equity > 0) ? der * equity : 0;
  // Fallback when the ratio is missing/unusable (e.g. negative equity): sum the
  // common raw debt concepts across filer conventions.
  if (!(debt > 0)) {
    const currentDebt = bsConcept(bs, "DebtCurrent")
      || (bsConcept(bs, "LongTermDebtCurrent", "LongTermDebtAndCapitalLeaseObligationsCurrent")
          + bsConcept(bs, "ShortTermBorrowings", "CommercialPaper", "NotesAndLoansPayable", "ShortTermDebt", "NotesPayableCurrent"));
    const longDebt = bsConcept(bs, "LongTermDebtNoncurrent", "LongTermDebt", "LongTermDebtAndCapitalLeaseObligations", "NotesPayable", "LoansPayable", "SecuredDebt", "UnsecuredDebt");
    debt = currentDebt + longDebt;
  }
  const MM = 1e6; // filings are in dollars; convert to millions to match marketCap
  return { assets: assets / MM, cash: cash / MM, recv: recv / MM, debt: debt / MM };
}
async function screenViaFinnhub(tk) {
  ...
  const f = fund || { assets: 0, cash: 0, recv: 0, debt: 0 };
