...
/**
 * The ethical / BDS exclusion list is hand-curated against sources that publish
 * as web pages and PDFs — there is no feed to cron against, so the list cannot
 * refresh itself. That makes silent rot the default outcome, and rot here is
 * not cosmetic: the list makes public factual claims about named companies, and
 * OHCHR removed 15 enterprises in a single pass for ceasing the listed activity.
 * A stale entry is a false accusation; a missing one is a broken promise.
 *
 * So the list carries its own reconciliation date and this sweep escalates once
 * the cadence lapses — same shape as checkLegalReview, and for the same reason:
 * a reminder derived from the artefact's own date cannot drift from it.
 */
export async function checkEthicalListReview({ alert = true, now = new Date() } = {}, sbAdmin) {
  const { ETHICAL_RECONCILED, ETHICAL_REVIEW_DAYS, ETHICAL_SOURCES, daysSinceEthicalReview, isEthicalReviewDue, ethicalExcludedTickers } =
    await import("./sharia.mjs");

  const days = daysSinceEthicalReview(now);
  const due  = isEthicalReviewDue(now);
  const tickers = ethicalExcludedTickers();
  const out = {
    label:        "Ethical / BDS list review",
    healthy:      !due,
    last_updated: ETHICAL_RECONCILED,
    days_since:   days,
    interval:     ETHICAL_REVIEW_DAYS,
    entries:      tickers.length,
    tickers,
    sources:      Object.values(ETHICAL_SOURCES).map((s) => s.name),
    checked_at:   now.toISOString(),
  };

  if (due) {
    warn("anomaly.ethical_list_review_due", { days_since: days, last_updated: ETHICAL_RECONCILED });
    if (alert) {
      sendAlert("ethical.review_due", {
        last_updated: ETHICAL_RECONCILED,
        days_since:   days,
        interval:     ETHICAL_REVIEW_DAYS,
        entries:      tickers.length,
        tickers:      tickers.join(", "),
        sources:      Object.values(ETHICAL_SOURCES).map((s) => `${s.name} (${s.url})`).join(", "),
        impact:       "The divestment-target list in lib/sharia.mjs has not been reconciled against its sources. "
                    + "Re-read both, ADD newly listed US-traded names and REMOVE de-listed ones — a name left on "
                    + "after it is de-listed is a false public claim about a real company.",
        severity:     "medium",
      }, sbAdmin);
    }
  }
