...
export const RATE_LIMITS = {
  ...
  "marketdata.search":240,   // /api/market/symbols — the Screener's ticker typeahead.
                              // Same reasoning as marketdata.ticket: someone typing a
                              // company name fires one debounced request per pause and
                              // must not be starved by background portfolio polling in
                              // the shared "marketdata" bucket. Upstream cost is near
                              // zero — company names are static, so responses cache for
                              // hours and most keystrokes never reach Finnhub at all.
...
export function matchAction(pathname, search = "") {
  ...
  // Symbol typeahead — dedicated bucket, ahead of the generic /api/market/ rule
  // below so it isn't swallowed by it.
  if (pathname === "/api/market/symbols")     return "marketdata.search";
  // Public, unauthenticated market-data proxies — tight shared bucket so an anon
  // caller can't drain the upstream free-tier quota for everyone. /api/market/*
  // (auth-gated candle + quote routes powering the price chart) share the same
  // upstream (Polygon aggregates / Finnhub quote) so they belong in this bucket.
