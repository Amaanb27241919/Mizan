...
export async function sendUserEmail(toEmail, subject, text, sbAdmin = null, opts = {}) {
  ...
  const payload = JSON.stringify({
    ...
    text: text || "",
    html,
    // Replies land in the operator's real inbox — ALERT_FROM (alerts@mizan.exchange)
    // is send-only and unmonitored. Caller can override via opts.replyTo.
    ...((opts.replyTo || OWNER_EMAIL) ? { reply_to: opts.replyTo || OWNER_EMAIL } : {}),
  });
