...
export async function fetchWithRetry(url, options = {}, { maxRetries = 3, baseDelayMs = 500 } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (res.status === 429) {
        ...
        warn("fetch_retry", { url, attempt, status: 429, delayMs: delay });
        await sleep(delay);
        continue;
      }
      if (res.status >= 500 && attempt < maxRetries) {
        const delay = baseDelayMs * Math.pow(2, attempt) + Math.random() * 200;
        warn("fetch_retry", { url, attempt, status: res.status, delayMs: Math.round(delay) });
        await sleep(delay);
        continue;
      }
      return res;
  ...
  logError("fetch_failed", { url, attempts: maxRetries + 1, err: lastError?.message });
  throw lastError || new Error(`fetchWithRetry exhausted: ${url}`);
}
