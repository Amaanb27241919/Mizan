...
import { isIpBlocked, trackAuthFailure, trackSnapTradeError, checkCronStaleness, checkNewDevice, checkDataFeeds, checkConfig, checkLegalReview, checkEthicalListReview } from "./anomaly.mjs";
// Shared with the client. Pure, no browser APIs — the nightly cron and the
// in-app snapshot MUST merge history identically, or they clobber each other.
import { mergeNetWorthHistory } from "../src/lib/netWorth.js";
import { fetchWithRetry } from "./fetchWithRetry.mjs";
import { botClaimLeaseThresholdIso } from "./botLease.mjs";
...
import { ETF_UNIVERSE, universeMeta, CURATED_HOLDINGS, parseAlphaVantageProfile, overlapMatrix } from "./etfHoldings.mjs";
import { validateCandleQuery, cacheTimespan, normalizeBars, SYMBOL_RE } from "./market/candles.mjs";
import { normalizeQuery, normalizeSymbolSearch, MAX_SUGGESTIONS } from "./market/symbolSearch.mjs";
import { usMarketStatus, marketSession, validateSessionOrder } from "./market/sessions.mjs";
import { validateOrderSizing, estimateOrderValue, withinCashCeiling } from "./market/orders.mjs";
// Shared with the client so the funnel groups sources exactly the way the
// browser classified them — one definition, no drift.
import { summarizeSources } from "../src/lib/attribution.js";
import { assertImpersonal } from "./compliance/policy.mjs";
import { ADVISOR_SYSTEM_PREFIX } from "./compliance/advisor-prompt.mjs";
import { filterAdvisorResponse } from "./compliance/advisor-filter.mjs";
...
// ── Alpha Vantage (ETF holdings, free tier: 25 req/day) ──
// Powers the ETF Overlap Analyzer. Because of the tight daily quota, holdings
// are fetched server-side ONLY and cached ~24h in etf_holdings_cache; the known
// halal ETF universe is ~7 symbols → ~7 calls/day, well under the cap.
const ALPHAVANTAGE_KEY = (process.env.ALPHAVANTAGE_KEY || "").trim();
const ETF_CACHE_TTL_MS = 24 * 3600 * 1000;        // Alpha Vantage rows: refresh daily
const CURATED_TTL_MS   = 90 * 24 * 3600 * 1000;   // curated MUTUAL-FUND rows (authoritative): ~90d
const ETF_CURATED_FALLBACK_TTL_MS = 60 * 60 * 1000; // curated ETF FALLBACK (AV throttled/down): retry AV in ~1h

// Sharia precheck — a cheap, hardcoded SAFETY NET of obviously non-compliant
// tickers (banks, alcohol, gambling, conventional bond funds). NOT the authority:
// the bot's entry engine now screens every picked candidate through the real
// AAOIFI service (lib/sharia.mjs / screenSymbol) and blocks a "haram" verdict, so
// a name that TURNS non-compliant is caught even if it's not in this list. This
// set just short-circuits the known-bad ones before spending a screen call.
...
// Halal "Bogleheads" lazy portfolio — the Islamic adaptation of the classic
// three-fund (US / international / bonds), the bond sleeve replaced by sukuk.
// Each SECTION offers picks the user chooses from (not one hardcoded basket). A
// `dca` strategy carrying params.basket buys the most-underweight AFFORDABLE
// member each period, converging on the weights via new contributions (no sells
// — tax-efficient + never auto-sold). Weights are relative (cron normalizes).
const HALAL_BOGLEHEADS_SLEEVES = [
  { key: "us",    label: "US equity",                  core: true,  weight: 0.50,
    options: [ { ticker: "SPUS", name: "SP Funds S&P 500 Sharia",       expense: 0.45 },
               { ticker: "HLAL", name: "Wahed FTSE USA Shariah",        expense: 0.50 } ] },
  { key: "intl",  label: "International / World ex-US", core: true,  weight: 0.30,
    options: [ { ticker: "SPWO", name: "SP Funds S&P World (ex-US)",     expense: 0.55 },
               { ticker: "UMMA", name: "Wahed Dow Jones Islamic World",  expense: 0.65 } ] },
  { key: "sukuk", label: "Sukuk (halal fixed income)", core: true,  weight: 0.20,
    options: [ { ticker: "SPSK", name: "SP Funds Dow Jones Global Sukuk", expense: 0.55 } ] },
  { key: "tech",  label: "Global technology tilt",     core: false, weight: 0.10,
    options: [ { ticker: "SPTE", name: "SP Funds S&P Global Technology",  expense: 0.55 } ] },
  { key: "reit",  label: "Global REIT tilt",           core: false, weight: 0.10,
    options: [ { ticker: "SPRE", name: "SP Funds S&P Global REIT Sharia", expense: 0.55 } ] },
];

// Compliance framing surfaced with every bot strategy the user builds. MĪZAN is a
// self-directed execution tool, NOT an investment adviser: the user chooses the
// symbols + settings and approves every order; Mizan never recommends. Keeps the
// bot on the compliant side of the personalized-advice line (see docs/COMPLIANCE.md).
const SELF_DIRECTED_DISCLAIMER = "You chose these symbols and settings. MĪZAN structures your own instruction and only trades with your approval — it is a self-directed tool, not investment advice or a recommendation. Past performance is no guarantee; you can lose money.";

// Assemble a weighted basket from per-sleeve picks. `sel` maps a sleeve key →
// chosen ticker (or {ticker,weight}). Core sleeves default to their first option;
// optional tilts (tech/reit) are included ONLY when explicitly selected. Every
// leg is validated against its sleeve's options + the Sharia blocklist.
function bogleheadsBasket(sel = {}) {
  const basket = [];
  for (const s of HALAL_BOGLEHEADS_SLEEVES) {
    const pick = sel[s.key];
    if (!s.core && !pick) continue; // optional sleeve, not chosen → skip
    const wanted = String(pick?.ticker || pick || "").toUpperCase();
    let ticker = s.options.some(o => o.ticker === wanted) ? wanted
               : (s.core ? s.options[0].ticker : null); // valid pick, else core default, else skip
    if (!ticker || HARAM_TICKERS.has(ticker)) continue;
    const weight = Number(pick?.weight) > 0 ? Number(pick.weight) : s.weight;
    basket.push({ ticker, weight, sleeve: s.key });
  }
  return basket;
}

// Detect sleeve picks from plain English: any option ticker named in the text
// fills its sleeve; naming "tech"/"reit" (or their tickers) adds that tilt.
function bogleheadsSelectionFromText(text = "") {
  const up = String(text).toUpperCase();
  const sel = {};
  for (const s of HALAL_BOGLEHEADS_SLEEVES) {
    const named = s.options.find(o => new RegExp(`\\b${o.ticker}\\b`).test(up));
    if (named) sel[s.key] = named.ticker;
  }
  if (!sel.tech && /\b(TECH|TECHNOLOGY)\b/.test(up)) sel.tech = "SPTE";
  if (!sel.reit && /\b(REIT|REAL ESTATE)\b/.test(up)) sel.reit = "SPRE";
  return sel;
}

// Build a ready-to-review Halal Bogleheads DCA strategy (same shape the NL parser
// returns), from the chosen sleeve picks. capital/amount default to 0 (user sets
// the weekly amount + account in the review UI). stop_loss_pct is a positive
// placeholder only to satisfy the create gate; DCA never evaluates stops.
function buildBogleheadsPreset(accountId = null, sel = {}) {
  const basket = bogleheadsBasket(sel);
  const tickers = basket.map(b => b.ticker);
  const wTot = basket.reduce((n, x) => n + x.weight, 0) || 1;
  const legLabel = basket.map(b => `${b.ticker} ${Math.round((b.weight / wTot) * 100)}%`).join(" · ");
  return {
    ticker: tickers[0] || "SPUS",
    account_id: (typeof accountId === "string" && accountId) ? accountId : null,
    strategy_type: "dca",
    capital_allocated: 0,
    profit_target_pct: 0,      // accumulate & hold — no take-profit
    stop_loss_pct: 20,         // inert for DCA; >0 required by the create gate
    max_drawdown_pct: 20,
    time_horizon_days: 365,    // long-term hold (past 1yr for long-term cap-gains)
    max_trades_per_day: 1,
    position_size_pct: 100,
    layer: "semi",
    universe_tickers: tickers,
    params: {
      basket, sleeves: sel, universe_tickers: tickers,
      dca_cadence_days: 7, dca_amount: 0, layer: "semi",
      universe: `Halal Bogleheads: ${legLabel}`,
      detailed_rules: `Weekly DCA into a halal lazy portfolio (${legLabel}). Each period buys the most-underweight member to rebalance via new contributions; holds long-term, never auto-sells.`,
      executed_as: "DCA basket — buys whole shares of the most-underweight affordable ETF toward the target weights each week; no stops or profit targets.",
    },
    risk_disclosure: "A long-term halal accumulation basket, not a return guarantee. Halal ETFs can lose value and are more concentrated (tech-tilted) than a conventional total-market fund. You choose the weekly amount; Mizan only executes the buys. Not financial advice.",
  };
}
...
// system prompt (and /api/advisor/count, so its token math matches). Now lives in
// lib/compliance/advisor-prompt.mjs alongside the compliance boundary it encodes.
// It stays STABLE per deploy so Anthropic prompt-caching keeps hitting on repeat
// turns. ADVISOR_SYSTEM_PREFIX is imported at the top of this file.
...
// "First Last" from a profiles row, or null when the user hasn't set a name
// yet (migration 026 columns are nullable for every pre-existing account).
// Used everywhere the admin panel labels a user.
const fullName = (p) => [p?.first_name, p?.last_name].filter(Boolean).join(" ").trim() || null;
...
function cronUnauthorized(headers) {
  ...
  if (!CRON_SECRET) return true; // fail-closed when the secret isn't configured
  // Constant-time compare so the bearer token can't be recovered via response-timing.
  const a = Buffer.from(authHeader);
  const b = Buffer.from(`Bearer ${CRON_SECRET}`);
  return a.length !== b.length || !crypto.timingSafeEqual(a, b);
}

// Cron heartbeat → the `cron_jobs` ledger that checkCronStaleness() reads.
//
// ALWAYS `await` this. Every cron used to write its heartbeat fire-and-forget
// (`.upsert(...).then(...)`) immediately before returning a response, which
// races the serverless runtime freezing the invocation once that response is
// sent. The crons that finish fastest lose that race most often, which is why
// dividend-check read as 23 days stale (2026-08-10) while it was in fact being
// invoked daily. A dropped heartbeat is indistinguishable from a dead
// scheduler, so the monitoring was reporting on its own write races.
//
// Recording failure is logged, never thrown: a cron that did its real work
// must not be reported as failed just because the bookkeeping row didn't land.
async function recordCronRun(jobName, status = "ok", lastError = null) {
  if (!sbAdmin) return;
  try {
    const { error } = await sbAdmin.from("cron_jobs").upsert({
      job_name:    jobName,
    ...
    if (error) warn("cron.upsert_failed", { job: jobName, err: error.message });
  } catch (e) {
    warn("cron.upsert_failed", { job: jobName, err: e.message });
  }
}
...
const APP_BASE_URL = (process.env.APP_BASE_URL || "https://app.mizan.exchange").trim().replace(/\/$/, "");

// The single net-worth history key. The client (src/components/MizanApp.jsx)
// and the nightly snapshot cron both write bank-inclusive totals here as
// { date, total, ... }; the Overview chart and the weekly digest both read it.
// The old server-only `networth_history` key is legacy — the snapshot cron
// merges it forward and nothing reads it after that.
const NW_HISTORY_KEY = "mizan_networth_history";

// Short-lived memo for buildAudience(), which costs one live SnapTrade call per
// user. Module scope so it survives between requests on a warm instance; only
// read when a caller explicitly passes a maxAgeMs.
let _audienceCache = { at: 0, data: null };

// ── Activation nudge copy ───────────────────────────────
// Shared by the daily /api/cron/activation sweep and the per-user "Send nudge"
// button in the admin panel, so the automatic and manual paths always send the
// identical email. Pure — takes a display name, returns { subject, text }.
function activationNudgeEmail(name) {
  const first = (name || "").split(" ")[0];
  return {
    subject: "Your Mīzan account is waiting on one step",
    text: [
      first ? `Assalamu alaikum ${first},` : "Assalamu alaikum,",
      "",
      "You created a Mīzan account but haven't connected anything yet — so right now the app has nothing to show you.",
      "",
      "Connecting takes about a minute and is read-only: Mīzan never sees your broker or bank password, and it can't move your money. Once an account is linked you'll get:",
      "",
      "• Every holding screened against AAOIFI standards — halal, review, or non-compliant",
      "• Your Zakat calculated from real balances, with live gold and silver nisab",
      "• Dividend purification tracked per holding, to the cent",
      "",
      "You keep your existing broker. Fidelity, Schwab, Robinhood, Coinbase and 60+ more are supported, plus your bank.",
      "",
      `Connect an account: ${APP_BASE_URL}`,
      "",
      "If something got in the way — a broker that wouldn't link, a step that felt off, or you'd just like a walkthrough — reply to this email and tell me. I read every one, and that feedback shapes what I build next.",
    ].join("\n"),
  };
}

// Why a given user was or wasn't picked up by the activation cron. Mirrors the
// filter in /api/cron/activation exactly; kept beside the copy so the admin
// panel explains the automation rather than restating it. Pure.
export function activationState(u, { nudgedAt, lastSignIn, now = Date.now() } = {}) {
  const MIN_AGE_MS    = 24 * 3600 * 1000;
  const MAX_AGE_MS    = 45 * 24 * 3600 * 1000;
  const ACTIVE_WINDOW = 7 * 24 * 3600 * 1000;
  if (nudgedAt)     return { state: "sent",      note: "nudge already sent" };
  if (u.suspended)  return { state: "skipped",   note: "account suspended" };
  if (u.connected)  return { state: "connected", note: "has a linked account" };
  if (!u.email)     return { state: "skipped",   note: "no email on file" };
  if (lastSignIn && (now - new Date(lastSignIn).getTime()) < ACTIVE_WINDOW) {
    return { state: "skipped", note: "signed in within 7 days — engaged, not lapsed" };
  }
  const age = now - new Date(u.created_at).getTime();
  if (age < MIN_AGE_MS) return { state: "waiting", note: "under 24h old" };
  if (age > MAX_AGE_MS) return { state: "skipped", note: "over 45 days old — send by hand" };
  return { state: "eligible", note: "will be emailed on the next daily run" };
}
...
export function csvEscapeCell(value) {
  ...
  // is executed as a formula by Excel/Sheets. Transaction merchant/payee names are
  // counterparty-controlled, so neutralize with a leading apostrophe before the
  // quote-escaping below. OWASP CSV Injection.
...
/**
 * Normalise a client-supplied budget month to the first of that month, or null.
 *
 * Validation at the boundary, not decoration: migration 027 puts a CHECK on
 * both budget tables requiring `date_trunc('month', month) = month`, so a
 * mid-month value would be rejected by Postgres with an opaque 23514. Worse, a
 * client that got one through would create a SECOND bucket for a month that is
 * supposed to have exactly one, and envelope balances would silently split in
 * two. Accepts "2026-08", "2026-08-15" or an ISO timestamp; returns "2026-08-01".
 */
function normalizeBudgetMonth(v) {
  if (typeof v !== "string" && !(v instanceof Date)) return null;
  const raw = v instanceof Date ? v.toISOString() : v.trim();
  if (!raw) return null;
  // Accept YYYY-MM as well as a full date, so a month picker can post either.
...
// ── ETF holdings: cache → Alpha Vantage → curated fallback ───────────────────
// Returns a normalized cache row { symbol, name, asset_class, vehicle, holdings,
// sectors, source, fetched_at } for a known-universe symbol, or null. Mutual
// funds (Amana) skip Alpha Vantage entirely (AV is ETF-only) and read curated
// data. Never throws — degrades to stale cache or curated so overlap always renders.
async function getHoldingsRecord(symbol) {
  const sym = String(symbol || "").toUpperCase().trim();
  const meta = universeMeta(sym);
  if (!meta || !sbAdmin) return null;

  let cached = null;
  try {
    const { data } = await sbAdmin.from("etf_holdings_cache").select("*").eq("symbol", sym).maybeSingle();
    cached = data || null;
  } catch (err) { warn("etf.cache.read_failed", { sym, err: err.message }); }

  // Freshness by row PROVENANCE, not just source: an Alpha Vantage row lasts a
  // day; a curated MUTUAL-FUND row is authoritative for ~90d; but a curated row
  // for an ETF is only a FALLBACK (AV throttled/down) — expire it in ~1h so a
  // transient AV rate-limit doesn't pin an AV-covered ETF to curated for months.
  const ttl = cached?.source === "alphavantage"
    ? ETF_CACHE_TTL_MS
    : meta.vehicle === "mutual_fund"
      ? CURATED_TTL_MS
      : ETF_CURATED_FALLBACK_TTL_MS;
  const isFresh = cached && (Date.now() - new Date(cached.fetched_at).getTime()) < ttl;
  if (isFresh) return cached;

  // ETFs → try Alpha Vantage live (mutual funds fall straight through to curated)
  if (meta.vehicle === "etf" && ALPHAVANTAGE_KEY) {
    try {
      const url = `https://www.alphavantage.co/query?function=ETF_PROFILE&symbol=${encodeURIComponent(sym)}&apikey=${ALPHAVANTAGE_KEY}`;
      const res = await fetchWithRetry(url, {}, { maxRetries: 1 });
      const raw = await res.json().catch(() => null);
      const parsed = parseAlphaVantageProfile(raw);
      if (parsed) {
        const row = {
          symbol: sym, name: meta.name, asset_class: meta.assetClass, vehicle: meta.vehicle,
          holdings: parsed.holdings, sectors: parsed.sectors, source: "alphavantage",
          fetched_at: new Date().toISOString(),
        };
        try { await sbAdmin.from("etf_holdings_cache").upsert(row); } catch (err) { warn("etf.cache.write_failed", { sym, err: err.message }); }
        return row;
      }
      // AV returned a rate-limit note / empty payload / uncovered symbol → fall back
      warn("etf.av.unusable", { sym, note: raw?.Information || raw?.Note || "empty" });
    } catch (err) { warn("etf.av.failed", { sym, err: err.message }); }
  }

  // Curated fallback (Amana always; ETFs when AV is absent/rate-limited/uncovered)
  const cur = CURATED_HOLDINGS[sym];
  if (cur) {
    const row = {
      symbol: sym, name: meta.name, asset_class: meta.assetClass, vehicle: meta.vehicle,
      holdings: cur.holdings, sectors: null, source: "curated", fetched_at: new Date().toISOString(),
    };
    try { await sbAdmin.from("etf_holdings_cache").upsert(row); } catch (err) { warn("etf.cache.write_failed", { sym, err: err.message }); }
    return row;
  }
...
//     })))
// SnapTrade signs with Python's json.dumps(sig_object, separators=(",",":"),
// sort_keys=True), which sorts object keys ALPHABETICALLY AND RECURSIVELY.
// JSON.stringify preserves insertion order, so any multi-key body (e.g. the
// 8-field /trade/impact payload) produced a content string the server couldn't
// reproduce → 401 "Unable to verify signature" (code 1076). Null-content GETs
// and single-key POST bodies happened to match, which is why reads worked.
// stableStringify mirrors json.dumps(sort_keys=True) with compact separators.
...
function buildSignature(reqPath, queryString, bodyObj) {
  ...
  return crypto
    ...
    .update(stableStringify(sigObject))
    .digest("base64");
}
...
// Finnhub proxy GET → JSON via fetchWithRetry (429 / Retry-After-aware backoff +
// exponential retry on 5xx / network errors). Finnhub's free tier is 60 req/min
// and the holdings accordion fans out news + earnings + profile + metric per
// holding on expand, so bursts hit 429; fetchWithRetry honors the Retry-After
// header the API returns (capped at 30s) before retrying. Returns the SAME
// { status, body } shape as httpsGetJson — JSON.parse'd body, or raw text on a
// parse failure — so the four Finnhub routes keep their exact response shapes.
// On exhausted retries it degrades softly to { status: 0, body: null } so each
// route falls through to its existing empty-shape fallback instead of 500-ing.
async function finnhubGetJson(urlStr) {
  try {
    const res = await fetchWithRetry(urlStr, {}, { maxRetries: 2 });
    const text = await res.text();
    let body;
    try { body = JSON.parse(text); } catch { body = text; }
    return { status: res.status, body };
  } catch (err) {
    warn("finnhub.fetch_failed", { err: err?.message });
    return { status: 0, body: null };
  }
}

// ── In-memory Finnhub response cache (per warm serverless instance) ──────────
// News + earnings payloads are IDENTICAL across users for a given window, but
// the holdings accordion fans them out per-holding on expand — so without a
// shared cache every user (and every re-expand) re-spends the 60/min Finnhub
// free-tier budget on the same data, which is exactly what trips the 429s F7
// hardens against. Cache the SHAPED response body keyed by the request
// signature, with short TTLs (news 5m, earnings 30m). Per-instance +
// migration-free by design — a de-dup buffer, not a source of truth (mirrors
// _cache in lib/sharia.mjs and _marketQuoteCache below). The client-side
// accordion cache stays in front of this untouched, so this only absorbs
// cross-user / cold-client repeats.
const _finnhubRespCache = new Map(); // key → { ts, body }
export const FINNHUB_NEWS_TTL_MS     = 5  * 60 * 1000; // 5 min  — headlines move intraday
export const FINNHUB_EARNINGS_TTL_MS = 30 * 60 * 1000; // 30 min — calendar barely changes
// Symbol search: a company's name and ticker do not change on any timescale a
// typeahead cares about, so cache hard. This is what keeps a keystroke-driven
// feature off the 60/min Finnhub budget — a popular prefix is fetched once and
// then served from memory for hours across every user on the instance.
export const FINNHUB_SEARCH_TTL_MS   = 6 * 60 * 60 * 1000; // 6 h

// Pure freshness decision (exported for unit tests): is a cache entry still
// within its TTL at nowMs? A missing/malformed entry is never fresh.
export function finnhubCacheFresh(entry, ttlMs, nowMs = Date.now()) {
  return !!entry && Number.isFinite(entry.ts) && (nowMs - entry.ts) < ttlMs;
}
function finnhubCacheGet(key, ttlMs) {
  const hit = _finnhubRespCache.get(key);
  if (finnhubCacheFresh(hit, ttlMs)) return hit.body;
  if (hit) _finnhubRespCache.delete(key); // expired → evict so the Map can't grow unbounded
  return null;
}
function finnhubCacheSet(key, body) {
  _finnhubRespCache.set(key, { ts: Date.now(), body });
}
...
// SnapTrade nests symbol metadata one or two levels deep, and sometimes hands
// back a bare string. Unwrap to a plain ticker; "" when there is nothing usable.
export function snapTicker(symLike) {
  let sym = symLike;
  let depth = 0;
  while (sym && typeof sym === "object" && sym.symbol && typeof sym.symbol === "object" && depth < 3) {
    sym = sym.symbol;
    depth += 1;
  }
  const tk = typeof sym === "string" ? sym : (sym?.symbol || sym?.raw_symbol || sym?.ticker || "");
  return typeof tk === "string" ? tk.trim().toUpperCase() : "";
}

// Is this activity a cash dividend? Deliberately narrower than the client's
// display bucketing (src/components/MizanApp.jsx inferType), which folds
// interest and dividend FEES in with dividends so the Activity tab can group
// them. Purification is about dividend income actually received: a DFEE is
// money out, and interest is riba — a different ruling that must not be
// presented as a purifiable dividend.
export function isDividendActivity(rawType) {
  const t = (rawType == null ? "" : String(rawType)).toUpperCase().trim();
  if (!t) return false;
  if (t === "CDIV" || t === "GDIV" || t === "QDIV" || t === "DIV") return true;
  if (t.includes("DFEE")) return false;
  return t.includes("DIVIDEND");
}

// SnapTrade returns broker-native activity `type` values — notably "CONTRIBUTION"
// for a cash deposit (and "WITHDRAWAL" for cash out), alongside BUY/SELL/DIVIDEND/etc.
// The client net-worth chart + performance math (src/lib/performance.js) key off a
// canonical set where money-IN must be "DEPOSIT". A raw "CONTRIBUTION" was invisible
// to the contribution model, so the net-worth line collapsed to a fake ramp from $0
// with a wildly inflated gain. Normalize at the source so every consumer sees
// consistent types. Conservative: only remaps unambiguous cash-in/out synonyms;
// BUY/SELL/DIVIDEND/INTEREST/FEE pass through untouched (uppercased).
function canonicalActivityType(raw) {
  const t = (raw == null ? "" : String(raw)).toUpperCase().trim();
  if (!t) return "";
  if (t === "DEPOSIT" || t === "WITHDRAWAL") return t;
  if (/CONTRIBUTION|TRANSFER[\s_-]?IN|FUNDS?[\s_-]?RECEIVED|\bDEPOSIT\b|ACH[\s_-]?IN|EFT[\s_-]?IN/.test(t)) return "DEPOSIT";
  if (/WITHDRAW|TRANSFER[\s_-]?OUT|DISTRIBUTION|DISBURSE|ACH[\s_-]?OUT|EFT[\s_-]?OUT/.test(t)) return "WITHDRAWAL";
  return t;
}
...
async function applyRateLimit(pathname, headers, query = {}) {
  ...
  if (!userId) {
    ...
    // Anon respects a tighter per-action cap when one exists (e.g. marketdata=150),
    // otherwise the blanket anon default — but never looser than 300/hr per IP.
    const anonMax = Math.min(RATE_LIMITS[action] ?? RATE_LIMITS.anon, RATE_LIMITS.anon);
    return checkRateLimit(null, `ip:${ip}`, action, anonMax);
  }
...
// Full-auto is gated on the `full_auto_enabled` allowlist (owner-controlled —
// set only via SQL, there is NO self-serve toggle). Decoupled from is_root on
// 2026-07-01 by the owner's explicit, informed decision, to let a
// permission-granted family beta tester run autonomous execution on their OWN
// account (the person the bot was built for).
// COMPLIANCE: full-auto = autonomous execution on a user's brokerage account.
// Extending it beyond the owner can implicate RIA registration — keep the
// allowlist to accounts you are personally authorized to trade, and revisit
// before ANY compensated / commercial arrangement. Owner-reviewed 2026-07-01.
...
// ── Per-user Alpaca credentials (migration 029) ──────────────────────────
// Stored AES-256-GCM encrypted in user_keys. There is no plaintext column for
// the secret, and the write path refuses when ENCRYPTION_KEY is unset, so an
// Alpaca trading credential cannot land in the clear.
const ALPACA_KEY_COLS =
  ...
  "alpaca_key_last4, alpaca_paper, alpaca_updated_at";

/**
 * Resolve which Alpaca account a request acts on.
 *
 * Order: the user's OWN credentials, else the shared env pair. Returns null
 * when neither exists, and `source` so callers can tell the user which account
 * they are looking at — "am I on my own paper account or the shared one" must
 * never be a guess when the answer changes whose blotter you are reading.
 *
 * FAILS CLOSED on a decryption error. A user who has stored their own key and
 * whose key cannot be decrypted must NOT be quietly dropped onto the shared
 * account: they would place orders they believe are theirs into someone else's
 * blotter. Better to return nothing and surface the fault.
 */
async function getAlpacaCreds(user) {
  if (user && sbAdmin) {
    const { data, error } = await sbAdmin
      .from("user_keys").select(ALPACA_KEY_COLS).eq("user_id", user.id).maybeSingle();
    if (error) {
      warn("alpaca.creds.read_failed", { err: error.message });
    } else if (data?.alpaca_key_id_ciphertext && data?.alpaca_secret_ciphertext) {
      try {
        return {
          keyId:  decryptSecret({ ciphertext: data.alpaca_key_id_ciphertext, iv: data.alpaca_key_id_iv, authTag: data.alpaca_key_id_auth_tag }),
          secret: decryptSecret({ ciphertext: data.alpaca_secret_ciphertext, iv: data.alpaca_secret_iv, authTag: data.alpaca_secret_auth_tag }),
          source: "user",
          last4:  data.alpaca_key_last4 || null,
          paper:  data.alpaca_paper !== false,
        };
      } catch (e) {
        logError("alpaca.creds.decrypt_failed", { err: e.message });
        return null; // fail closed — never fall through to the shared account
      }
    }
  }
  if (ALPACA_KEY_ID && ALPACA_SECRET) {
    return { keyId: ALPACA_KEY_ID, secret: ALPACA_SECRET, source: "shared", last4: ALPACA_KEY_ID.slice(-4), paper: true };
  }
  return null;
}

/** Prove a credential pair works, and that it is a PAPER pair. */
async function verifyAlpacaCreds({ keyId, secret }) {
  let res;
  try {
    // paper-api only authenticates paper credentials, so a 200 here IS the
    // paper check — stronger than sniffing the key prefix, which Alpaca has
    // never documented as stable.
    res = await fetchWithRetry(`${ALPACA_BASE}/account`, {
      headers: { "APCA-API-KEY-ID": keyId, "APCA-API-SECRET-KEY": secret },
    });
  } catch {
    return { ok: false, status: 502, error: "Could not reach Alpaca to verify the credentials." };
  }
  if (res.status === 401 || res.status === 403) {
    return { ok: false, status: 400, error: "Alpaca rejected those credentials. Check you copied a PAPER key pair from the paper dashboard." };
  }
  if (res.status < 200 || res.status >= 300) {
    return { ok: false, status: 502, error: `Alpaca returned ${res.status} while verifying the credentials.` };
  }
  const acct = await res.json().catch(() => ({}));
  return { ok: true, accountNumber: acct?.account_number || null, status: acct?.status || null };
}

/** Settled cash for the account behind these creds, or null if unknown. */
async function alpacaAccountCash({ keyId, secret }) {
  try {
    const r = await fetchWithRetry(`${ALPACA_BASE}/account`, {
      headers: { "APCA-API-KEY-ID": keyId, "APCA-API-SECRET-KEY": secret },
    });
    if (!r.ok) return null;
    const j = await r.json().catch(() => ({}));
    const c = Number(j?.cash);
    return Number.isFinite(c) ? c : null;
  } catch { return null; }   // unknown cash → the ceiling reports unenforced
}

/** Asset metadata (we want `fractionable`), or null if it can't be read. */
async function alpacaAsset(symbol, { keyId, secret }) {
  try {
    const r = await fetchWithRetry(`${ALPACA_BASE}/assets/${encodeURIComponent(symbol)}`, {
      headers: { "APCA-API-KEY-ID": keyId, "APCA-API-SECRET-KEY": secret },
    });
    if (!r.ok) return null;
...
async function placeAlpacaOrder({ symbol, qty = null, notional = null, side, type = "market", limitPrice = null, timeInForce = "day", allowExtendedHours = false, creds = null }) {
  const keyId  = creds?.keyId  || ALPACA_KEY_ID;
  const secret = creds?.secret || ALPACA_SECRET;
  if (!keyId || !secret) return { ok: false, status: 503, error: "Alpaca not configured" };
  const symbolUp = String(symbol || "").toUpperCase();
  if (!symbolUp) return { ok: false, status: 400, error: "symbol required" };
  // Shape of the size: qty XOR notional, and fractional implies a day order.
  // See lib/market/orders.mjs — refusing here names the reason instead of
  // relaying Alpaca's 422.
  const sizing = validateOrderSizing({ qty, notional, type, timeInForce });
  if (!sizing.ok) return { ok: false, status: 400, code: sizing.code, error: sizing.error };
  ...
  // Session gate. Callers must OPT IN to extended hours; without the flag this
  // behaves exactly as it did before — regular hours only. The refusal for a
  // market order outside regular hours is deliberate and stricter than the
  // broker: Alpaca would silently QUEUE it to the next session, so the user
  // believes they traded and then fills hours later at a price they never saw.
  const session = marketSession();
  if (!allowExtendedHours && session.session !== "regular") {
    return { ok: false, status: 425, market_closed: true, session: session.session,
             error: `market_closed (${session.reason})` };
  }
  if (allowExtendedHours) {
    const v = validateSessionOrder({ type, timeInForce, limitPrice });
    if (!v.ok) {
      return { ok: false, status: v.code === "market_closed" ? 425 : 400,
               market_closed: v.code === "market_closed", code: v.code,
               session: v.session.session, error: v.error };
    }
  }

  const orderBody = { symbol: symbolUp, side, type, time_in_force: timeInForce };
  if (notional !== null && notional !== undefined && notional !== "") orderBody.notional = String(notional);
  else orderBody.qty = String(qty);
  ...
  // Alpaca only honours this in pre/after sessions, and only for limit + day/gtc
  // — all enforced above. Sending it during regular hours is a no-op, but we
  // omit it anyway so the request says exactly what it means.
  if (session.extendedHours) orderBody.extended_hours = true;

  // A fractional order on a non-fractionable asset is rejected by Alpaca after
  // the fact ("requested asset is not fractionable"). Ask first so the caller
  // gets a reason, not a broker error. Only spent when it can actually apply.
  if (sizing.isFractional) {
    const asset = await alpacaAsset(symbolUp, { keyId, secret });
    if (asset && asset.fractionable === false) {
      return { ok: false, status: 400, code: "not_fractionable",
               error: `${symbolUp} cannot be traded fractionally. Use a whole-share qty.` };
    }
  }

  // THE CEILING IS CASH, NOT BUYING POWER — margin is riba. Alpaca reports 4x
  // buying power against cash by default, so the permissive field is the one
  // sitting right there in the same response. See lib/market/orders.mjs.
  if (side === "buy") {
    const cash = await alpacaAccountCash({ keyId, secret });
    const value = estimateOrderValue({ qty, notional, price: limitPrice });
    const ceiling = withinCashCeiling({ side, estimatedValue: value, cash });
    if (!ceiling.ok) return { ok: false, status: 400, code: ceiling.code, error: ceiling.error };
  }
...
// US equity market clock. `usMarketStatus` keeps its exact old contract
// ({ open, reason }, open ONLY during 09:30–16:00 ET) so the bot cron and the
// SnapTrade execution gate below are unchanged — both place MARKET orders,
// which every broker rejects outside regular hours ("Outside market hours",
// impact 403 code 1019).
//
// The session detail that extended-hours trading needs lives in the same
// module (`marketSession`, `validateSessionOrder`) rather than in a second
// clock beside this one. Moved out of this file 2026-08-20; the holiday set
// moved with it, so there is exactly one list to update each year.
// See lib/market/sessions.mjs.

// Approval window for a DCA / accumulation buy. Unlike a momentum/breakout
// signal (whose price thesis goes stale fast, so the 60-min pending_signals
// default is right), a DCA buy is deliberately price-insensitive — "buy a whole
// share on a weekly cadence and hold for years". A 60-min window means a
// semi-auto user must catch the push and approve within the hour or the whole
// week's contribution is silently skipped. Give it the full trading day instead:
// capped at today's 16:00 ET close (executeSnapTradeOrder won't fill after
// hours, so a later expiry would only invite market_closed), and floored at the
// 60-min default so a late-in-day signal is never SHORTENED below current behavior.
// DCA signals are only generated while the market is open (the cron gates on
// usMarketStatus), so `now` is always inside 09:30–16:00 ET here.
function dcaSignalExpiryIso(now = new Date()) {
  ...
  const minsET = hour * 60 + parseInt(p.minute, 10);
  const CLOSE = 16 * 60; // 16:00 ET
  const closeMs = now.getTime() + (CLOSE - minsET) * 60000; // now → 16:00 ET (same ET day)
  const floorMs = now.getTime() + 60 * 60000;               // never shorter than the 60-min default
  return new Date(Math.max(closeMs, floorMs)).toISOString();
}
...
async function executeSnapTradeOrder({ userId, userSecret, accountId, ticker, side, qty, orderType = "Market" }) {
  ...
  // Defense-in-depth: never hit the broker with a market order when the market
  // is closed (covers the manual approve path + any future caller). The cron
  // gates earlier so it doesn't even generate signals after-hours.
  const mkt = usMarketStatus();
  if (!mkt.open) return { ok: false, status: 425, market_closed: true, error: `market_closed (${mkt.reason})` };
...
// ── Polygon aggregate bars — shared fetch + 24h cache + backoff ─────────
// Backs both /api/polygon/bars (backtester) and /api/market/candles (price
// chart), so the OHLC fetch/cache logic lives in exactly one place. Cache rows
// are in polygon_cache, keyed UNIQUE(ticker, from_date, to_date, timespan);
// cacheTimespan() encodes the multiplier for sub-day resolutions so a 5-minute
// fetch never collides with a 1-minute (or the backtester's daily) row.
//
// This function's output is a pure function of (ticker, from, to, timespan,
// multiplier) — it is IMPERSONAL. Never thread a userId in here. It never
// throws: on an upstream failure it serves a stale cache row when allowStale
// is set, otherwise an empty array.
async function getPolygonBars({ ticker, from, to, timespan = "day", multiplier = 1, allowStale = false }) {
  const tickerUp = String(ticker || "").toUpperCase();
  if (!tickerUp || !POLYGON_KEY) return { bars: [], cached: false, stale: false };
  const cacheSpan = cacheTimespan(multiplier, timespan);
  const TTL_MS = 24 * 3600 * 1000;

  let staleRow = null;
  if (sbAdmin) {
    try {
      // Destructure `error` too. Without it a PostgREST failure is
      // indistinguishable from a cache miss, so a systematically broken read
      // for one key shape just looks like "that timeframe is never cached" —
      // which is exactly the state production is in for intraday bars, with no
      // log anywhere saying why. See memory supabase-error-check.
      const { data: hit, error: cacheErr } = await sbAdmin.from("polygon_cache")
        .select("data, cached_at")
        .eq("ticker", tickerUp).eq("from_date", from).eq("to_date", to).eq("timespan", cacheSpan)
        .maybeSingle();
      if (cacheErr) warn("polygon.bars.cache_query_error", { err: cacheErr.message, ticker: tickerUp, timespan: cacheSpan });
      if (hit?.cached_at) {
        const rows = Array.isArray(hit.data) ? hit.data : [];
        if ((Date.now() - new Date(hit.cached_at).getTime()) < TTL_MS) {
          return { bars: rows, cached: true, stale: false };
        }
        staleRow = rows; // keep as last-resort fallback if the refetch fails
      }
    } catch (e) {
  ...
  const polyUrl =
    ...
    `/range/${multiplier}/${encodeURIComponent(timespan)}/${encodeURIComponent(from)}/${encodeURIComponent(to)}` +
    `?apiKey=${encodeURIComponent(POLYGON_KEY)}&adjusted=true&sort=asc&limit=50000`;
  let arr = [];
  try {
    const res  = await fetchWithRetry(polyUrl, {}, { maxRetries: 2 });
    const body = await res.json().catch(() => ({}));
    arr = Array.isArray(body?.results) ? body.results : [];
  } catch (e) {
    warn("polygon.bars.fetch_failed", { err: e?.message, symbol: tickerUp });
    if (allowStale && staleRow) return { bars: staleRow, cached: true, stale: true };
    return { bars: [], cached: false, stale: false };
  }
  ...
  if (sbAdmin && arr.length > 0) {
    sbAdmin.from("polygon_cache").upsert({
      ticker: tickerUp, from_date: from, to_date: to, timespan: cacheSpan,
      data: arr, cached_at: new Date().toISOString(),
    }, { onConflict: "ticker,from_date,to_date,timespan" })
      .then(({ error }) => { if (error) warn("polygon.bars.cache_write_failed", { err: error.message }); });
  }
  if (arr.length === 0 && allowStale && staleRow) return { bars: staleRow, cached: true, stale: true };
  return { bars: arr, cached: false, stale: false };
}

// Short-lived in-memory quote cache for /api/market/quote. The chart polls a
// live price every ~12s per open symbol; this collapses concurrent viewers of
// the same symbol onto one upstream call within the window. Per-instance and
// tiny by design — it's a de-dup buffer, not a source of truth.
const _marketQuoteCache = new Map(); // symbol → { ts, quote }
const MARKET_QUOTE_TTL_MS = 12_000;
...
async function _handle({ method, pathname, query, body, headers, rawBody }) {
  ...
  //
  // SCOPED to the unauthenticated auth surface only (/api/auth/*). The block
  // used to gate EVERY endpoint, which made it self-inflicting: a user's own
  // browser reports its failed logins (auth.jsx → /api/auth/track-failure), so
  // honest password typos tripped the counter and 429'd Plaid, portfolio, and
  // everything else for 24h — while a real attacker, who hits Supabase Auth
  // directly and never calls our reporter, was never affected. Gating only the
  // auth surface keeps the throttle where it belongs; a valid session always
  // reaches its data endpoints (which are independently auth-checked anyway).
  const ip = clientIp(h);
  if (pathname.startsWith("/api/auth/") && await isIpBlocked(sbAdmin, ip)) {
    warn("ip_block.rejected", { ip, path: pathname });
    return { status: 429, body: { error: "Temporarily blocked due to suspicious activity" }, headers: { "Retry-After": "1800" } };
  }
  ...
  if (pathname === "/api/snaptrade/login" && method === "POST") {
    ...
    if (loginRes.status !== 200) {
      ...
      if (loginRes.body?.code === "1012" || /does not support trade/.test(bodyTxt)) {
        // SnapTrade rejects trade auth for brokers it can only read (e.g.
        // Robinhood, Fidelity). Not transient — tell the user plainly instead
        // of the misleading "please try again."
        code = "BROKER_NO_TRADE_SUPPORT";
        const prettyBroker = broker ? broker.charAt(0).toUpperCase() + broker.slice(1).toLowerCase() : "This broker";
        message = `${prettyBroker} can only be connected in read-only mode — SnapTrade doesn't support placing trades on it. To trade, connect a trade-enabled broker such as E*Trade.`;
      } else if (/limit|quota|plan/.test(bodyTxt)) {
        code = "SNAPTRADE_LIMIT_REACHED";
        message = "Brokerage connection limit reached for this account. Please contact support.";
      }
  ...
  if (pathname === "/api/snaptrade/all") {
    ...
    const settled = await Promise.allSettled(accounts.map(async acct => {
      // Positions + balances in parallel. Most brokers (E*Trade, Robinhood, …)
      // return ONLY balance.total on the /accounts object — actual cash and
      // buying power live on /accounts/{id}/balances. Reading acct.balance.cash
      // alone made those accounts show $0 cash on Overview. On the real-time
      // plan these balances are live, so a bot trade reflects on the next sync.
      const [posRes, balRes] = await Promise.all([
        snapReq("GET", `/accounts/${acct.id}/positions`, null, { userId, userSecret }),
        snapReq("GET", `/accounts/${acct.id}/balances`,  null, { userId, userSecret }),
      ]);
      const balArr = Array.isArray(balRes.body) ? balRes.body : [];
      const cashFromBalances  = balArr.reduce((s, b) => s + (Number(b?.cash)         || 0), 0);
      const buyingPowerSum    = balArr.reduce((s, b) => s + (Number(b?.buying_power) || 0), 0);
      const instName = acct.brokerage?.name || acct.institution_name || "Unknown";
      const positions = Array.isArray(posRes.body) ? posRes.body : [];
      // Prefer the authoritative /balances cash; fall back to the (rarely
      // present) inline cash, then 0.
      const cash = balArr.length ? cashFromBalances : (acct.balance?.cash?.amount || 0);
      // Account total. Many custodians — notably 401k providers (Empower,
      // Fidelity NetBenefits, Principal, …) — return NO top-level balance on
      // /accounts; the value lives only in the positions. When that's the case
      // derive it from cash + Σ(position market value), the invariant every
      // real account satisfies (mirrors the demo normalizer). Without this the
      // account arrives as balance:0 and gets dropped from every balance-driven
      // surface — Net Worth AND the Zakat account picker (which filters
      // balance > 0), which is why linked 401k's failed to appear there.
      const reportedTotal = acct.balance?.total?.amount || acct.total_value?.amount || 0;
      const posMarketValue = positions.reduce((s, p) => {
        const units = Number(p?.units);
        const price = Number(p?.price);
        return s + (Number.isFinite(units) && Number.isFinite(price) ? units * price : 0);
      }, 0);
      const balance = reportedTotal > 0
        ? reportedTotal
        : Math.round((cash + posMarketValue) * 100) / 100;
      return {
        accountId:     acct.id,
        ...
        balance,
        cash,
        buying_power:  balArr.length ? buyingPowerSum : null,
  ...
  if (pathname === "/api/snaptrade/activities") {
    ...
    const rawArr = Array.isArray(r.body) ? r.body : (r.body?.activities || []);
    const arr = rawArr.map(a => (a && typeof a === "object") ? { ...a, type: canonicalActivityType(a.type) } : a);
    // Telemetry: distinct RAW SnapTrade type strings, so we can confirm the
    // canonical mapping catches every cash-in/out variant (e.g. CONTRIBUTION).
    const rawTypes = [...new Set(rawArr.map(a => a && a.type).filter(Boolean))].slice(0, 40);
    info("snaptrade.activities.ok", { count: arr.length, startDate, endDate, rawTypes });
  ...
  if (pathname === "/api/finnhub/earnings") {
    ...
    // Shared per-instance cache (F7): the calendar is identical for every user
    // over this window, so serve a warm copy instead of re-spending the budget.
    const cacheKey = `earnings|${from}|${to}`;
    const cachedBody = finnhubCacheGet(cacheKey, FINNHUB_EARNINGS_TTL_MS);
    if (cachedBody) {
      info("finnhub.earnings.cache_hit", { from, to });
    ...
    const r = await finnhubGetJson(finUrl);
    const arr = Array.isArray(r.body?.earningsCalendar) ? r.body.earningsCalendar : [];
    const body = r.body && typeof r.body === "object" ? r.body : { earningsCalendar: [] };
    // Cache ONLY a real upstream success — never the soft-fail fallback (status 0
    // from exhausted retries), so a transient 429 can't pin an empty calendar.
    if (r.status === 200 && r.body && typeof r.body === "object") finnhubCacheSet(cacheKey, body);
  ...
  // ── Symbol typeahead (IMPERSONAL) ─────────────────────────────────────────
  // GET /api/market/symbols?q=apple → { symbols: [{symbol, name, type}] }
  //
  // Feeds the Screener's "screen any ticker" lookup so the user can type a
  // company name instead of having to already know the ticker. Auth-gated like
  // the other /api/market/* routes, with its own rate-limit bucket
  // (marketdata.search) so a person typing is never starved by the app's
  // background quote polling.
  //
  // COMPLIANCE: IMPERSONAL (lib/compliance/policy.mjs). A name matcher is a
  // pure function of the typed string — identical for every user, ordered by
  // the provider's text relevance and never by any judgment about the
  // investment. It must stay a matcher: do NOT attach halal verdicts to the
  // suggestions. A dropdown of names each stamped "Halal ✓" reads as a curated
  // list of things to buy, which is the line this app does not cross. The
  // verdict appears only after the user picks ONE symbol.
  if (pathname === "/api/market/symbols") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    const parsedQ = normalizeQuery(q.q);
    if (!parsedQ.ok) return { status: 400, body: { error: parsedQ.error } };
    if (!FINNHUB_KEY) return { status: 200, body: { symbols: [] } };

    const cacheKey = `search:${parsedQ.q.toLowerCase()}`;
    const cachedBody = finnhubCacheGet(cacheKey, FINNHUB_SEARCH_TTL_MS);
    if (cachedBody) return { status: 200, body: cachedBody };

    // exchange=US keeps the list to symbols our screening + price data actually
    // cover. Non-US listings would return suggestions that then fail to screen,
    // which is a worse experience than not offering them (see BACKLOG U2).
    const finUrl = `https://finnhub.io/api/v1/search?q=${encodeURIComponent(parsedQ.q)}`
      + `&exchange=US&token=${encodeURIComponent(FINNHUB_KEY)}`;
    const r = await finnhubGetJson(finUrl);
    // Upstream trouble must not break typing — an empty list degrades to "no
    // suggestions", and the user can still type the ticker directly.
    if (r.status !== 200 || !r.body || typeof r.body !== "object") {
      warn("market.symbols.upstream_failed", { status: r.status, q: parsedQ.q });
      return { status: 200, body: { symbols: [] } };
    }
    const symbols = normalizeSymbolSearch(r.body, { limit: MAX_SUGGESTIONS });
    const body = { symbols };
    finnhubCacheSet(cacheKey, body);
    return { status: 200, body };
  }
  ...
  if (pathname === "/api/finnhub/news") {
    ...
    if (symbol) {
      ...
      const to   = q.to   || today.toISOString().slice(0, 10);
      // Shared per-instance cache (F7): same symbol+window → same headlines for
      // every user. Serve a warm copy before spending the Finnhub budget.
      const cacheKey = `news|${symbol}|${from}|${to}`;
      const cachedBody = finnhubCacheGet(cacheKey, FINNHUB_NEWS_TTL_MS);
      if (cachedBody) {
        info("finnhub.company_news.cache_hit", { symbol });
        return { status: 200, body: cachedBody };
      }
      ...
      const r = await finnhubGetJson(url);
      const arr = Array.isArray(r.body) ? r.body : [];
      ...
      const body = { news };
      // Cache only a real upstream success (array body) — never a soft-fail
      // fallback, so a transient 429 can't pin an empty list for 5 min.
      if (r.status === 200 && Array.isArray(r.body)) finnhubCacheSet(cacheKey, body);
    ...
    const cachedBody = finnhubCacheGet(cacheKey, FINNHUB_NEWS_TTL_MS);
    if (cachedBody) {
      info("finnhub.market_news.cache_hit", {});
      return { status: 200, body: cachedBody };
    }
  ...
  // ── Metals spot: provider chain (feeds the Zakat nisab) ──────────────────
  // 2026-07-28: Stooq HARD-404'd its xauusd/xagusd CSVs. Because that was the
  // only source, every user's nisab silently fell back to the client's static
  // constants — i.e. Zakat was being computed against a stale gold price with
  // no visible failure. A single free endpoint disappearing must never break a
  // religious obligation, so quotes now come from an ordered PROVIDER CHAIN:
  // the first provider returning a sane gold+silver pair wins, and which one
  // won is reported in `source` so a degraded state is observable, not silent.
  // Every provider is keyless (Finnhub's free tier rejects commodity quotes).
  async function metalsFetch(url, { json = false, ms = 6000 } = {}) {
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), ms);
    try {
      const r = await fetch(url, {
        signal: ctl.signal,
        headers: { "User-Agent": "Mizan/1.0 (zakat-nisab)" },
      });
  ...
  // Stooq: one-line CSV, Close is column 6. Kept first so the original source
  // resumes automatically if they ever restore the symbols.
  async function metalsFromStooq() {
    const one = async (symbol) => {
      const text = await metalsFetch(`https://stooq.com/q/l/?s=${symbol}&f=sd2t2ohlc&h&e=csv`);
      const lines = String(text || "").trim().split(/\r?\n/);
      if (lines.length < 2) return NaN;
      return Number(lines[1].split(",")[6]);
    };
    const [gold, silver] = await Promise.all([one("xauusd"), one("xagusd")]);
    return { gold, silver };
  }

  // Yahoo chart API — same host/shape the quote fallback already uses
  // (fetchYahooQuotes). XAUUSD=X/XAGUSD=X are spot FX; GC=F/SI=F are the
  // front-month futures, which track spot closely enough for a nisab threshold.
  function metalsFromYahoo(goldSym, silverSym) {
    return async () => {
      const one = async (sym) => {
        const d = await metalsFetch(
          `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?interval=1d&range=1d`,
          { json: true },
        );
        return Number(d?.chart?.result?.[0]?.meta?.regularMarketPrice);
      };
      const [gold, silver] = await Promise.all([one(goldSym), one(silverSym)]);
      return { gold, silver };
    };
  }

  const METALS_PROVIDERS = [
    ["stooq",         metalsFromStooq],
    ["yahoo-spot",    metalsFromYahoo("XAUUSD=X", "XAGUSD=X")],
    ["yahoo-futures", metalsFromYahoo("GC=F", "SI=F")],
  ];

  // Walks the chain, returns the first sane pair. Sanity bounds catch a provider
  // that responds 200 with a placeholder/garbage price rather than erroring.
  async function fetchMetalsSpot() {
    const errors = [];
    for (const [name, fn] of METALS_PROVIDERS) {
      try {
        const { gold, silver } = await fn();
        const sane = (n, lo, hi) => Number.isFinite(n) && n > lo && n < hi;
        if (!sane(gold, 100, 100000) || !sane(silver, 1, 10000)) {
          throw new Error(`implausible quote gold=${gold} silver=${silver}`);
        }
        if (name !== METALS_PROVIDERS[0][0]) {
          warn("metals.spot.fallback_provider", { provider: name, tried: errors });
        }
        return { goldOz: gold, silverOz: silver, source: name };
      } catch (err) {
  ...
  // calculator. Converts USD/oz to USD/g and pre-computes the two nisab
  // thresholds (87.48 g gold, 612.36 g silver). No API key required.
  // 12-hour cache on success; short negative cache on total failure so a
  // dead upstream can't make every request pay a 3-provider timeout.
  //
  // Falls back to {ok:false, source:"fallback"} only when EVERY provider
  // fails, so the client renders its static defaults instead of NaN.
  if (pathname === "/api/metals/spot" && method === "GET") {
    const TROY_OZ_TO_G = 31.1034768;
    const CACHE_TTL_MS = 12 * 60 * 60 * 1000;
    const FAIL_TTL_MS  = 5 * 60 * 1000;
    const cache = globalThis.__mizan_metals_cache;
    if (cache && (Date.now() - cache.at) < (cache.body?.ok ? CACHE_TTL_MS : FAIL_TTL_MS)) {
      return { status: 200, body: cache.body };
    }
    const round2 = (n) => Math.round(n * 100) / 100;
    try {
      const { goldOz, silverOz, source } = await fetchMetalsSpot();
      const body = {
        ok: true,
        source,
      ...
      info("metals.spot.ok", { source, gold_oz: body.gold_usd_per_oz, silver_oz: body.silver_usd_per_oz, nisab_gold: body.nisab_gold_usd, nisab_silver: body.nisab_silver_usd });
      return { status: 200, body };
    } catch (err) {
      ...
      const body = {
        ...
        reason: "fetch_failed", error: err.message,
      };
      globalThis.__mizan_metals_cache = { at: Date.now(), body };
      return { status: 200, body };
    }
  ...
  if (pathname === "/api/user/features" && method === "GET") {
    ...
    // Weekly-digest opt-out flag (default true; column added in migration 023).
    // First/last name (migration 026) rides along on the same row read — the
    // client needs it at boot to decide whether to show the name prompt, and a
    // second round-trip for two strings isn't worth it.
    let email_digest = true;
    let first_name = "", last_name = "";
    try {
      const { data } = await sbAdmin
        .from("profiles").select("email_digest, first_name, last_name")
        .eq("id", user.id).maybeSingle();
      if (data && typeof data.email_digest === "boolean") email_digest = data.email_digest;
      first_name = data?.first_name || "";
      last_name  = data?.last_name  || "";
    } catch (e) { warn("user.features.profile_read_failed", { err: e.message }); }
    return {
      status: 200,
      body: {
        trading_bot, full_auto, is_root, trading_bot_consented, email_digest,
        first_name, last_name,
        // The client gates its "tell us your name" prompt on this rather than
        // re-deriving the rule, so the definition of "filled in" lives here.
        needs_name: !first_name || !last_name,
      },
    };
  }

  // POST /api/user/profile — set the caller's own first/last name.
  // Service-role write: profiles has no user-facing UPDATE policy, and giving
  // one would expose is_root / suspended on the same row.
  if (pathname === "/api/user/profile" && method === "POST") {
    ...
    const first_name = clean(parsed?.first_name);
    const last_name  = clean(parsed?.last_name);
    if (!first_name || !last_name) {
      return { status: 400, body: { error: "First and last name are both required." } };
    }
    // Letters (incl. accents/Arabic), spaces, hyphens and apostrophes only —
    // this is a display name in an admin table, not free-form text.
    const NAME_RE = /^[\p{L}\p{M}][\p{L}\p{M}\s'’-]*$/u;
    if (!NAME_RE.test(first_name) || !NAME_RE.test(last_name)) {
      return { status: 400, body: { error: "Names can only contain letters, spaces, hyphens and apostrophes." } };
    }
    const { error } = await sbAdmin
      ...
      .eq("id", user.id);
    if (error) {
      logError("user.profile.persist_failed", { err: error.message });
      return { status: 500, body: { error: "Could not save your name. Try again." } };
    }
    audit({ userId: user.id, action: "user.profile.name_set", headers: h });
    return { status: 200, body: { ok: true, first_name, last_name } };
  }

  // POST /api/user/attribution — record where this user came from (BACKLOG G2).
  //
  // FIRST WRITE WINS, enforced here and not only on the client. The client
  // already refuses to overwrite, but the client is also where a stale tab, a
  // second device, or a re-install can send a fresh "direct" long after the
  // real first touch — so the authoritative guard lives on the server. Once a
  // user has an origin recorded, it is immutable.
  //
  // Stored in user_state rather than on `profiles` because a profiles column
  // needs a migration, and CLAUDE.md §8 gates those on an explicit owner ask.
  // The admin funnel reads these rows with the service role.
  //
  // PRIVACY: only the three short tokens are persisted, re-sanitized here.
  // Never a full referring URL — a referrer query string can carry search terms
  // or session ids. The client sends nothing else; this re-checks anyway.
  if (pathname === "/api/user/attribution" && method === "POST") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    ...
    const source = tok(parsed?.source);
    if (!source) return { status: 400, body: { error: "source is required" } };

    const { data: existing, error: readErr } = await sbAdmin
      .from("user_state").select("value")
      .eq("user_id", user.id).eq("key", "mizan_attribution").maybeSingle();
    if (readErr) {
      warn("user.attribution.read_failed", { err: readErr.message });
      return { status: 500, body: { error: "could not read attribution" } };
    }
    // Already attributed → succeed without changing anything. Returning 200
    // (not a conflict) keeps this a fire-and-forget call the client can make on
    // every load without needing to reason about it.
    if (existing?.value?.source) {
      return { status: 200, body: { ok: true, stored: false, source: existing.value.source } };
    }

    const value = {
      source,
      medium: tok(parsed?.medium) || "unknown",
      campaign: tok(parsed?.campaign) || null,
      at: new Date().toISOString(),
    };
    const { error } = await sbAdmin.from("user_state").upsert({
      user_id: user.id, key: "mizan_attribution", value,
      updated_at: new Date().toISOString(),
    });
    if (error) {
      warn("user.attribution.persist_failed", { err: error.message });
      return { status: 500, body: { error: "could not save attribution" } };
    }
    info("user.attribution.recorded", { source: value.source, medium: value.medium });
    return { status: 200, body: { ok: true, stored: true, source: value.source } };
  }

  // POST /api/user/email-digest — toggle the weekly digest email for the caller.
  if (pathname === "/api/user/email-digest" && method === "POST") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    const enabled = !!parsed.enabled;
    const { error } = await sbAdmin.from("profiles").update({ email_digest: enabled }).eq("id", user.id);
    if (error) { warn("user.email_digest.persist_failed", { err: error.message }); return { status: 500, body: { error: "db_error" } }; }
    audit({ userId: user.id, action: "user.email_digest.set", metadata: { enabled }, headers: h });
    return { status: 200, body: { ok: true, email_digest: enabled } };
  }

  if (pathname === "/api/advisor" && method === "POST") {
    // Auth FIRST — this proxies a paid Anthropic endpoint. Without this gate an
    // anonymous caller could bill Claude to Mizan (and use the proxy as a free
    // Claude relay). verifyUser here also tightens the rate-limit bucket to the
    // per-user "anthropic" cap (60/hr) instead of the looser anon-IP cap.
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    if (!ANTHROPIC_KEY) {
    ...
    // Compliance backstop (defense in depth). The hardened system prompt is the
    // primary guardrail; this scans the GENERATED answer for personalized
    // buy/sell/hold recommendations, suitability judgments, security rankings
    // for the user, and tailored price levels — and, on a hit, rewrites the
    // answer to the compliant redirect. Only applied to successful 200 turns;
    // tool_use (web_search) and error turns pass through untouched. Every flag
    // is logged + audited for human review and pattern tuning.
    let responseBody = anthropicRes.body;
    let complianceFlagged = false;
    if ((anthropicRes.status || 200) === 200) {
      const filtered = filterAdvisorResponse(anthropicRes.body);
      responseBody = filtered.body;
      complianceFlagged = filtered.flagged;
      if (filtered.flagged) {
        warn("advisor.compliance_filtered", { user: user.id, matched: filtered.matched.map(m => m.name) });
      }
    }
    ...
    try {
      audit({
        ...
        metadata: {
          ...
          model,
          status: anthropicRes.status,
          compliance_filtered: complianceFlagged,
        },
        headers: h,
      });
    } catch { /* audit is best-effort */ }
    // Never surface a raw upstream error body (Anthropic billing/credit/auth
    // messages) to end users — it leaks operator detail and points them at a
    // billing page that isn't theirs. The real status + request_id are logged
    // and audited above; the user gets a neutral, actionable message.
    const advisorStatus = anthropicRes.status || 200;
    if (advisorStatus >= 400) {
      warn("advisor.upstream_error", { status: advisorStatus, request_id: anthropicRes.body?.request_id });
      return { status: 502, body: { error: "The AI advisor is temporarily unavailable. Please try again in a bit." } };
    }
    return { status: advisorStatus, body: responseBody };
  }
  ...
  if (pathname === "/api/advisor/count" && method === "POST") {
    ...
    if (countRes.status !== 200 || typeof countRes.body !== "object") {
      warn("advisor.count.upstream_failed", { status: countRes.status, request_id: countRes.body?.request_id });
      // Don't leak the raw upstream error (e.g. Anthropic billing text), and
      // don't return a 400 — the client reads a 400 here as "context too large".
      // This is a best-effort pre-flight; a neutral 502 lets it fall through to
      // the real call instead of blocking the turn.
      return { status: 502, body: { error: "count check unavailable" } };
    }
  ...
  if (pathname === "/api/snaptrade/refresh" && method === "POST") {
    ...
    // Manual refresh is intentionally disabled for accounts on SnapTrade's
    // real-time / Pay-as-you-go plan — /accounts and /activities already
    // stream live, so there is nothing to "queue". SnapTrade signals this a
    // few different ways depending on the plan/connection state:
    //   • 403 + code 1141 "Manual refresh not enabled on the real-time plan"
    //   • 402 Payment Required (PAYG — manual refresh is a billable add-on)
    //   • a 4xx whose detail mentions the real-time plan / manual refresh
    // All of these are success-shaped state, not a failure. We detect them so
    // we don't render the misleading "rejected the refresh" toast. Genuinely
    // broken connections (401 auth, 404 unknown connection) are NOT matched
    // here and still surface as a hard failure so the user knows to reconnect.
    const isRealtimeBenign = r => {
      const detail = typeof r.body?.detail === "string" ? r.body.detail : "";
      return (
        r.status === 402 ||
        (r.status === 403 && (r.body?.code === "1141" || r.body?.code === 1141)) ||
        /real-?time plan|manual refresh|already.*real-?time|refresh (is )?not (needed|enabled|available)/i.test(detail)
      );
    };
    const queued = results.filter(r => r.status >= 200 && r.status < 300).length;
    const throttled = results.filter(r => r.status === 429).length;
    const realtimeDisabled = results.filter(isRealtimeBenign).length;
    const hardFailures = results.filter(r =>
      !(r.status >= 200 && r.status < 300) && r.status !== 429 && !isRealtimeBenign(r));

    audit({ userId: supabaseUser?.id, action: "broker.force_refresh", metadata: { total: auths.length, queued, throttled, realtimeDisabled, failed: hardFailures.length }, headers: h });

    // Some connections actually succeeded — partial success is still success.
    if (queued > 0) {
      info("snaptrade.refresh.ok", { queued, total: auths.length, throttled, realtimeDisabled, failed: hardFailures.length });
      return { status: 200, body: { ok: true, queued, total: auths.length, throttled, realtimeDisabled, results } };
    }

    // Nothing queued, but every connection is on the real-time plan (or PAYG
    // manual-refresh-off). Data is already live — report it as the no-op it is.
    if (hardFailures.length === 0 && realtimeDisabled > 0) {
      info("snaptrade.refresh.realtime_plan", { total: auths.length, throttled });
      return {
        status: 200,
        body: {
          ok: true,
    ...
    // Nothing queued and no hard failures → everything was throttled.
    ...
    // Genuine failure. Log what SnapTrade actually returned so this branch is
    // diagnosable instead of a blind 502.
    logError("snaptrade.refresh.failed", {
      total: auths.length,
      failures: hardFailures.map(r => ({ status: r.status, code: r.body?.code, detail: r.body?.detail || r.error })),
    });
  ...
  if (pathname === "/api/polygon/bars") {
    ...
    // Shared fetch + 24h cache + backoff (see getPolygonBars). Response shape
    // is preserved exactly for the backtester callers: { bars } or { bars, cached }.
    const { bars, cached } = await getPolygonBars({ ticker: symbol, from, to, timespan });
    info("polygon.bars.ok", { count: bars.length, symbol: symbol.toUpperCase(), from, to, timespan, cached });
    return { status: 200, body: cached ? { bars, cached: true } : { bars } };
  }

  // ── /api/market/candles — IMPERSONAL OHLC history for the price chart ───
  // COMPLIANCE CONTRACT: the response is a PURE FUNCTION of (symbol, resolution,
  // from, to). No userId, holdings, or preferences influence the output — two
  // different authenticated users requesting identical params get byte-identical
  // candles. The auth check below is ACCESS CONTROL (don't proxy market data to
  // anonymous callers or bill the upstream quota for them), NOT personalization.
  // Data tier: IMPERSONAL. See docs/COMPLIANCE.md + lib/compliance/policy.mjs.
  if (pathname === "/api/market/candles") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    const v = validateCandleQuery(q);
    if (v.error) return { status: 400, body: { error: v.error } };
    assertImpersonal("/api/market/candles response");
    const { bars, cached, stale } = await getPolygonBars({
      ticker: v.symbol, from: v.from, to: v.to,
      timespan: v.timespan, multiplier: v.multiplier, allowStale: true,
    });
    const candles = normalizeBars(bars);
    info("market.candles.ok", { symbol: v.symbol, resolution: v.resolution, count: candles.length, cached, stale });
    return { status: 200, body: { symbol: v.symbol, resolution: v.resolution, candles, cached: !!cached, stale: !!stale } };
  }

  // ── /api/market/session — IMPERSONAL market clock ───────────────────────
  // Which session the US equity market is in right now, and what that implies
  // for order entry. A pure function of the clock — no user data of any kind,
  // so it is Tier 1 IMPERSONAL like candles and quote.
  //
  // Server-owned deliberately: a browser clock can be wrong, in the wrong zone,
  // or simply lying, and "am I allowed to place this order" must not be decided
  // by the client. The UI renders what this says; the order path re-checks it
  // anyway (placeAlpacaOrder calls marketSession itself), so a tampered client
  // gets a 425 rather than an unexpected fill.
  if (pathname === "/api/market/session") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    assertImpersonal("/api/market/session response");
    const s = marketSession();
    return { status: 200, body: {
      session:       s.session,        // pre | regular | after | closed
      label:         s.label,
      reason:        s.reason,
      tradeable:     s.tradeable,
      extendedHours: s.extendedHours,
      requiresLimit: s.requiresLimit,
      asOf:          new Date().toISOString(),
    } };
  }

  // ── /api/market/quote — IMPERSONAL latest quote (short-lived) ──────────
  // Same contract as /api/market/candles: a pure function of `symbol`, no user
  // personalization. Powers the chart's optional live last-price line. Data
  // tier: IMPERSONAL.
  if (pathname === "/api/market/quote") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    const symbol = String(q.symbol || "").toUpperCase();
    if (!SYMBOL_RE.test(symbol)) return { status: 400, body: { error: "invalid symbol" } };
    assertImpersonal("/api/market/quote response");
    const hit = _marketQuoteCache.get(symbol);
    if (hit && (Date.now() - hit.ts) < MARKET_QUOTE_TTL_MS) {
      return { status: 200, body: { symbol, quote: hit.quote, cached: true } };
    }
    const raw = await finnhubQuote(symbol); // { current, prevClose } | null
    const quote = raw
      ? {
          price:     raw.current,
          prevClose: raw.prevClose,
          changePct: raw.prevClose ? ((raw.current - raw.prevClose) / raw.prevClose) * 100 : null,
        }
      : null;
    if (_marketQuoteCache.size > 500) _marketQuoteCache.clear(); // per-instance de-dup buffer, bounded
    _marketQuoteCache.set(symbol, { ts: Date.now(), quote });
    return { status: 200, body: { symbol, quote } };
  }
  ...
  if (pathname.startsWith("/api/plaid/")) {
        ...
        if (user_id && sbAdmin && NEEDS_EMAIL.has(webhook_code)) {
          try {
            ...
            if (email) {
              ...
              const body = `Your connected bank requires you to sign in again so we can keep balances and transactions in sync. Visit ${APP_BASE_URL} and click Re-authorize on ${instLabel}.`;
    ...
    if (pathname === "/api/plaid/link-token" && method === "POST") {
      try {
        ...
        const baseConfig = {
          ...
          country_codes: ["US"],
          language: "en",
          redirect_uri: process.env.PLAID_REDIRECT_URI || `${APP_BASE_URL}/oauth-redirect`,
          webhook: process.env.PLAID_WEBHOOK_URL || `${APP_BASE_URL}/api/plaid/webhook`,
        };
    ...
    // ?live=1 forces a real-time pull from the institution via accountsBalanceGet
    // (used by the manual "Sync" button); the default accountsGet returns Plaid's
    // cached balance, which is what the cheap 90s background poll uses.
    if (pathname === "/api/plaid/accounts") {
      const live = q.live === "1" || q.live === "true";
      try {
        ...
        const settled = await Promise.allSettled(tokens.map(async t => {
          try {
            const r = live
              ? await plaidClient.accountsBalanceGet({ access_token: t.access_token })
              : await plaidClient.accountsGet({ access_token: t.access_token });
            return {
              ok: true,
      ...
      if (snap?.userId && snap?.userSecret) {
        ...
        auths.forEach(a => {
          ...
          items.push({
            ...
            // Trade vs read-only surfacing. `type` is the connection type this
            // authorization was created with ("read"/"trade"); brokerage.allows_trading
            // is whether the broker can EVER trade via SnapTrade (Robinhood = false).
            // Lets the client badge each connection accurately.
            trade_type:            typeof a.type === "string" ? a.type.toLowerCase() : null,
            broker_allows_trading: a.brokerage?.allows_trading === false ? false
                                   : a.brokerage?.allows_trading === true ? true : null,
          });
        });
      }
    } catch (e) {
...
`;
  ...
  // POST /api/nav-usage — body {path}. Increments a per-user view counter.
  // Counters only: no timeline, no financial data, no ip/user_agent. See
  // migration 028 and src/lib/navUsage.js for why it is shaped this way.
  if (pathname === "/api/nav-usage" && method === "POST") {
    if (!sbAdmin) return { status: 503, body: { error: "Supabase not configured" } };
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "Unauthenticated" } };

    const raw = typeof parsed.path === "string" ? parsed.path.trim().toLowerCase() : "";
    // Validate at the boundary. Anything that is not a plain nav path is
    // dropped silently rather than stored — this table must never become a
    // place arbitrary client strings can land.
    if (!raw || raw.length > 96 || !/^[a-z0-9]+(?:\/[a-z0-9 \/-]+)*$/.test(raw)) {
      return { status: 400, body: { error: "invalid path" } };
    }
    const { error } = await sbAdmin.rpc("increment_nav_usage_admin", { p_user: user.id, p_path: raw });
    if (error) {
      if (isMissingRelationError(error)) return { status: 200, body: { ok: false, skipped: "migration_pending" } };
      // Never fail a request over instrumentation.
      info("nav_usage.write.failed", { err: error.message });
      return { status: 200, body: { ok: false } };
    }
    return { status: 200, body: { ok: true } };
  }

  // ── Envelope budgeting (migration 027) ──────────────────
  // Supersedes /api/budgets, which had no month dimension and therefore could
  // not do rollover. The old route stays for now; it has never had a mounted UI
  // and holds no rows.
  //
  // GET /api/budget
  //   Returns the caller's WHOLE budget history (capped), not just one month.
  //   That is deliberate: envelope balances are PATH-DEPENDENT — each month's
  //   leftover feeds the next — so a client given only the current month would
  //   compute a plausible WRONG balance. src/lib/budgetPlan.js computes the
  //   series; this route just supplies every month it needs.
  if (pathname === "/api/budget" && method === "GET") {
    if (!sbAdmin) return { status: 503, body: { error: "Supabase not configured" } };
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "Unauthenticated" } };

    // 36 months is ~5 years of realistic use at a few categories each, and
    // bounds the response so a long-lived account can't return unboundedly.
    const HISTORY_MONTHS = 36;
    const since = new Date();
    since.setUTCMonth(since.getUTCMonth() - HISTORY_MONTHS, 1);
    const sinceISO = since.toISOString().slice(0, 10);

    const [entriesRes, monthsRes] = await Promise.all([
      sbAdmin.from("budget_entries")
        .select("month, category, budgeted, carryover")
        .eq("user_id", user.id).gte("month", sinceISO)
        .order("month", { ascending: true }),
      sbAdmin.from("budget_months")
        .select("month, manual_income")
        .eq("user_id", user.id).gte("month", sinceISO)
        .order("month", { ascending: true }),
    ]);
    for (const r of [entriesRes, monthsRes]) {
      if (r.error) {
        if (isMissingRelationError(r.error)) {
          info("budget.read.migration_pending");
          return missingMigrationResponse("Budgeting", "027_envelope_budgeting.sql");
        }
        logError("budget.read.failed", { err: r.error.message });
        return { status: 500, body: { error: r.error.message } };
      }
    }
    return { status: 200, body: {
      ...
      months: (monthsRes.data || []).map(r => ({
        month: String(r.month).slice(0, 10),
        manual_income: r.manual_income === null ? null : Number(r.manual_income),
      })),
    } };
  }

  // PUT /api/budget/entry — body {month, category, budgeted, carryover}.
  // budgeted === null removes the category from that month entirely.
  if (pathname === "/api/budget/entry" && method === "PUT") {
    if (!sbAdmin) return { status: 503, body: { error: "Supabase not configured" } };
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "Unauthenticated" } };

    const month = normalizeBudgetMonth(parsed.month);
    if (!month) return { status: 400, body: { error: "month must be a valid YYYY-MM-01 date" } };
    const category = typeof parsed.category === "string" ? parsed.category.trim() : "";
    if (!category) return { status: 400, body: { error: "category required" } };
    if (category.length > 64) return { status: 400, body: { error: "category too long" } };

    const raw = parsed.budgeted;
    if (raw === null || raw === undefined) {
      const { error } = await sbAdmin.from("budget_entries").delete()
        .eq("user_id", user.id).eq("month", month).eq("category", category);
      if (error) {
        if (isMissingRelationError(error)) return missingMigrationResponse("Budgeting", "027_envelope_budgeting.sql");
        return { status: 500, body: { error: error.message } };
      }
      return { status: 200, body: { ok: true, deleted: true, month, category } };
    }

    const budgeted = Number(raw);
    if (!Number.isFinite(budgeted) || budgeted < 0) {
      return { status: 400, body: { error: "budgeted must be a non-negative number" } };
    }
    const { error } = await sbAdmin.from("budget_entries").upsert({
      user_id: user.id, month, category,
      budgeted, carryover: !!parsed.carryover, updated_at: new Date().toISOString(),
    }, { onConflict: "user_id,month,category" });
    if (error) {
      if (isMissingRelationError(error)) return missingMigrationResponse("Budgeting", "027_envelope_budgeting.sql");
      logError("budget.entry.write.failed", { err: error.message });
      return { status: 500, body: { error: error.message } };
    }
    return { status: 200, body: { ok: true, month, category, budgeted, carryover: !!parsed.carryover } };
  }

  // PUT /api/budget/month — body {month, manual_income}.
  // The fallback for users with no linked bank: envelope budgeting needs to
  // know how much money actually arrived, and Plaid can't say when there is no
  // Plaid. null means "derive from linked accounts instead".
  if (pathname === "/api/budget/month" && method === "PUT") {
    if (!sbAdmin) return { status: 503, body: { error: "Supabase not configured" } };
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "Unauthenticated" } };
    ...
    const raw = parsed.manual_income;
    let manual_income = null;
    if (raw !== null && raw !== undefined && raw !== "") {
      manual_income = Number(raw);
      if (!Number.isFinite(manual_income) || manual_income < 0) {
        return { status: 400, body: { error: "manual_income must be a non-negative number or null" } };
      }
    }
    const { error } = await sbAdmin.from("budget_months").upsert({
      user_id: user.id, month, manual_income, updated_at: new Date().toISOString(),
    }, { onConflict: "user_id,month" });
    if (error) {
      if (isMissingRelationError(error)) return missingMigrationResponse("Budgeting", "027_envelope_budgeting.sql");
      logError("budget.month.write.failed", { err: error.message });
      return { status: 500, body: { error: error.message } };
    }
    return { status: 200, body: { ok: true, month, manual_income } };
  }
      ...
      try {
        ...
        arr.forEach((a) => {
          ...
          rows.push({
            ...
            type:         canonicalActivityType(a?.type),
            ticker:       typeof ticker === "string" ? ticker : "",
            units:        Number.isFinite(units) ? units : "",
            price:        Number.isFinite(price) ? price : "",
  ...
  if (pathname === "/api/admin/db-status") {
    ...
    // Every table-creating migration → its tables. Column/policy/RPC-only
    // migrations (006 sessions, 011 cursor, 013–017 RLS/encrypt, 022 bot beta
    // cols) create no tables, so they can't be table-probed and are omitted.
    // Keep this in sync with supabase/migrations/ so the "all applied" badge
    // can't read green while later schema is actually missing.
    const MIGRATIONS = [
      { file: "001_init.sql",                  tables: ["user_snaptrade", "user_state", "user_keys", "profiles"] },
      { file: "002_plaid.sql",                 tables: ["plaid_tokens", "plaid_accounts", "plaid_transactions"] },
      { file: "003_ratelimit.sql",             tables: ["rate_limits"] },
      { file: "004_push.sql",                  tables: ["push_subscriptions"] },
      { file: "005_polygon.sql",               tables: ["polygon_cache"] },
      { file: "007_cron.sql",                  tables: ["cron_jobs"] },
      { file: "008_nicknames.sql",             tables: ["account_nicknames"] },
      { file: "009_budgets.sql",               tables: ["budgets"] },
      { file: "010_goals.sql",                 tables: ["goals"] },
      { file: "012_audit_log.sql",             tables: ["audit_log"] },
      { file: "018_security_events.sql",       tables: ["security_events"] },
      { file: "019_purification.sql",          tables: ["purification_ratios"] },
      { file: "020_trading_bot.sql",           tables: ["bot_strategies", "pending_signals"] },
      { file: "021_full_auto_per_account.sql", tables: ["account_full_auto"] },
    ];
    ...
    // Upstream feed health, probed live. alert:false — this is an on-demand
    // admin read, so it must not fire owner emails every panel refresh; the
    // daily cleanup cron owns alerting.
    let feeds = {};
    try {
      feeds = await checkDataFeeds(sbAdmin, APP_BASE_URL, { alert: false });
    } catch (e) { warn("admin.db_status.data_feeds_failed", { err: e.message }); }

    // Credential/env preflight, same alert:false reasoning as feeds above.
    let config = {};
    try {
      config = await checkConfig(sbAdmin, { alert: false });
    } catch (e) { warn("admin.db_status.config_failed", { err: e.message }); }
  ...
  // Vercel's scheduled crons are unreliable on the Hobby plan, so this
  // lets the owner fire a named cron on demand from the admin panel. It
  // proxies an authenticated POST to the same prod deployment's
  // /api/cron/<job> route (using CRON_SECRET) so the cron runs through
  // its normal fail-closed auth path.
  if (pathname === "/api/admin/run-cron" && method === "POST") {
    const gate = await requireAdmin();
    if (gate.early) return gate.early;

    const ALLOWED_JOBS = ["sync", "cleanup", "nightly-snapshot", "dividend-check", "bill-reminders", "weekly-digest", "activation", "bot-signals"];
    const job = parsed.job;
    if (!ALLOWED_JOBS.includes(job)) {
      return { status: 400, body: { error: "invalid job" } };
    }

    const cronSecret = process.env.CRON_SECRET;
    if (!cronSecret) {
      warn("admin.run_cron.no_secret", { job });
      return { status: 503, body: { error: "CRON_SECRET not configured" } };
    }

    info("admin.run_cron.start", { job, adminId: gate.user?.id });
    const cronResp = await fetch(`${APP_BASE_URL}/api/cron/${job}`, {
      method: "POST",
      headers: { Authorization: `Bearer ${cronSecret}` },
    });
    const cronStatus = cronResp.status;
    let result;
    const rawText = await cronResp.text().catch(() => "");
    try {
    ...
    audit({ userId: gate.user?.id, action: "admin.run_cron", target: job, metadata: { cronStatus }, headers: h });
    info("admin.run_cron.done", { job, cronStatus });
    return { status: 200, body: { ok: true, job, cronStatus, result } };
  }

  // ── Admin: one-time backfill — encrypt existing plaintext secrets at rest ──
  // Owner-only. Encrypts user_snaptrade.snaptrade_user_secret + user_keys.*_key
  // into the ciphertext columns (migration 016) with ENCRYPTION_KEY. Idempotent
  // (skips rows already ciphertext or with no plaintext), round-trip verified per
  // row. Leaves plaintext intact so reads keep working until 017 drops them.
  // Server-side so the key never leaves prod. Mirrors scripts/encrypt-existing-
  // secrets.mjs. See BACKLOG O24.
  if (pathname === "/api/admin/encrypt-backfill" && method === "POST") {
    const gate = await requireAdmin();
    if (gate.early) return gate.early;
    if (!ENC_ENABLED) {
      return { status: 503, body: { error: "ENCRYPTION_KEY not set — set it in Vercel and redeploy before running this." } };
    }

    const snap = { migrated: 0, skipped: 0, errors: 0 };
    const keys = { migrated: 0, skipped: 0, errors: 0 };

    // 1. user_snaptrade.snaptrade_user_secret
    const { data: snapRows, error: snapErr } = await sbAdmin
      .from("user_snaptrade")
      .select("user_id, snaptrade_user_secret, secret_ciphertext");
    if (snapErr) {
      logError("admin.encrypt_backfill.read_failed", { table: "user_snaptrade", err: snapErr.message });
      return { status: 500, body: { error: `read user_snaptrade: ${snapErr.message}` } };
    }
    for (const row of snapRows || []) {
      if (row.secret_ciphertext) { snap.skipped++; continue; }
      if (!row.snaptrade_user_secret) { snap.skipped++; continue; }
      try {
        const enc = encryptSecret(row.snaptrade_user_secret);
        if (decryptSecret(enc) !== row.snaptrade_user_secret) throw new Error("round-trip mismatch");
        const { error: upErr } = await sbAdmin
          .from("user_snaptrade")
          .update({ secret_ciphertext: enc.ciphertext, secret_iv: enc.iv, secret_auth_tag: enc.authTag })
          .eq("user_id", row.user_id);
        if (upErr) throw new Error(upErr.message);
        snap.migrated++;
      } catch (e) {
        snap.errors++;
        logError("admin.encrypt_backfill.snap_row_failed", { err: e.message });
      }
    }

    // 2. user_keys.finnhub_key + polygon_key (non-fatal if the table read fails)
    const { data: keyRows, error: keyErr } = await sbAdmin
      .from("user_keys")
      .select("user_id, finnhub_key, polygon_key, finnhub_key_ciphertext, polygon_key_ciphertext");
    if (keyErr) {
      logError("admin.encrypt_backfill.read_failed", { table: "user_keys", err: keyErr.message });
    } else {
      for (const row of keyRows || []) {
        const updates = {};
        for (const field of ["finnhub_key", "polygon_key"]) {
          if (row[`${field}_ciphertext`]) { keys.skipped++; continue; }
          if (!row[field]) { keys.skipped++; continue; }
          try {
            const enc = encryptSecret(row[field]);
            if (decryptSecret(enc) !== row[field]) throw new Error("round-trip mismatch");
            ...
            keys.errors++;
            logError("admin.encrypt_backfill.key_row_failed", { field, err: e.message });
          }
        }
        if (Object.keys(updates).length > 0) {
          updates.encrypted = true;
          const { error: upErr } = await sbAdmin.from("user_keys").update(updates).eq("user_id", row.user_id);
          if (upErr) { keys.errors++; logError("admin.encrypt_backfill.key_write_failed", { err: upErr.message }); }
        }
      }
    }

    audit({ userId: gate.user?.id, action: "admin.encrypt_backfill", target: "secrets", metadata: { snap, keys }, headers: h });
    info("admin.encrypt_backfill.done", { snap, keys });
    return { status: 200, body: { ok: true, snaptrade: snap, keys } };
  }

  // ── Admin: invite a user (branded) ──────────────────────
  // Owner-only. Generates a Supabase invite action-link WITHOUT triggering
  // Supabase's default (unbranded, poorly-delivering) auth email, then sends a
  // MĪZAN-branded invite through the app's Resend path (renderBrandedEmail via
  // sendUserEmail, from ALERT_FROM = alerts@mizan.exchange). Fixes both the
  // off-brand look and lets the owner preview before sending. See BACKLOG F8.
  if (pathname === "/api/admin/invite" && method === "POST") {
    ...
    // type:"invite" creates the (unconfirmed) user if needed and returns the
    // action link, but sends no email — so we own the template and the sender.
    ...
    if (linkErr) {
      logError("admin.invite.link_failed", { err: linkErr.message });
      const already = /already|registered|exists/i.test(linkErr.message || "");
      return { status: 400, body: { error: already ? "That email is already invited or registered." : linkErr.message } };
    }
    const actionLink = linkData?.properties?.action_link;
    if (!actionLink) return { status: 500, body: { error: "Could not generate an invite link" } };
    ...
    const body =
      ...
      "Tap below to accept your invitation and set up your account. This link is personal to you — please don't forward it.";
    const sent = await sendUserEmail(email, "You're invited to MĪZAN", body, sbAdmin, {
      eyebrow:   "Private invitation",
      title:     "You're invited to MĪZAN",
      ctaUrl:    actionLink,
      ctaLabel:  "Accept your invitation →",
      ctaUrl2:   "https://www.mizan.exchange",
      ctaLabel2: "Learn more about Mizan →",
      footNote:  "You received this invitation from Mizan. If you weren't expecting it, you can safely ignore this email.",
      variant:   "user",
    });
    if (!sent.ok) {
      return { status: 502, body: { error: `Invite created but the email failed to send (${sent.err || sent.skipped || "unknown"}).` } };
    }
    audit({ userId: gate.user?.id, action: "admin.user_invited", target: email, headers: h });
    return { status: 200, body: { ok: true, email } };
  }

  // ── Admin: broadcast audience (connection status per user) ──────
  // Powers the Broadcast composer's segments. Bank status is DB-cheap
  // (plaid_tokens); brokerage status needs a live SnapTrade /accounts call
  // per user, because SnapTrade holdings aren't mirrored in our DB. Done
  // best-effort with bounded concurrency + a per-user timeout so one slow
  // upstream call can't stall the endpoint. On a SnapTrade error the user's
  // brokerage count is null ("unknown") and they're treated as connected —
  // we'd rather under-nudge than email someone who is actually linked.
  // NB: fine at the current user count; add a cache before this scales.
  // ── Shared audience / connection status ──────────────────────────────────
  // ONE definition of "connected", used by the broadcast picker, the activation
  // cron, and the admin funnel metric, so the three can never drift apart.
  // Deliberately treats an UNKNOWN brokerage (upstream error/timeout) as
  // connected: it under-counts activation rather than nagging someone who has
  // in fact linked an account.
  //
  // maxAgeMs serves the admin panel, which loads /stats and /users together and
  // would otherwise hit SnapTrade once per user TWICE per refresh. Pass 0 (the
  // default) anywhere a decision is made — sends, disconnections, the cron.
  async function buildAudience({ maxAgeMs = 0 } = {}) {
    if (maxAgeMs > 0 && _audienceCache.data && (Date.now() - _audienceCache.at) < maxAgeMs) {
      return _audienceCache.data;
    }
    const built = await buildAudienceUncached();
    _audienceCache = { at: Date.now(), data: built };
    return built;
  }

  async function buildAudienceUncached() {
    const { data: profiles, error: profErr } = await sbAdmin
      .from("profiles")
      .select("id, email, first_name, last_name, created_at, suspended")
      .order("created_at", { ascending: true });
    if (profErr) throw new Error(profErr.message);

    // Bank connections: one query, count plaid_tokens per user.
    const { data: pt, error: ptErr } = await sbAdmin.from("plaid_tokens").select("user_id");
    if (ptErr) throw new Error(ptErr.message);
    const bankByUser = new Map();
    for (const row of pt || []) bankByUser.set(row.user_id, (bankByUser.get(row.user_id) || 0) + 1);

    // SnapTrade registrations → live account count (best-effort).
    // NOTE: SNAP_SECRET_COLS does NOT contain user_id, so this select must
    // prepend it explicitly. It didn't, which keyed every row under `undefined`
    // and made brokerageCount() return 0 for everyone — brokerage connections
    // were invisible and "connected" silently meant "has a bank". Fixed here.
    const { data: snapRows } = await sbAdmin.from("user_snaptrade").select(`user_id, ${SNAP_SECRET_COLS}`);
    const snapByUser = new Map();
    for (const row of snapRows || []) snapByUser.set(row.user_id, row);

    // Resolve p (a snapReq promise) or a {status:0} sentinel after `ms`,
    // clearing the timer once the race settles so it never leaks.
    const withTimeout = (p, ms) => {
      let t;
      const timeout = new Promise((res) => { t = setTimeout(() => res({ status: 0, body: null }), ms); });
      return Promise.race([p, timeout]).finally(() => clearTimeout(t));
    };
    async function brokerageCount(userId) {
      const row = snapByUser.get(userId);
      if (!row) return 0;                                    // never registered
      const secret = extractUserSecret(row);
      if (!row.snaptrade_user_id || !secret) return 0;
      try {
        const r = await withTimeout(
          snapReq("GET", "/accounts", null, { userId: row.snaptrade_user_id, userSecret: secret }),
          6000);
        if (r.status === 200 && Array.isArray(r.body)) return r.body.length;
        return null;                                         // upstream error/timeout → unknown
      } catch { return null; }
    }

    // Bounded-concurrency worker pool over profiles.
    const CONC = 5;
    const out = new Array(profiles.length);
    let idx = 0;
    async function worker() {
      while (idx < profiles.length) {
        const i = idx++;
        const p = profiles[i];
        const bank = (bankByUser.get(p.id) || 0) > 0;
        const brokerage = await brokerageCount(p.id);
        const connected = bank || brokerage === null || (brokerage || 0) > 0;
        out[i] = { id: p.id, email: p.email, name: fullName(p), created_at: p.created_at, suspended: !!p.suspended, bank, brokerage, connected };
      }
    }
    await Promise.all(Array.from({ length: Math.min(CONC, profiles.length || 1) }, worker));

    const active = out.filter((u) => !u.suspended);
    const counts = {
      all:          active.length,
      connected:    active.filter((u) => u.connected).length,
      notConnected: active.filter((u) => !u.connected).length,
    };
    return { users: out, counts };
  }

  if (pathname === "/api/admin/broadcast-audience") {
    const gate = await requireAdmin();
    if (gate.early) return gate.early;
    try {
      const { users, counts } = await buildAudience();
      return { status: 200, body: { users, counts } };
    } catch (e) {
      return { status: 500, body: { error: e.message } };
    }
  }

  // ── Admin: broadcast a branded email to selected users ─────────
  // Owner-only. Takes explicit userIds (NOT emails, NOT a segment name) and
  // resolves addresses from profiles server-side, so a broadcast can only
  // ever reach real Mizan users — never an arbitrary address. `testToSelf`
  // routes the send to the calling admin only (no client-side self id
  // needed). Emails send one at a time through the same branded
  // sendUserEmail path as invites; ONE summary row lands in audit_log
  // (action="admin.broadcast"). Suspended users are always skipped.
  if (pathname === "/api/admin/broadcast" && method === "POST") {
    const gate = await requireAdmin();
    if (gate.early) return gate.early;

    const testToSelf = parsed?.testToSelf === true;
    const userIds = Array.isArray(parsed?.userIds)
      ? [...new Set(parsed.userIds.filter((x) => typeof x === "string"))]
      : [];
    const subject   = String(parsed?.subject   || "").trim();
    const message   = String(parsed?.message   || "").trim();
    const title     = String(parsed?.title     || "").trim();
    const eyebrow   = String(parsed?.eyebrow   || "").trim();
    const ctaUrl    = String(parsed?.ctaUrl    || "").trim();
    const ctaLabel  = String(parsed?.ctaLabel  || "").trim();
    const ctaUrl2   = String(parsed?.ctaUrl2   || "").trim();
    const ctaLabel2 = String(parsed?.ctaLabel2 || "").trim();

    if (!subject) return { status: 400, body: { error: "A subject is required." } };
    if (!message) return { status: 400, body: { error: "A message is required." } };
    const httpOk = (u) => !u || /^https?:\/\//i.test(u);
    if (!httpOk(ctaUrl) || !httpOk(ctaUrl2)) {
      return { status: 400, body: { error: "Button links must start with http:// or https://" } };
    }

    // Resolve recipients from the DB — never trust client-supplied addresses.
    let recipients, skipped = 0;
    if (testToSelf) {
      if (!gate.user?.email) return { status: 400, body: { error: "Your account has no email to test with." } };
      recipients = [{ id: gate.user.id, email: gate.user.email }];
    } else {
      if (!userIds.length) return { status: 400, body: { error: "Select at least one recipient." } };
      if (userIds.length > 500) return { status: 400, body: { error: "Too many recipients (max 500)." } };
      const { data: profs, error: pErr } = await sbAdmin
        .from("profiles").select("id, email, suspended").in("id", userIds);
      if (pErr) return { status: 500, body: { error: pErr.message } };
      recipients = (profs || []).filter((p) => p.email && !p.suspended);
      skipped = (profs || []).length - recipients.length;
      if (!recipients.length) {
        return { status: 400, body: { error: "No sendable recipients (all missing an email or suspended)." } };
      }
    }
    ...
    const opts = {
      ...
      ctaLabel:  ctaLabel || "Open Mizan",
      ctaUrl2:   ctaUrl2  || undefined,
      ctaLabel2: ctaLabel2|| undefined,
      variant:   "user",
    };
    ...
    for (const p of recipients) {
      try {
        const s = await sendUserEmail(p.email, subject, message, sbAdmin, opts);
        results.push({ email: p.email, ok: !!s.ok, err: s.ok ? null : (s.err || s.skipped || "unknown") });
      } catch (e) {
        results.push({ email: p.email, ok: false, err: e?.message || "send threw" });
      }
    }
    const sent   = results.filter((r) => r.ok).length;
    const failed = results.length - sent;

    // One summary row — a per-recipient row would flood the log.
    audit({
      userId: gate.user?.id, action: "admin.broadcast", target: subject.slice(0, 200),
      metadata: {
        test: testToSelf, recipientCount: recipients.length, sent, failed, skipped,
        recipients: (!testToSelf && recipients.length <= 50) ? recipients.map((r) => r.email) : undefined,
      },
      headers: h,
    });
    info("admin.broadcast.done", { test: testToSelf, sent, failed, skipped });
    return { status: 200, body: { ok: true, test: testToSelf, sent, failed, skipped, results } };
  }

  // ── Messages: user's own support thread ─────────────────
  // Two-way async contact channel (migration 025). Served through the service
  // role so a read can atomically clear unread operator messages in the same
  // request. Motivated by users having no inbound mailbox to reply to.
  if (pathname === "/api/messages" && method === "GET") {
    ...
    // Mark operator→user messages read now that the user is viewing them.
    sbAdmin.from("messages")
      .update({ read_at: new Date().toISOString() })
      .eq("user_id", user.id).eq("sender", "admin").is("read_at", null)
      .then(({ error: e }) => { if (e) warn("messages.mark_read_failed", { err: e.message }); });
  ...
  if (pathname === "/api/messages" && method === "POST") {
    ...
    if (!body) return { status: 400, body: { error: "Message can't be empty." } };
    if (body.length > 4000) return { status: 400, body: { error: "Message too long (max 4000 characters)." } };
    const { data, error } = await sbAdmin
      .from("messages")
      .insert({ user_id: user.id, sender: "user", body })
      .select("id, sender, body, created_at, read_at").single();
    if (error) return { status: 500, body: { error: error.message } };
    audit({ userId: user.id, action: "message.sent", headers: h });
    // Notify the operator (best-effort). reply_to = the user so an email reply
    // reaches them even though the in-app thread is the primary channel.
    if (OWNER_EMAIL) {
      sendUserEmail(OWNER_EMAIL, `New Mizan message from ${user.email || "a user"}`,
        `${user.email || "A user"} wrote:\n\n${body}\n\nReply in Admin → Messages.`, sbAdmin,
        { eyebrow: "User message", title: "New message in Mizan", ctaUrl: `${APP_BASE_URL}/`, ctaLabel: "Open Mizan", replyTo: user.email || undefined })
        .catch((e) => warn("messages.notify_owner_failed", { err: e?.message }));
    }
    return { status: 200, body: { ok: true, message: data } };
  }

  // ── Admin: message threads (list, view, reply) ──────────
  if (pathname === "/api/admin/messages" && method === "GET") {
    ...
    if (uid) {
      const { data, error } = await sbAdmin
        .from("messages")
        .select("id, sender, body, created_at, read_at")
        .eq("user_id", uid).order("created_at", { ascending: true }).limit(500);
      if (error) return { status: 500, body: { error: error.message } };
      sbAdmin.from("messages")
        .update({ read_at: new Date().toISOString() })
        .eq("user_id", uid).eq("sender", "user").is("read_at", null)
        .then(({ error: e }) => { if (e) warn("messages.admin_mark_read_failed", { err: e.message }); });
      return { status: 200, body: { messages: data || [] } };
    }
    // Thread list: newest 1000 messages, grouped per user in JS (low volume).
    const { data, error } = await sbAdmin
      .from("messages")
    ...
    for (const m of data || []) {
      ...
      if (!t) { t = { user_id: m.user_id, lastBody: m.body, lastSender: m.sender, lastAt: m.created_at, unread: 0 }; threads.set(m.user_id, t); }
      if (m.sender === "user" && !m.read_at) t.unread++;
    }
    const ids = [...threads.keys()];
    let profById = new Map();
    if (ids.length) {
      const { data: profs } = await sbAdmin
        .from("profiles").select("id, email, first_name, last_name").in("id", ids);
      profById = new Map((profs || []).map((p) => [p.id, p]));
    }
    const list = [...threads.values()]
      .map((t) => ({
        ...t,
        email: profById.get(t.user_id)?.email || null,
        name:  fullName(profById.get(t.user_id)),
      }))
      .sort((a, b) => (a.lastAt < b.lastAt ? 1 : -1));
    return { status: 200, body: { threads: list } };
  }
  ...
  if (pathname === "/api/admin/messages" && method === "POST") {
    ...
    const body = String(parsed?.body || "").trim();
    if (!uid)  return { status: 400, body: { error: "userId is required." } };
    if (!body) return { status: 400, body: { error: "Message can't be empty." } };
    if (body.length > 4000) return { status: 400, body: { error: "Message too long (max 4000 characters)." } };
    const { data: prof } = await sbAdmin.from("profiles").select("email").eq("id", uid).maybeSingle();
    const { data, error } = await sbAdmin
      .from("messages")
      .insert({ user_id: uid, sender: "admin", body })
      .select("id, sender, body, created_at, read_at").single();
    if (error) return { status: 500, body: { error: error.message } };
    audit({ userId: gate.user?.id, action: "admin.message.replied", target: uid, headers: h });
    // Notify the user by email (best-effort) that there's a reply waiting.
    if (prof?.email) {
      sendUserEmail(prof.email, "You have a new message from Mizan",
        `${body}\n\nOpen Mizan → Settings → Messages to reply.`, sbAdmin,
        { eyebrow: "Message from Mizan", title: "New message from the Mizan team", ctaUrl: `${APP_BASE_URL}/`, ctaLabel: "Open Messages" })
        .catch((e) => warn("messages.notify_user_failed", { err: e?.message }));
    }
    return { status: 200, body: { ok: true, message: data } };
  }
  ...
  if (pathname === "/api/admin/users") {
    ...
    const { data: profiles, error: profErr, count } = await sbAdmin
      .from("profiles")
      .select("id, email, first_name, last_name, is_root, suspended, created_at", { count: "exact" })
      .order("created_at", { ascending: false })
    ...
    // ?activation=1 — per-user connection + nudge state. Answers "who hasn't
    // connected, and who has the automation already emailed?" in the panel
    // instead of in a hand-written SQL query against audit_log.
    if (q.activation === "1") {
      try {
        const audience = await buildAudience({ maxAgeMs: 60_000 });
        const connById = new Map((audience.users || []).map((u) => [u.id, u]));

        const { data: nudges, error: nudgeErr } = await sbAdmin.from("audit_log")
          .select("user_id, created_at")
          .eq("action", "user.activation_nudge")
          .order("created_at", { ascending: false });
        if (nudgeErr) throw new Error(nudgeErr.message);
        const nudgedAtById = new Map();
        for (const r of nudges || []) {
          if (r.user_id && !nudgedAtById.has(r.user_id)) nudgedAtById.set(r.user_id, r.created_at);
        }

        users = users.map((u) => {
          const conn = connById.get(u.id);
          const merged = { ...u, connected: conn ? conn.connected : null, bank: conn?.bank ?? null, brokerage: conn?.brokerage ?? null };
          const nudgedAt = nudgedAtById.get(u.id) || null;
          const { state, note } = activationState(merged, { nudgedAt, lastSignIn: u.last_sign_in });
          return { ...merged, nudged_at: nudgedAt, activation_state: state, activation_note: note };
        });
      } catch (e) {
        // The base roster is more important than the enrichment — degrade to it
        // rather than 500ing the whole panel on a SnapTrade hiccup.
        warn("admin.users.activation_failed", { err: e.message });
      }
    }
  ...
  // ── Admin: send one user the activation nudge by hand ───
  // The cron deliberately skips people it shouldn't automate at — someone who
  // signed in yesterday but still hasn't linked anything is usually BLOCKED
  // (a broker that won't connect), not uninterested, and that deserves a human
  // decision rather than a scheduled email. This is that decision, as a button.
  // Same copy and same audit action as the cron, so a manual send also marks
  // the user as nudged and the automation won't email them again.
  const nudgeMatch = pathname.match(/^\/api\/admin\/users\/([^/]+)\/nudge$/);
  if (nudgeMatch && method === "POST") {
    ...
    const { data: prof, error: profErr } = await sbAdmin
      .from("profiles").select("id, email, first_name, last_name, suspended")
      .eq("id", targetId).maybeSingle();
    if (profErr) return { status: 500, body: { error: profErr.message } };
    if (!prof?.email) return { status: 404, body: { error: "User not found or has no email" } };
    if (prof.suspended) return { status: 400, body: { error: "User is suspended" } };

    const { data: priorNudge } = await sbAdmin.from("audit_log")
      .select("created_at").eq("action", "user.activation_nudge").eq("user_id", targetId)
      .order("created_at", { ascending: false }).limit(1);
    // Re-sending is allowed (the owner asked for it explicitly by clicking),
    // but never silently — the response says so and the audit row records it.
    const resend = !!priorNudge?.[0];

    const { subject, text } = activationNudgeEmail(fullName(prof));
    try {
      const r = await sendUserEmail(prof.email, subject, text, sbAdmin);
      if (r && r.ok === false && !r.skipped) throw new Error(r.error || "send failed");
      audit({ userId: targetId, action: "user.activation_nudge", target: prof.email, metadata: { manual: true, resend, by: gate.user?.id }, headers: h });
      info("admin.nudge.sent", { targetId, resend });
      return { status: 200, body: { ok: true, email: prof.email, resend, skipped: r?.skipped || null } };
    } catch (e) {
      logError("admin.nudge.failed", { targetId, err: e.message });
  ...
  if (pathname === "/api/admin/audit-log") {
    ...
    const enriched = rows.map((r) => ({
      ...
      name:  fullName(profById.get(r.user_id)),
    }));
  ...
  if (pathname === "/api/admin/stats") {
    ...
    // Activation funnel. The number that actually gates growth: signups are
    // easy, connected-and-returning is not. "returned" = signed in on a later
    // calendar day than they signed up — a second session, not just the one.
    let funnel = null;
    try {
      const { users } = await buildAudience({ maxAgeMs: 60_000 });
      const real = (users || []).filter((u) => !u.suspended);
      let lastSignInById = new Map();
      try {
        const { data: au } = await sbAdmin.auth.admin.listUsers({ page: 1, perPage: 1000 });
        lastSignInById = new Map((au?.users || []).map((u) => [u.id, u.last_sign_in_at]));
      } catch { /* enrichment is best-effort */ }
      const returned = real.filter((u) => {
        const last = lastSignInById.get(u.id);
        if (!last) return false;
        return (new Date(last).getTime() - new Date(u.created_at).getTime()) > 24 * 3600 * 1000;
      }).length;
      const connected = real.filter((u) => u.connected).length;
      const nudged = await sbAdmin.from("audit_log")
        .select("*", { count: "exact", head: true }).eq("action", "user.activation_nudge");
      // Acquisition sources (BACKLOG G2). Users who signed up before
      // attribution shipped have no row, and summarizeSources surfaces them as
      // "unattributed" rather than quietly shrinking the denominator — a
      // breakdown that looks complete when it isn't is worse than no breakdown.
      let sources = [];
      try {
        const { data: attrRows, error: attrErr } = await sbAdmin
          .from("user_state").select("value").eq("key", "mizan_attribution");
        if (attrErr) throw new Error(attrErr.message);
        sources = summarizeSources((attrRows || []).map((r) => r.value || {}), real.length);
      } catch (e) { warn("admin.stats.sources_failed", { err: e.message }); }

      funnel = {
        signups:   real.length,
        connected,
        returned,
        connect_rate: real.length ? Math.round((connected / real.length) * 100) : 0,
        return_rate:  real.length ? Math.round((returned / real.length) * 100) : 0,
        nudges_sent:  nudged?.count || 0,
        sources,
      };
    } catch (e) { warn("admin.stats.funnel_failed", { err: e.message }); }
  ...
  // GET /api/etf/universe — the pickable halal ETF / fund list for the Overlap
  // Analyzer (symbol, name, vehicle, assetClass). Available to any signed-in user.
  if (pathname === "/api/etf/universe" && method === "GET") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    return { status: 200, body: { universe: ETF_UNIVERSE } };
  }

  // GET /api/etf/overlap?symbols=SPUS,HLAL — holdings + pairwise overlap for a
  // selection of 2–4 halal ETFs/funds. Holdings come from the daily cache
  // (Alpha Vantage for ETFs, curated for Amana mutual funds). Overlap % for each
  // pair = Σ min(weightA, weightB). Equity-vs-sukuk pairs are flagged not-comparable.
  if (pathname === "/api/etf/overlap" && method === "GET") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    const requested = String(q.symbols || "")
      .split(",").map((s) => s.trim().toUpperCase()).filter(Boolean);
    const symbols = [...new Set(requested)].filter((s) => universeMeta(s)).slice(0, 4);
    if (symbols.length < 2) return { status: 400, body: { error: "need_two_symbols" } };

    // Sequential (not Promise.all): each ETF fetch may hit Alpha Vantage, whose
    // free tier throttles concurrent bursts — firing 2–4 at once made the later
    // ones fall back to curated. Serializing keeps each on live AV data. Cache
    // hits make this near-instant after the first load.
    const rows = [];
    for (const s of symbols) rows.push(await getHoldingsRecord(s));
    const funds = symbols.map((s, i) => {
      const meta = universeMeta(s);
      const row = rows[i];
      return {
        symbol: s, name: meta.name, vehicle: meta.vehicle, assetClass: meta.assetClass,
        source: row?.source || null, asOf: row?.fetched_at || null,
        holdings: row?.holdings || [], sectors: row?.sectors || null,
        available: !!(row && Array.isArray(row.holdings) && row.holdings.length),
      };
    });

    const records = funds
      .filter((f) => f.available)
      .map((f) => ({ symbol: f.symbol, holdings: f.holdings, assetClass: f.assetClass }));
    const pairs = records.length >= 2 ? overlapMatrix(records) : [];
    return { status: 200, body: { funds, pairs } };
  }

  // GET  /api/bot/bogleheads — the sleeve menu (US / Intl / Sukuk + optional
  //                            tech & REIT tilts) that drives the frontend picker.
  // POST /api/bot/bogleheads — build a server-validated Halal Bogleheads DCA
  //                            preset from the chosen picks (body: { account_id, sel }).
  // Kept as one source of truth so the picker never hardcodes tickers/weights and
  // every leg is re-validated against its sleeve options + the Sharia blocklist.
  if (pathname === "/api/bot/bogleheads") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "unauthorized" } };
    if (!(await canUseTradingBot(user))) return { status: 403, body: { error: "trading_not_enabled" } };
    if (method === "GET") return { status: 200, body: { sleeves: HALAL_BOGLEHEADS_SLEEVES } };
    if (method === "POST") {
      const sel = (parsed && typeof parsed.sel === "object" && parsed.sel) ? parsed.sel : {};
      const accountId = typeof parsed?.account_id === "string" ? parsed.account_id : null;
      const strategy = buildBogleheadsPreset(accountId, sel);
      if (!Array.isArray(strategy.universe_tickers) || strategy.universe_tickers.length < 2) {
        return { status: 400, body: { error: "invalid_selection" } };
      }
      return { status: 200, body: { strategy, sleeves: HALAL_BOGLEHEADS_SLEEVES, disclaimer: SELF_DIRECTED_DISCLAIMER } };
    }
    return { status: 405, body: { error: "method_not_allowed" } };
  }
  ...
  if (pathname.startsWith("/api/bot/strategies")) {
    ...
    if (method === "POST" && pathname === "/api/bot/strategies") {
      ...
      // COMPLIANCE: full-auto (autonomous execution) requires the full_auto_enabled
      // allowlist (owner-controlled, no self-serve). Non-allowlisted users run
      // Manual/Semi (a human approves every trade on their own account) and can't
      // create a full-auto strategy.
      if ((effLayer === "full" || dbMode === "full") && !(await canUseFullAuto(user))) {
      ...
      // Normalize the deployable share to 1-100% of buying power (user's choice).
      if (mergedParams.deploy_pct != null) {
        mergedParams.deploy_pct = Math.min(100, Math.max(1, Math.round(Number(mergedParams.deploy_pct)) || 25));
      }
    ...
    if (method === "PATCH" && patchMatch) {
      ...
      const allowed = ["enabled","mode","ticker","account_id","strategy_type","params","capital_allocated","profit_target_pct","stop_loss_pct","max_drawdown_pct","time_horizon_days","max_trades_per_day","nl_risk_disclosed"];
      const updates = {};
      ...
      // COMPLIANCE: switching a strategy to full-auto requires the full_auto_enabled
      // allowlist (owner-controlled). Non-allowlisted users stay Manual/Semi.
      if ((parsed.layer === "full" || parsed.mode === "full") && !(await canUseFullAuto(user))) {
        return { status: 403, body: { error: "full_auto_root_only" } };
      }
  ...
  if (pathname.startsWith("/api/bot/signals")) {
    ...
    if (method === "POST" && approveMatch) {
      ...
      // Mark approved via an ATOMIC compare-and-swap (pending → approved), then
      // execute the REAL order via SnapTrade (Sharia gate re-applied inside
      // executeSnapTradeOrder). Layer 2 = semi-auto.
      //
      // The `.eq("status","pending")` + `.select()` turns this into a single
      // atomic `UPDATE ... WHERE id=? AND status='pending' RETURNING id`. Two
      // concurrent approve requests can never both win: exactly one gets the row
      // back, the other matches zero rows and bails BEFORE touching the broker.
      // This DB-level swap — not the earlier in-code `sig.status` read — is the
      // real duplicate-order guard, because SnapTrade's checked-place endpoint
      // exposes no client idempotency key (see executeSnapTradeOrder).
      const { data: claimed, error: claimErr } = await sbAdmin.from("pending_signals")
        .update({ status: "approved" })
        .eq("id", signalId)
        .eq("status", "pending")
        .select("id");
      if (claimErr) return { status: 500, body: { error: "db_error" } };
      if (!claimed || claimed.length === 0) {
        // Lost the race (or the signal was already approved/expired/rejected by
        // a concurrent request). Never fire a second broker order for one signal.
        audit({ userId: user.id, action: "bot.signal.approve_conflict", target: signalId, metadata: { ticker: sig.ticker, side: sig.side, layer: "semi" }, headers: h });
        return { status: 409, body: { error: "signal_already_processing" } };
      }
  ...
  if (pathname.startsWith("/api/bot/full-auto-accounts")) {
    ...
    if (method === "PATCH" && faMatch) {
      // COMPLIANCE: opting an account into full-auto requires the full_auto_enabled allowlist.
      if (!(await canUseFullAuto(user))) return { status: 403, body: { error: "full_auto_root_only" } };
  ...
  if (pathname === "/api/bot/strategy/nl" && method === "POST") {
    ...
    // Preset shortcut — the Halal Bogleheads three-fund lazy portfolio. Recognized
    // by keyword so it skips the LLM parse and returns the canonical DCA basket
    // (SPUS/UMMA/SPSK) for the user to fund + confirm in the review UI.
    if (/\b(bogle\w*|three[-\s]?fund|3[-\s]?fund|lazy[-\s]?portfolio|couch[-\s]?potato)\b/i.test(description)) {
      const sel = bogleheadsSelectionFromText(description);
      const strategy = buildBogleheadsPreset(accounts[0]?.id || null, sel);
      audit({ userId: user.id, action: "bot.strategy.nl_parsed", metadata: { strategy_type: "dca", preset: "halal_bogleheads" }, headers: h });
      // Return the full sleeve menu so the review UI can offer per-section options.
      return { status: 200, body: { strategy, description, preset: "halal_bogleheads", sleeves: HALAL_BOGLEHEADS_SLEEVES, disclaimer: SELF_DIRECTED_DISCLAIMER } };
    }
...
- If the user named NO explicit ticker to trade (only a theme, a style, or an open-ended goal), output ONLY {"needs_symbols": true} — never invent, propose, or "fit" symbols. MĪZAN is self-directed and does not pick securities for the user.
...
- universe_tickers (array of uppercase strings): ONLY the tickers the USER explicitly named. List every symbol the user named, correcting obvious typos to the real symbol (APPL→AAPL, NTFLX→NFLX, TESLA→TSLA, FB→META, GOOGLE→GOOGL) and KEEPING the corrected name — never drop a ticker merely for a misspelling. Every entry must be Sharia-compliant; omit any non-compliant name the user named (alcohol/tobacco/gambling/weapons/adult/conventional-finance) and any non-Shariah broad-market ETF (SPY, QQQ hold conventional financials — exclude them unless the user explicitly named a Shariah ETF such as SPUS, HLAL, UMMA). ⚠️ You MUST NOT invent, propose, suggest, add, or "fit" any ticker the user did not name. MĪZAN is a SELF-DIRECTED tool: the user chooses what to trade, and you only structure their own instruction — you never select securities for them. If the user named a THEME or goal but NO explicit ticker (e.g. "halal tech", "Shariah ETFs", "a good halal growth strategy", "double my money"), do NOT guess or propose any symbols — output ONLY the JSON object {"needs_symbols": true} and nothing else.
...
- deploy_pct (number, 1-100): the share of the account's BUYING POWER this strategy may deploy — the user's choice. Use the percentage the user states. If they give a range like "25-50%", use the LOWER end. If they don't mention one, default to 25 (conservative). The rest of buying power stays reserved/untouched.
- capital_allocated (number, USD): the deployable cash. If the user states total buying power, set this to buying_power x deploy_pct/100. If the user gives only an absolute deploy amount, use that amount. If only total buying power is stated with no percent, use 25%. 0 if no money is mentioned.
...
- params (object): { "universe": <universe>, "universe_tickers": <universe_tickers>, "entry_rules": <entry_rules>, "exit_rules": <exit_rules>, "position_size_pct": <position_size_pct>, "deploy_pct": <deploy_pct>, "detailed_rules": <detailed_rules>, "executed_as": <executed_as> } — mirror these into params so they persist in the params jsonb column and drive the screener + the strategy review.
- detailed_rules (string): capture the trader's FULL intent, including any intraday, multi-timeframe, key-level, divergence, premarket-low-stop, "N tests of a level", scalping, or scale-in/scale-out nuances — even the parts the daily engine cannot run tick-by-tick. This is the faithful record of what they asked for (3-5 sentences max).
- executed_as (string): one short sentence on how the daily, long-only spot engine actually approximates the above (e.g. "Daily breakout: buys the strongest universe name on positive day momentum, 10% stop, long-only spot — intraday scalping and multi-timeframe signals are not run tick-by-tick").
...
Keep entry_rules and exit_rules to at most 2 sentences each. If the description piles on many intraday/technical conditions (multiple timeframes, key levels, divergence, "N tests of a level", scalping, scaling in/out, adding to winners), distill them into the single closest testable LONG-ONLY edge — do not enumerate every condition. The bot executes daily-bar, long-only spot equity; intraday scalping and multi-timeframe TA are approximated, not run tick-by-tick.
...
Output the JSON object only.`;
    ...
    const truncated = nlRes.body?.stop_reason === "max_tokens";
    let parsedStrategy;
    try {
      // Claude should output only JSON. Strip accidental code fences, then slice
      // to the outermost {...} so any stray prose before/after can't break parse.
      let cleaned = content.replace(/^```json?\s*/i, "").replace(/```\s*$/i, "").trim();
      const first = cleaned.indexOf("{"), last = cleaned.lastIndexOf("}");
      if (first !== -1 && last > first) cleaned = cleaned.slice(first, last + 1);
      parsedStrategy = JSON.parse(cleaned);
    } catch {
      warn("bot.nl.parse_failed", { truncated, content: content.slice(0, 500) });
      return { status: 422, body: { code: "could_not_parse_strategy", error: truncated
        ? "That description was too long to structure in one pass. Trim it to one clear edge — entry, exit, and stop-loss — and a handful of tickers, then try again."
        : "I couldn't structure that into a strategy. Try naming a clear entry, an exit, a stop-loss, and a few tickers — e.g. “Buy SPY/QQQ breakouts above the prior-day high, 10% stop, take profit 10%, $100.”" } };
    }
    ...
    // SELF-DIRECTED guard (compliance): the user described a theme/goal but named
    // no explicit ticker. MĪZAN must NOT pick securities for them — instead point
    // them at the impersonal, same-for-everyone model presets to choose from.
    if (parsedStrategy && parsedStrategy.needs_symbols === true) {
      audit({ userId: user.id, action: "bot.strategy.nl_needs_symbols", headers: h });
      return { status: 200, body: {
        needs_symbols: true,
        message: "MĪZAN is a self-directed tool — you choose what to trade; it doesn't pick securities for you. Name the halal ticker(s) you want to trade (e.g. SPUS, HLAL, or a specific stock), or start from a ready-made model portfolio below (the same one every user sees).",
        preset: "halal_bogleheads",
        sleeves: HALAL_BOGLEHEADS_SLEEVES,
      } };
    }
    ...
    // Deployable share of buying power — user's choice, 1-100% (default 25).
    const deploy_pct      = Math.min(100, Math.max(1, Math.round(num(p.deploy_pct ?? p.params?.deploy_pct, 25)) || 25));
    const detailed_rules  = typeof p.detailed_rules === "string" ? p.detailed_rules : (p.params?.detailed_rules || "");
    const executed_as     = typeof p.executed_as === "string" ? p.executed_as : (p.params?.executed_as || "");
    // Deterministic typo backstop (in case the model still emits a misspelling).
    const TICKER_TYPOS = { APPL:"AAPL", APPLE:"AAPL", NTFLX:"NFLX", NETFLIX:"NFLX", NFLIX:"NFLX", TESLA:"TSLA", AMAZON:"AMZN", MICROSOFT:"MSFT", NVIDIA:"NVDA", FACEBOOK:"META", FB:"META", GOOGLE:"GOOGL", ALPHABET:"GOOGL" };
    const fixTk = t => { const u = String(t || "").toUpperCase().trim(); return TICKER_TYPOS[u] || u; };
    const tickerUp        = fixTk(p.ticker);
    // Candidate set the screener picks from — clean, fix typos, dedupe, drop
    // known-haram, and guarantee the primary ticker is included.
    ...
    const universe_tickers = [...new Set([tickerUp, ...rawCandidates.map(fixTk)].filter(Boolean))]
                              .filter(t => !HARAM_TICKERS.has(t));
    ...
    const strategy = {
      ...
      universe_tickers,
      params:             { universe, universe_tickers, entry_rules, exit_rules, position_size_pct, deploy_pct, detailed_rules, executed_as, layer: "semi" },
    ...
    return { status: 200, body: { strategy, description, disclaimer: SELF_DIRECTED_DISCLAIMER } };
  }
  ...
  // ── Activation nudge ─────────────────────────────────────────────────────
  // The leak this closes: a user signs up, lands on an empty app, never
  // connects an account, and nothing ever follows up. Measured 2026-07-29 —
  // every user who connected an account came back; every user who didn't,
  // didn't. Connection IS retention, and nothing was chasing it.
  //
  // Sends ONE email, ever, per user. Deduped on an audit_log row rather than a
  // new column, so no migration. Window is 1-30 days old: under a day is too
  // eager, over 30 days belongs in a hand-written broadcast, not an automation.
  if (pathname === "/api/cron/activation") {
    if (cronUnauthorized(h)) { warn("cron.activation.unauth"); return { status: 401, body: { error: "Unauthorized" } }; }
    if (!sbAdmin) return { status: 200, body: { ok: true, skipped: "no-supabase" } };

    const MIN_AGE_MS    = 24 * 3600 * 1000;
    const MAX_AGE_MS    = 45 * 24 * 3600 * 1000;
    // Someone who signed in this week is ENGAGED, not lapsed — telling them
    // "you haven't connected anything" is both wrong-feeling and the wrong
    // channel (that belongs in-app). This guard is what makes the wider 45-day
    // window safe: it targets dormant users, not merely unconnected ones.
    const ACTIVE_WINDOW = 7 * 24 * 3600 * 1000;
    let audience;
    try {
      audience = await buildAudience();
    } catch (e) {
      logError("cron.activation.audience_failed", { err: e.message });
      return { status: 200, body: { ok: false, error: e.message } };
    }

    // Everyone already nudged — one email per user for the life of the account.
    const { data: prior } = await sbAdmin.from("audit_log")
      .select("user_id").eq("action", "user.activation_nudge");
    const alreadyNudged = new Set((prior || []).map((r) => r.user_id).filter(Boolean));

    // last_sign_in_at lives on auth.users, not profiles. Best-effort: if this
    // lookup fails we fall back to nudging on age alone rather than skipping
    // the run entirely.
    let lastSignInById = new Map();
    try {
      const { data: au } = await sbAdmin.auth.admin.listUsers({ page: 1, perPage: 1000 });
      lastSignInById = new Map((au?.users || []).map((x) => [x.id, x.last_sign_in_at]));
    } catch (e) { warn("cron.activation.listusers_failed", { err: e.message }); }

    const now = Date.now();
    const targets = (audience.users || []).filter((u) => {
      if (u.suspended || u.connected || !u.email) return false;
      if (alreadyNudged.has(u.id)) return false;
      const last = lastSignInById.get(u.id);
      if (last && (now - new Date(last).getTime()) < ACTIVE_WINDOW) return false; // engaged
      const age = now - new Date(u.created_at).getTime();
    ...
    for (const u of targets) {
      const { subject, text } = activationNudgeEmail(u.name);
      try {
        const r = await sendUserEmail(u.email, subject, text, sbAdmin);
        if (r && r.ok === false && !r.skipped) throw new Error(r.error || "send failed");
        audit({ userId: u.id, action: "user.activation_nudge", target: u.email, metadata: { days_since_signup: Math.round((now - new Date(u.created_at).getTime()) / 86400000) }, headers: h });
        sent++;
      } catch (e) {
        failed++;
        warn("cron.activation.send_failed", { err: e.message });
      }
    }

    info("cron.activation.ok", { candidates: targets.length, sent, failed });
    audit({ action: "cron.activation", metadata: { candidates: targets.length, sent, failed }, headers: h });
    return { status: 200, body: { ok: true, candidates: targets.length, sent, failed } };
  }
  ...
  if (pathname === "/api/cron/cleanup") {
    ...
    // Upstream data-feed health. Piggybacks on the daily cleanup rather than
    // adding a cron path (Vercel Hobby = daily-only). Alerts the owner when a
    // third-party feed dies quietly — see checkDataFeeds() for why.
    let feeds = {};
    try {
      feeds = await checkDataFeeds(sbAdmin, APP_BASE_URL);
    } catch (e) { warn("cron.cleanup.data_feeds_failed", { err: e.message }); }

    // Credential/env preflight — ALERT_FROM pointing at an unverified domain,
    // an unset CRON_SECRET and an out-of-credits Anthropic account have each
    // taken a subsystem down while every dashboard looked green.
    let config = {};
    try {
      config = await checkConfig(sbAdmin);
    } catch (e) { warn("cron.cleanup.config_failed", { err: e.message }); }

    // Cron staleness lived ONLY in /api/admin/db-status, so the "a cron stopped
    // running" alert could only fire while the owner happened to be looking at
    // the admin panel — the one moment they'd notice anyway. Running it here
    // makes it an actual monitor. Note the ordering: this reads audit_log for
    // each job's last run, so it must come BEFORE cleanup self-records below.
    let cronHealth = {};
    try {
      cronHealth = await checkCronStaleness(sbAdmin);
    } catch (e) { warn("cron.cleanup.cron_staleness_failed", { err: e.message }); }

    // Quarterly legal-document review. Rides the daily sweep rather than its
    // own schedule — the plan only allows daily crons, and a reminder derived
    // from the document's own "last updated" date cannot drift from what users
    // are actually reading. See checkLegalReview() for what prompted it.
    let legal = {};
    try {
      legal = await checkLegalReview({}, sbAdmin);
    } catch (e) { warn("cron.cleanup.legal_review_failed", { err: e.message }); }

    // Same rationale for the curated divestment list: no feed exists to poll,
    // so the only thing standing between it and silent rot is this reminder.
    let ethical = {};
    try {
      ethical = await checkEthicalListReview({}, sbAdmin);
    } catch (e) { warn("cron.cleanup.ethical_review_failed", { err: e.message }); }

    const unhealthy = {
      feeds:   Object.entries(feeds).filter(([, f]) => !f.healthy).map(([k]) => k),
      config:  Object.entries(config).filter(([, c]) => !c.healthy).map(([k]) => k),
      crons:   Object.entries(cronHealth).filter(([, c]) => !c).map(([k]) => k),
      legal:   legal.healthy === false ? [`review overdue (${legal.days_since}d)`] : [],
      ethical: ethical.healthy === false ? [`list review overdue (${ethical.days_since}d)`] : [],
    };

    info("cron.cleanup.ok", { deleted_audit: deletedAudit, deleted_rate_limits: deletedRate, deleted_security_events: deletedSecEvents, unhealthy });
    // Self-record so /api/admin/db-status can surface "last cleanup ran X hours ago"
    audit({ action: "cron.cleanup", metadata: { deleted_audit: deletedAudit, deleted_rate_limits: deletedRate, deleted_security_events: deletedSecEvents, feeds_down: unhealthy.feeds.length, config_broken: unhealthy.config.length }, headers: h });
    return { status: 200, body: { ok: true, deleted_audit: deletedAudit, deleted_rate_limits: deletedRate, deleted_security_events: deletedSecEvents, unhealthy } };
  }
  ...
  // Walks every SnapTrade-connected user, sums brokerage + net bank, and
  // appends today's total to `user_state.<NW_HISTORY_KEY>` — the same series
  // the in-app chart draws (capped at NW_HISTORY_CAP). Vercel: 04:55 UTC daily.
    ...
    try {
      ...
      for (const u of (users || [])) {
        try {
          ...
          const accounts = Array.isArray(acctRes.body) ? acctRes.body : [];
          // Brokerage total. Mirror /api/snaptrade/all: when a custodian (many
          // 401k providers — Empower, Fidelity NetBenefits, Principal) reports
          // NO top-level balance, derive it from cash + Σ(position value) rather
          // than dropping the account as $0 (which silently undercounted the
          // history vs the live headline). Only balance-less accounts incur the
          // extra positions/balances calls.
          let brokerageTotal = 0;
          for (const a of accounts) {
            const reported = a.balance?.total?.amount ?? a.total_value?.amount ?? 0;
            if (Number.isFinite(reported) && reported > 0) { brokerageTotal += reported; continue; }
            try {
              const [posRes, balRes] = await Promise.all([
                snapReq("GET", `/accounts/${a.id}/positions`, null, { userId: u.snaptrade_user_id, userSecret: uSecret }),
                snapReq("GET", `/accounts/${a.id}/balances`,  null, { userId: u.snaptrade_user_id, userSecret: uSecret }),
              ]);
              const balArr = Array.isArray(balRes.body) ? balRes.body : [];
              const cash = balArr.length
                ? balArr.reduce((s, b) => s + (Number(b?.cash) || 0), 0)
                : (a.balance?.cash?.amount || 0);
              const posMV = (Array.isArray(posRes.body) ? posRes.body : []).reduce((s, p) => {
                const units = Number(p?.units), price = Number(p?.price);
                return s + (Number.isFinite(units) && Number.isFinite(price) ? units * price : 0);
              }, 0);
              brokerageTotal += cash + posMV;
            } catch (derErr) {
              warn("cron.snapshot.derive_failed", { userId: u.user_id, accountId: a.id, err: derErr.message });
            }
          }

          // Add the user's net bank position from Plaid so the history tracks the
          // SAME quantity as the live Net Worth headline. Net = depository −
          // (credit + loan); investment-type Plaid accounts are excluded (routed
          // through SnapTrade as the canonical brokerage source, so counting them
          // here would double-count a broker linked via both providers). Manual
          // assets are client-only (localStorage) and cannot be mirrored server-
          // side — the one net-worth component the snapshot structurally can't
          // include. Uses the cheap cached-balance accountsGet (not live).
          let bankNet = 0;
          if (plaidClient) {
            try {
              const { data: ptokens } = await sbAdmin
                .from("plaid_tokens").select("access_token").eq("user_id", u.user_id);
              for (const t of (ptokens || [])) {
                try {
                  const pr = await plaidClient.accountsGet({ access_token: t.access_token });
                  for (const a of (pr.data.accounts || [])) {
                    const v = Number(a.balances?.current) || 0;
                    if (a.type === "depository") bankNet += v;
                    else if (a.type === "credit" || a.type === "loan") bankNet -= v;
                  }
                } catch (itemErr) {
                  warn("cron.snapshot.plaid_item_failed", { userId: u.user_id, err: itemErr.message });
                }
              }
            } catch (plaidErr) {
              warn("cron.snapshot.plaid_failed", { userId: u.user_id, err: plaidErr.message });
            }
          }

          const total = Math.round((brokerageTotal + bankNet) * 100) / 100;

          // ONE history, in the key the app's chart actually reads.
          // This used to write its own `networth_history` key that only the
          // weekly digest consumed, while the chart drew a separate,
          // brokerage-only `mizan_networth_history` written by the client. Two
          // series, two compositions, and the app disagreed with its own digest
          // email. Both now write bank-inclusive totals into NW_HISTORY_KEY.
          // THROW on a failed read — do not fall through. mergeNetWorthHistory
          // treats a non-array `canonical` as an EMPTY history, so a Supabase
          // error here (which resolves as { data: null, error } rather than
          // throwing) would make `prev?.value` undefined, merge to a
          // single-entry array, and then UPSERT that over the user's real
          // multi-month history — counted as `processed += 1`, with nothing
          // logged. That is the same destruction the 2026-07-30 merge fix was
          // written to prevent; an unchecked read quietly defeats it.
          // Throwing hands this user to the catch below: failed += 1, warn,
          // skip. A missing snapshot for one night is recoverable. An
          // overwritten history is not.
          const { data: prev, error: prevErr } = await sbAdmin.from("user_state")
            .select("value")
            .eq("user_id", u.user_id)
            .eq("key", NW_HISTORY_KEY)
            .maybeSingle();
          if (prevErr) throw new Error(`nw_history read failed: ${prevErr.message}`);
          // One-time repair of the pre-unification past — see
          // mergeNetWorthHistory() for the precedence rules.
          const { data: legacy, error: legacyErr } = await sbAdmin.from("user_state")
            .select("value")
            .eq("user_id", u.user_id)
            .eq("key", "networth_history")
            .maybeSingle();
          if (legacyErr) throw new Error(`legacy history read failed: ${legacyErr.message}`);

          const next = mergeNetWorthHistory(prev?.value, legacy?.value, { date: today, total });

          const { error: upErr } = await sbAdmin.from("user_state").upsert({
            user_id:    u.user_id,
            key:        NW_HISTORY_KEY,
            value:      next,
            updated_at: new Date().toISOString(),
          });
          // A silently-dropped write means tomorrow's run re-merges from a
          // stale row and today's point is lost with `processed` still counting
          // it as a success.
          if (upErr) throw new Error(`nw_history write failed: ${upErr.message}`);
          processed += 1;
        } catch (e) {
          failed += 1;
          lastErr = e.message;
      ...
      await recordCronRun("nightly_snapshot", lastErr ? "error" : "ok", lastErr);
    }
  ...
  // change from NW_HISTORY_KEY and emails a summary. Vercel schedule:
  ...
  if (pathname === "/api/cron/weekly-digest") {
    ...
    try {
      ...
      for (const p of profiles) {
        ...
        try {
          // Same key, same composition as the in-app chart, so the digest can
          // never quote a different net worth than the app shows.
          ...
          // Entries are { date, total }; `value` is the legacy server shape,
          // still readable for rows written before the two series merged.
          const amount = e => Number(e?.total ?? e?.value) || 0;
          const latest = arr[arr.length - 1];
          // Find the entry from ~7 days ago (closest match).
          const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 3600 * 1000).toISOString().slice(0, 10);
          const baseline = arr.find(e => e.date >= sevenDaysAgo) || arr[0];
          const change = amount(latest) - amount(baseline);
          const pct = amount(baseline) ? (change / amount(baseline)) * 100 : 0;
          const sign = change >= 0 ? "+" : "−";
          ...
          const text = [
            ...
            ``,
            `  Net worth: ${fmt(amount(latest))} (${sign}${fmt(change)}, ${sign}${Math.abs(pct).toFixed(2)}%)`,
            `  Compared to ${baseline.date}.`,
            ``,
            `Tap below to see your full breakdown.`,
            ``,
            `— MIZAN`,
          ].join("\n");
      ...
      await recordCronRun("weekly_digest", lastErr ? "error" : "ok", lastErr);
    }
  ...
  // ── Dividend purification cron ──────────────────────────
  // Walks every SnapTrade-linked user's own synced activities, finds dividends
  // on tickers we hold an AAOIFI impurity ratio for, and pushes the estimated
  // purification amount. Schedule: 11:00 UTC daily.
  //
  // Rewritten 2026-08-10 because the previous implementation had never once
  // worked. It asked Finnhub for TOMORROW's market-wide dividend calendar, an
  // endpoint that is not on our plan — and unlike the per-symbol
  // /stock/dividend, which returns an explicit 403 "You don't have access to
  // this resource", the calendar answers `200 {}`. So `json.dividendCalendar`
  // was always undefined, `items` was always empty, the loop never ran, and
  // the cron logged `cron.dividend_check.ok` and wrote an "ok" heartbeat every
  // day while doing nothing at all. audit_log confirmed it: zero
  // `dividend_upcoming` and zero `purification.notif_sent` rows had EVER been
  // written. A 200 that means "no access" is invisible to a status check, so
  // the fix is to stop asking a third party about dividends we already sync.
  //
  // Reading our own activities is also strictly better data: it fires on
  // dividends the user ACTUALLY received, with the real amount, rather than a
  // market-wide ex-date calendar with no idea what anyone holds. The old code
  // pushed every purifiable ticker to every user with a push subscription,
  // whether or not they owned it; this notifies the holder only.
  //
  // The in-app feed already records dividends client-side
  // (src/lib/notifications.js dividendNotifications). This cron exists for the
  // part that needs a server: reaching a user who is not in the app, and
  // attaching the purification figure. Scoped to tickers with a known ratio —
  // without one there is no number to give, only a duplicate of the in-app note.
  if (pathname === "/api/cron/dividend-check") {
    if (cronUnauthorized(h)) return { status: 401, body: { error: "Unauthorized" } };
    if (!sbAdmin) return { status: 200, body: { ok: true, skipped: "no-supabase" } };

    // Dividends post with broker lag and a cron can miss a day, so look back
    // further than the daily cadence. Re-notification is prevented by activity
    // id, not by the window, so widening it is safe.
    const DIVIDEND_LOOKBACK_DAYS = 14;
    const USER_BATCH = 5;

    let notified = 0, scanned = 0, usersOk = 0, usersFailed = 0;
    let lastErr = null;
    try {
      const { data: ratioRows, error: ratioErr } = await sbAdmin
        .from("purification_ratios")
        .select("ticker, impurity_pct");
      if (ratioErr) throw new Error(`purification_ratios select: ${ratioErr.message}`);
      const ratios = new Map((ratioRows || [])
        .filter(r => r?.ticker && Number.isFinite(Number(r.impurity_pct)))
        .map(r => [String(r.ticker).trim().toUpperCase(), Number(r.impurity_pct)]));

      const { data: stUsers, error: usersErr } = await sbAdmin
        .from("user_snaptrade")
        .select(`user_id, ${SNAP_SECRET_COLS}`);
      if (usersErr) throw new Error(`user_snaptrade select: ${usersErr.message}`);

      const now       = Date.now();
      const startDate = new Date(now - DIVIDEND_LOOKBACK_DAYS * 86400000).toISOString().slice(0, 10);
      const endDate   = new Date(now).toISOString().slice(0, 10);
      // Look back further than the scan window when reading what we've already
      // sent, so a dividend near the edge of the window can't be re-notified.
      const sentSince = new Date(now - (DIVIDEND_LOOKBACK_DAYS + 7) * 86400000).toISOString();

      const targets = ratios.size > 0 ? (stUsers || []) : [];
      if (ratios.size === 0) warn("cron.dividend_check.no_ratios", {});

      for (let i = 0; i < targets.length; i += USER_BATCH) {
        const batch = targets.slice(i, i + USER_BATCH);
        const settled = await Promise.allSettled(batch.map(async u => {
          const uSecret = extractUserSecret(u);
          if (!uSecret) throw new Error(`no secret for user_id=${u.user_id}`);

          const r = await snapReq("GET", "/activities", null, {
            userId: u.snaptrade_user_id, userSecret: uSecret, startDate, endDate,
          });
          const rows = Array.isArray(r.body) ? r.body : (r.body?.activities || []);
          const dividends = (rows || []).filter(a =>
            a?.id
            && isDividendActivity(a.type)
            && Number(a.amount) > 0
            && ratios.has(snapTicker(a.symbol))
          );
          scanned += dividends.length;
          if (!dividends.length) return 0;

          // One lookup per user rather than one per dividend: pull the ids we
          // have already pushed for and diff in memory.
          const { data: sentRows, error: sentErr } = await sbAdmin
            .from("audit_log")
            .select("metadata")
            .eq("user_id", u.user_id)
            .eq("action", "purification.notif_sent")
            .gte("created_at", sentSince);
          if (sentErr) throw new Error(`audit_log select: ${sentErr.message}`);
          const already = new Set((sentRows || [])
            .map(x => x?.metadata?.activity_id)
            .filter(Boolean));

          let sent = 0;
          for (const d of dividends) {
            if (already.has(d.id)) continue;
            const tk     = snapTicker(d.symbol);
            const pct    = ratios.get(tk);
            const amount = Number(d.amount);
            const est    = (amount * pct) / 100;

            // Insert BEFORE the push so a retry can never double-notify —
            // same ordering as the bill reminders.
            const { error: insErr } = await sbAdmin.from("audit_log").insert({
              user_id: u.user_id,
              action:  "purification.notif_sent",
              target:  tk,
              metadata: {
                ticker: tk, activity_id: d.id, trade_date: d.trade_date || null,
                dividend: amount, pct, est: +est.toFixed(4),
              },
            });
            if (insErr) {
              warn("cron.dividend_check.audit_failed", { ticker: tk, err: insErr.message });
              continue;
            }
            already.add(d.id);

            await sendPushToUser(sbAdmin, u.user_id, {
              title: `Dividend purification — ${tk}`,
              body:  `${tk} paid $${amount.toFixed(2)} · about $${est.toFixed(2)} to purify.`,
        ...
        for (const s of settled) {
          if (s.status === "fulfilled") { usersOk += 1; notified += s.value; }
          else { usersFailed += 1; warn("cron.dividend_check.user_failed", { err: s.reason?.message }); }
        }
      }
      info("cron.dividend_check.ok", {
        users: targets.length, usersOk, usersFailed, scanned, notified, startDate, endDate,
      });
    } catch (e) {
      lastErr = e.message;
      logError("cron.dividend_check.failed", { err: e.message });
    } finally {
      await recordCronRun("dividend_check", lastErr ? "error" : "ok", lastErr);
    }
    return { status: 200, body: { ok: !lastErr, notified, scanned } };
  }
  ...
  if (pathname === "/api/cron/bill-reminders") {
    ...
    try {
      ...
      await recordCronRun("bill_reminders", lastErr ? "error" : "ok", lastErr);
    }
    return { status: 200, body: { ok: !lastErr, sent, skipped, failed } };
  }
  ...
  // ── /api/alpaca/keys — the tester's own paper credentials ───────────────
  // GET    → whether a key is stored, which account it points at, never the secret
  // PUT    → verify against Alpaca, then store encrypted
  // DELETE → remove, falling the user back to the shared account (if configured)
  if (pathname === "/api/alpaca/keys") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "Unauthenticated" } };
    if (!(await canUseTradingBot(user))) return { status: 403, body: { error: "trading_not_enabled" } };
    if (!sbAdmin) return { status: 503, body: { error: "Supabase not configured" } };
    ...
    if (method === "GET") {
      const creds = await getAlpacaCreds(user);
      // Deliberately no secret, and no key id — only the last 4, which
      // identifies the key to the person who created it and to nobody else.
      return { status: 200, body: {
        configured: !!creds,
        source:     creds?.source || null,      // "user" | "shared" | null
        last4:      creds?.last4 || null,
        paper:      creds?.paper ?? true,
        encryption: ENC_ENABLED,
      } };
    }

    if (method === "PUT") {
      // Fail closed. Without a master key we cannot encrypt, and there is no
      // plaintext column to fall back to — by design (migration 029).
      if (!ENC_ENABLED) {
        return { status: 503, body: { error: "Server encryption is not configured, so trading credentials cannot be stored. Set ENCRYPTION_KEY." } };
      }
      const keyId  = String(parsed?.keyId || "").trim();
      const secret = String(parsed?.secret || "").trim();
      if (!keyId || !secret) return { status: 400, body: { error: "keyId and secret are required." } };
      if (keyId.length > 128 || secret.length > 256) return { status: 400, body: { error: "That does not look like an Alpaca key pair." } };

      const check = await verifyAlpacaCreds({ keyId, secret });
      if (!check.ok) {
        // Never log the credential itself, only that a verification failed.
        warn("alpaca.keys.verify_failed", { userId: user.id, status: check.status });
        return { status: check.status, body: { error: check.error } };
      }

      const kid = encryptSecret(keyId);
      const sec = encryptSecret(secret);
      const { error: upErr } = await sbAdmin.from("user_keys").upsert({
        user_id:                  user.id,
        alpaca_key_id_ciphertext: kid.ciphertext,
        alpaca_key_id_iv:         kid.iv,
        alpaca_key_id_auth_tag:   kid.authTag,
        alpaca_secret_ciphertext: sec.ciphertext,
        alpaca_secret_iv:         sec.iv,
        alpaca_secret_auth_tag:   sec.authTag,
        alpaca_key_last4:         keyId.slice(-4),
        alpaca_paper:             true,
        alpaca_updated_at:        new Date().toISOString(),
        updated_at:               new Date().toISOString(),
      }, { onConflict: "user_id" });
      if (upErr) {
        logError("alpaca.keys.store_failed", { err: upErr.message });
        return { status: 500, body: { error: "Could not store the credentials." } };
      }
      audit({ userId: user.id, action: "alpaca.keys_stored", metadata: { last4: keyId.slice(-4), account_status: check.status || null }, headers: h });
      info("alpaca.keys.stored", { userId: user.id });
      return { status: 200, body: { ok: true, configured: true, source: "user", last4: keyId.slice(-4), paper: true } };
    }

    if (method === "DELETE") {
      const { error: delErr } = await sbAdmin.from("user_keys").update({
        alpaca_key_id_ciphertext: null, alpaca_key_id_iv: null, alpaca_key_id_auth_tag: null,
        alpaca_secret_ciphertext: null, alpaca_secret_iv: null, alpaca_secret_auth_tag: null,
        alpaca_key_last4: null, alpaca_updated_at: null,
      }).eq("user_id", user.id);
      if (delErr) {
        logError("alpaca.keys.delete_failed", { err: delErr.message });
        return { status: 500, body: { error: "Could not remove the credentials." } };
      }
      audit({ userId: user.id, action: "alpaca.keys_removed", headers: h });
  ...
  if (pathname === "/api/alpaca/order" && method === "POST") {
    const user = await verifyUser(h);
    if (!user) return { status: 401, body: { error: "Unauthenticated" } };
    // Authorization-scope to the trading-bot beta. A user without their own
    // stored credentials falls back to the SHARED paper account, which several
    // testers would write into at once — hence the allowlist stays.
    if (!(await canUseTradingBot(user))) return { status: 403, body: { error: "trading_not_enabled" } };
    const creds = await getAlpacaCreds(user);
    if (!creds) {
      return { status: 503, body: { error: "Alpaca not configured. Add your paper-account keys in Settings, or ask the owner to set the shared keys." } };
    }

    const { symbol, qty, side, type, limitPrice, timeInForce, extendedHours } = parsed || {};
    const symbolUp = String(symbol || "").toUpperCase();
    const result = await placeAlpacaOrder({
      symbol, qty, side, type, limitPrice, creds,
      timeInForce: timeInForce || "day",
      // Opt-in per request. The client sends this only when the user has seen
      // the extended-hours session state and the risk disclosure; absent the
      // flag the order is regular-hours-only, exactly as before.
    ...
    if (!result.ok) {
      warn("alpaca.order.failed", { status: result.status, symbol: symbolUp, code: result.code || null, session: result.session || null });
      // Session refusals are OUR rule, not the broker's response, so they carry
      // a readable error instead of an empty upstream body.
      if (result.code || result.market_closed) {
        return { status: result.status, body: { error: result.error, code: result.code || "market_closed", session: result.session || null } };
      }
    ...
    audit({
      ...
      target: symbolUp,
      metadata: { qty, side, type, limitPrice: limitPrice ?? null, orderId: result.orderId, extendedHours: extendedHours === true },
      headers: h,
    });
    info("alpaca.order.ok", { symbol: symbolUp, qty, side, type, extended: extendedHours === true });
  ...
  if (pathname === "/api/alpaca/orders" && method === "GET") {
    ...
    if (!(await canUseTradingBot(user))) return { status: 403, body: { error: "trading_not_enabled" } };
    const creds = await getAlpacaCreds(user);
    if (!creds) return { status: 503, body: { error: "Alpaca not configured" } };
    let res;
    try {
  ...
  // Scheduled once daily at 14:00 UTC (Vercel Hobby = daily crons only). For
  // sub-daily checks, hit this endpoint with the CRON_SECRET bearer from an
  // external scheduler (or upgrade to Vercel Pro for native */15 crons).
  ...
  if (pathname === "/api/cron/bot-signals" && (method === "GET" || method === "POST")) {
    ...
    // Market-hours gate: the bot places MARKET orders, which the broker rejects
    // outside 09:30–16:00 ET. The pg_cron schedule fires */15 in UTC (24h), so
    // most slots land while US markets are closed — skip those runs entirely so
    // we never generate a signal that can't fill (and never spam retries).
    // Heartbeat. Recorded on EVERY invocation, including the market-closed
    // early return below, because the thing worth monitoring is "is the
    // scheduler still firing this endpoint" — not "did it trade". The 15-min
    // weekday cadence comes from GitHub Actions, whose schedule triggers are
    // best-effort and silently drop fires; without this, the bot going quiet
    // and the market being closed look identical from the outside. Full-auto
    // DCA moves real money, so a silent stop is a real loss.
    // Must be awaited at both call sites — the market-closed one returns
    // immediately after, and that is the majority of invocations.
    const botHeartbeat = (status, err = null) => recordCronRun("bot_signals", status, err);

    const market = usMarketStatus();
    if (!market.open) {
      info("cron.bot_signals.market_closed", { reason: market.reason });
      await botHeartbeat("closed");
      return { status: 200, body: { ok: true, skipped: "market_closed", reason: market.reason } };
    }
    ...
    // Get all enabled strategies. Do NOT embed profiles here — there is no FK
    // from bot_strategies → profiles, so a PostgREST embed 400s (PGRST200) and
    // the whole cron silently no-ops (returned processed:0 forever). Fetch the
    // master full-auto switch separately and index it by user_id.
    ...
    if (stratErr) {
      warn("cron.bot_signals.load_failed", { err: stratErr.message });
      return { status: 500, body: { ok: false, error: "load_failed" } };
    }
    if (!strategies?.length) {
      info("cron.bot_signals.no_strategies", { count: 0 });
      return { status: 200, body: { ok: true, processed: 0 } };
    }

    const faUserIds = [...new Set(strategies.map(s => s.user_id).filter(Boolean))];
    const { data: faProfiles, error: faErr } = await sbAdmin.from("profiles")
      .select("id, full_auto_enabled")
      .in("id", faUserIds);
    if (faErr) warn("cron.bot_signals.profiles_load_failed", { err: faErr.message });
    const fullAutoByUser = new Map((faProfiles || []).map(p => [p.id, !!p.full_auto_enabled]));

    let signalsGenerated = 0, autoExecuted = 0, sharaBlocked = 0;

    // Per-tick CLAIM lease (see lib/botLease.mjs). Read the clock ONCE so every
    // strategy this tick claims against the same threshold.
    const claimNowMs = Date.now();
    const claimNowIso = new Date(claimNowMs).toISOString();
    const claimLeaseThresholdIso = botClaimLeaseThresholdIso(claimNowMs);

    for (const strat of strategies) {
      // Tracks the signal an in-flight full-auto order belongs to, so a mid-
      // execution THROW resolves it terminally in the catch (race #6) instead of
      // leaving a reusable 'pending' row. null = nothing in flight (semi/manual
      // signals are deliberately left pending for the user to approve).
      let inFlightSignalId = null;
      try {
        // ── ATOMIC PER-TICK CLAIM (closes duplicate-exec races #3/#4/#5) ──────
        // Two overlapping cron fires (GitHub Actions */15 + the Vercel daily
        // backstop, or a manual re-trigger) could each run this whole iteration
        // for the same strategy and double-execute — SnapTrade exposes no client
        // idempotency key. Claim the strategy for THIS tick via a single atomic
        // compare-and-swap on updated_at used as a lease: set updated_at=now
        // WHERE id=? AND updated_at < (now - lease). Exactly one concurrent fire
        // gets the row back; the loser matches zero rows and skips. Same DB-CAS
        // idiom as the approve-path fix (eafae77). Because the duplicate-BUY (#3),
        // double-SELL (#4) and racy-DCA (#5) races all execute INSIDE this one
        // claimed iteration, this single guard closes all three.
        const { data: claimed, error: claimErr } = await sbAdmin.from("bot_strategies")
          .update({ updated_at: claimNowIso })
          .eq("id", strat.id)
          .lt("updated_at", claimLeaseThresholdIso)
          .select("id");
        if (claimErr) { warn("bot.signal.claim_failed", { strategy_id: strat.id, err: claimErr.message }); continue; }
        if (!claimed || claimed.length === 0) { info("bot.signal.claim_skipped", { strategy_id: strat.id }); continue; }
        ...
        const isFullAuto = strat.mode === "full"
          && fullAutoByUser.get(strat.user_id) === true
          && await accountFullAutoEnabled(strat.user_id, strat.account_id);

        // ── DCA / LONG-TERM ACCUMULATION ────────────────────────────────────
        // Buys whole shares of the target ticker on a fixed cadence (default
        // weekly) and HOLDS — no momentum entry, no stop/target/trailing exits
        // (you exit manually, ideally past 1yr for long-term cap-gains). Keeps
        // buying until capital_allocated is deployed; raise the cap (add money)
        // to accumulate more. Runs before — and instead of — the momentum engine,
        // so it can add to an existing position and is never auto-sold.
        if (strat.strategy_type === "dca") {
          const cadenceDays = Math.max(1, Number(strat.params?.dca_cadence_days) || 7);
          const last = strat.params?.last_dca_date ? new Date(strat.params.last_dca_date) : null;
          const daysSince = last ? (Date.now() - last.getTime()) / 86400000 : Infinity;
          if (daysSince < cadenceDays) continue; // accumulated recently — wait for the next window

          // One attempt per day max (avoids hammering the broker with retries
          // before the account is funded; daily until the first fill, then cadence).
          const { count: todayCount } = await sbAdmin.from("pending_signals")
            .select("*", { count: "exact", head: true }).eq("strategy_id", strat.id).gte("created_at", todayDate);
          if (todayCount && todayCount > 0) continue;

          // Per-period budget, capped by remaining capital (open exposure).
          const perPeriod = Number(strat.params?.dca_amount) > 0 ? Number(strat.params.dca_amount) : Number(strat.capital_allocated) || 0;
          const { data: dcaRows } = await sbAdmin.from("pending_signals")
            .select("side,qty,suggested_price,status,ticker").eq("strategy_id", strat.id)
            .in("status", ["pending", "approved", "executed"]);
          let dcaDeployed = 0;
          for (const s of dcaRows || []) { const n = (Number(s.qty) || 0) * (Number(s.suggested_price) || 0); dcaDeployed += s.side === "sell" ? -n : n; }
          const dcaBudget = Math.min(perPeriod, Math.max(0, (Number(strat.capital_allocated) || 0) - dcaDeployed));
          if (dcaBudget <= 0) continue; // capital fully deployed — holding; raise the cap to add more

          // Target selection. A weighted BASKET (Halal Bogleheads three-fund) buys
          // the most-underweight AFFORDABLE member each period — rebalancing via new
          // contributions (never sells). A plain DCA buys its single target ticker.
          const basket = Array.isArray(strat.params?.basket) && strat.params.basket.length >= 2 ? strat.params.basket : null;
          let dcaTarget, dq;
          if (basket) {
            const heldByTicker = {};
            for (const s of dcaRows || []) {
              const t = String(s.ticker || "").toUpperCase().trim(); if (!t) continue;
              heldByTicker[t] = (heldByTicker[t] || 0) + (s.side === "sell" ? -(Number(s.qty) || 0) : (Number(s.qty) || 0));
            }
            const legs = []; let totalVal = 0, wSum = 0;
            for (const b of basket) {
              const t = String(b.ticker || "").toUpperCase().trim(); const w = Number(b.weight) || 0;
              if (!t || HARAM_TICKERS.has(t) || w <= 0) continue;
              const q = await finnhubQuote(t);
              if (!q || !(q.current > 0)) continue;
              const val = (heldByTicker[t] || 0) * q.current;
              legs.push({ ticker: t, weight: w, price: q.current, value: val }); totalVal += val; wSum += w;
            }
            if (!legs.length || wSum <= 0) { sharaBlocked++; continue; }
            // Most-underweight member we can afford ≥1 whole share of this period.
            let best = null, bestGap = -Infinity;
            for (const leg of legs) {
              const gap = (leg.weight / wSum) - (totalVal > 0 ? leg.value / totalVal : 0);
              if (leg.price <= dcaBudget && gap > bestGap) { bestGap = gap; best = leg; }
            }
            if (!best) continue; // no affordable member this period — wait for more budget
            dcaTarget = best.ticker; dq = { current: best.price };
          } else {
            dcaTarget = strategyUniverse(strat)[0] || String(strat.ticker || "").toUpperCase().trim();
            if (!dcaTarget || HARAM_TICKERS.has(dcaTarget)) { sharaBlocked++; continue; }
            dq = await finnhubQuote(dcaTarget);
            if (!dq || !(dq.current > 0)) continue;
          }
          const dcaQty = Math.floor(dcaBudget / dq.current);
          if (dcaQty < 1) continue; // can't afford a whole share this period — wait

          const { data: dcaSig, error: dcaErr } = await sbAdmin.from("pending_signals").insert({
            user_id: strat.user_id, strategy_id: strat.id, ticker: dcaTarget, side: "buy", qty: dcaQty,
            suggested_price: dq.current, sharia_passed: true, status: "pending",
            // Full-trading-day approval window (not the 60-min default) so a
            // semi-auto weekly DCA isn't missed inside an hour. See dcaSignalExpiryIso.
            expires_at: dcaSignalExpiryIso(),
          }).select().single();
          if (dcaErr) { warn("bot.dca.insert_failed", { err: dcaErr.message }); continue; }
          signalsGenerated++;

          if (isFullAuto) {
            // #6: mark the signal in-flight so a mid-execution THROW (ambiguous
            // broker outcome) resolves it terminally in the loop catch rather
            // than leaving a reusable 'pending' row.
            inFlightSignalId = dcaSig.id;
            const creds = await getSnapCredsForUser(strat.user_id);
            const exec = creds
              ? await executeSnapTradeOrder({ ...creds, accountId: strat.account_id, ticker: dcaTarget, side: "buy", qty: dcaQty })
              : { ok: false, status: 400, error: "broker_not_connected" };
            if (exec.ok) {
              inFlightSignalId = null; // order placed — from here it's bookkeeping
              await sbAdmin.from("pending_signals").update({ status: "executed", executed_at: new Date().toISOString() }).eq("id", dcaSig.id);
              // Advance the cadence ONLY on a real fill, and count the trade.
              await sbAdmin.from("bot_strategies").update({ params: { ...(strat.params || {}), last_dca_date: todayDate }, trades_today: strat.trades_today + 1, trades_today_date: todayDate }).eq("id", strat.id);
              audit({ userId: strat.user_id, action: "bot.dca.auto_executed", target: dcaSig.id, metadata: { ticker: dcaTarget, qty: dcaQty, price: dq.current, account: strat.account_id }, headers: {} });
              autoExecuted++;
              await sendPushToUser(sbAdmin, strat.user_id, { title: `DCA: bought ${dcaQty} ${dcaTarget}`, body: `Accumulated at ~$${dq.current.toFixed(2)}. Long-term hold.`, url: "/trade" }).catch(() => {});
            } else {
              inFlightSignalId = null; // definitive failure — the reject below is terminal
              // Failed (e.g. account not funded yet) — reject the signal so it never
              // counts as exposure, and DON'T advance the cadence (retries tomorrow).
              await sbAdmin.from("pending_signals").update({ status: "rejected", error_msg: String(exec.error || "execution_failed").slice(0, 300) }).eq("id", dcaSig.id);
              audit({ userId: strat.user_id, action: "bot.dca.auto_execute_failed", target: dcaSig.id, metadata: { ticker: dcaTarget, qty: dcaQty, error: exec.error }, headers: {} });
              warn("bot.dca.auto_execute_failed", { strategy_id: strat.id, signal_id: dcaSig.id, status: exec.status });
            }
          } else {
            // Semi/manual: the signal waits for approval; advance the cadence so it
            // isn't re-proposed every day while pending.
            await sbAdmin.from("bot_strategies").update({ params: { ...(strat.params || {}), last_dca_date: todayDate } }).eq("id", strat.id);
            audit({ userId: strat.user_id, action: "bot.dca.generated", target: dcaSig.id, metadata: { ticker: dcaTarget, qty: dcaQty, price: dq.current, layer }, headers: {} });
            if (layer === "semi") await sendPushToUser(sbAdmin, strat.user_id, { title: `DCA suggests: buy ${dcaQty} ${dcaTarget}`, body: `Tap to approve this accumulation buy.`, url: "/trade" }).catch(() => {});
          }
          continue;
        }
        ...
        if (pos.netQty > 0 && pos.avgEntry && pos.avgEntry > 0) {
          ...
          const exitClose = async (reason, pushTitle, pushBody) => {
            ...
            signalsGenerated++;
            if (isFullAuto) {
              // #6: in-flight so a mid-execution throw rejects this SELL in the catch.
              inFlightSignalId = xsig.id;
              const creds = await getSnapCredsForUser(strat.user_id);
              const exec = creds
                ? await executeSnapTradeOrder({ ...creds, accountId: strat.account_id, ticker: heldTicker, side: "sell", qty: pos.netQty })
                : { ok: false, status: 400, error: "broker_not_connected" };
              if (exec.ok) {
                inFlightSignalId = null; // order placed — bookkeeping only from here
                ...
                inFlightSignalId = null; // definitive failure — resolve terminally below
                // #6: resolve terminally (mirrors the DCA path) so a failed SELL can't
                // sit 'pending' and be re-approved into a double-sell (no broker idempotency key).
                await sbAdmin.from("pending_signals").update({ status: "rejected", error_msg: String(exec.error || "execution_failed").slice(0, 300) }).eq("id", xsig.id);
          ...
          // Track the position's high-water price since entry (persisted in
          // params.high_water). The trailing exit locks gains on a winner that
          // pulls back — capturing more of a trend than the fixed target, in ONE
          // taxable sale. It only ever sells in PROFIT (the stop owns the
          // downside) and only arms once the peak is up >= trail_activate_pct.
          const trailPct = Number(strat.params?.trail_pct) || 0; // 0 = trailing off
          const trailArmPct = Number(strat.params?.trail_activate_pct ?? 10);
          const priorPeak = Number(strat.params?.high_water) || pos.avgEntry;
          const peak = Math.max(priorPeak, currentPrice);
          if (peak > priorPeak) {
            strat.params = { ...(strat.params || {}), high_water: peak };
            await sbAdmin.from("bot_strategies").update({ params: strat.params, updated_at: new Date().toISOString() }).eq("id", strat.id);
          }
          const peakGainPct = pos.avgEntry > 0 ? ((peak - pos.avgEntry) / pos.avgEntry) * 100 : 0;
          const dropFromPeakPct = peak > 0 ? ((peak - currentPrice) / peak) * 100 : 0;
          // Reset the high-water mark whenever a position closes so the next starts fresh.
          const clearPeak = async () => {
            strat.params = { ...(strat.params || {}), high_water: null };
            await sbAdmin.from("bot_strategies").update({ params: strat.params, updated_at: new Date().toISOString() }).eq("id", strat.id);
          };
          ...
          const hitTrail = trailPct > 0 && gainPct > 0 && peakGainPct >= trailArmPct && dropFromPeakPct >= trailPct;
          ...
          if (hitStop || hitDrawdown) {
            ...
            await exitClose("bot.strategy.target_hit",
              `Target reached on ${heldTicker} — locking in`,
              `Up ${gainPct.toFixed(1)}% (target ${strat.profit_target_pct}%). Locking in profit.`);
            await clearPeak();
          } else if (hitTrail) {
            await exitClose("bot.strategy.trailing_stop",
              `Trailing stop on ${heldTicker} — gains locked`,
              `Peaked +${peakGainPct.toFixed(1)}%, pulled back ${dropFromPeakPct.toFixed(1)}% to +${gainPct.toFixed(1)}% — sold to lock profit.`);
            await clearPeak();
          }
        ...
        // Per-strategy entry threshold (params.entry_threshold_pct, in %), default
        // 1.5%. The day-momentum proxy that opens a position: a universe name up at
        // least this much vs its previous close.
        const BUY_THRESHOLD = (Number(strat.params?.entry_threshold_pct) || 1.5) / 100;

        // Compute the deployable budget FIRST so candidate selection can require
        // AFFORDABILITY. Position size: a % of allocated capital when set, else an
        // even split across the daily trade cap.
        ...
        // Deployable-capital cap: a strategy's open exposure (in-flight + executed
        // buys, minus executed sells) can never exceed capital_allocated, so the
        // reserved share of buying power is never touched — in ANY mode.
        let deployable = dollars;
        {
          const { data: sigRows } = await sbAdmin.from("pending_signals")
            .select("side,qty,suggested_price,status").eq("strategy_id", strat.id)
            .in("status", ["pending", "approved", "executed"]);
          let deployed = 0;
          for (const s of sigRows || []) {
            const n = (Number(s.qty) || 0) * (Number(s.suggested_price) || 0);
            deployed += s.side === "sell" ? -n : n;
          }
          deployable = Math.min(dollars, Math.max(0, Number(strat.capital_allocated || 0) - deployed));
        }

        // Pick the strongest name that clears the threshold AND is affordable — at
        // least one WHOLE share fits the deployable budget. Without the affordability
        // filter the bot locks onto the biggest mover even when it's too pricey to
        // buy (e.g. AMD @ $582 on a $100 budget) and trades nothing while cheaper
        // qualifying names wait. Pricier names simply activate once capital is raised.
        let best = null; // { ticker, price, score }
        for (const cand of universe) {
          const q = await finnhubQuote(cand);
          if (!q || !q.prevClose || !(q.current > 0)) continue;
          const score = (q.current - q.prevClose) / q.prevClose;
          const affordable = Math.floor(deployable / q.current) >= 1;
          if (score >= BUY_THRESHOLD && affordable && (!best || score > best.score)) best = { ticker: cand, price: q.current, score };
        }
        if (!best) continue; // nothing affordable triggered an entry this tick

        const tickerUp = best.ticker;
        const currentPrice = best.price;
        const side = "buy";
        // WHOLE shares only — SnapTrade brokers (E-Trade included) reject
        // fractional orders ("Units to purchase should be whole units", code
        // 1095). If the slice can't afford even one share within the deployable
        // budget, skip this tick.
        const qty = Math.floor(deployable / currentPrice);
        if (qty <= 0) continue;

        // Engine-verified halal gate: screen the picked ticker through the SAME
        // AAOIFI service that governs h.sh_ app-wide (lib/sharia.mjs / screenSymbol),
        // not just the static HARAM_TICKERS pre-filter. Block ONLY a definitive
        // "haram" verdict — halal ETFs (SPUS/HLAL/…) screen "unknown" (no company
        // fundamentals; halal by construction) and curated-universe stocks flagged
        // "review" are borderline, not disqualified. This catches a universe name
        // that has TURNED non-compliant, which the hardcoded blocklist alone misses.
        let shariaStatus = "unknown";
        try { const v = await screenSymbol(tickerUp); shariaStatus = v?.status || "unknown"; }
        catch (err) { warn("bot.signal.screen_error", { ticker: tickerUp, err: err.message }); }
        if (shariaStatus === "haram") {
          sharaBlocked++;
          warn("bot.signal.sharia_blocked", { ticker: tickerUp, strategyId: strat.id, verdict: "haram" });
          audit({ userId: strat.user_id, action: "bot.signal.sharia_blocked", target: tickerUp, metadata: { strategyId: strat.id, verdict: "haram", layer: "entry_engine" } });
          continue;
        }
        ...
        if (isFullAuto) {
          // #6: in-flight so a mid-execution throw rejects this BUY in the catch.
          inFlightSignalId = signal.id;
          const creds = await getSnapCredsForUser(strat.user_id);
          const exec = creds
          ...
          if (exec.ok) {
            ...
            inFlightSignalId = null; // definitive failure — resolve terminally below
            // #6: resolve terminally (mirrors the DCA path) instead of leaving the
            // signal 'pending'. A failed/ambiguous full-auto order must not stay
            // approvable, or a later human tap could double-fill it (SnapTrade has
            // no client idempotency key). error_msg records why.
            await sbAdmin.from("pending_signals").update({ status: "rejected", error_msg: String(exec.error || "execution_failed").slice(0, 300) }).eq("id", signal.id);
        ...
        // #6 (throw path): if we threw WHILE a full-auto order was in flight (e.g.
        // the broker place call dropped after the order may have reached the
        // broker), never leave the signal 'pending' — a human could approve it and
        // cause a SECOND fill. Resolve it terminally; error_msg + a distinct audit
        // action flag the ambiguity for manual reconciliation. The
        // .eq("status","pending") guard means a signal a concurrent/earlier path
        // already resolved is left untouched.
        if (inFlightSignalId) {
          try {
            await sbAdmin.from("pending_signals")
              .update({ status: "rejected", error_msg: `ambiguous_execution: ${String(e.message || "").slice(0, 240)}` })
              .eq("id", inFlightSignalId).eq("status", "pending");
            audit({ userId: strat.user_id, action: "bot.signal.ambiguous_execution", target: inFlightSignalId, metadata: { strategy_id: strat.id, err: String(e.message || "").slice(0, 240) }, headers: {} });
          } catch (e2) {
            warn("bot.signal.ambiguous_resolve_failed", { signal_id: inFlightSignalId, err: e2.message });
          }
        }
      }
    }
