...
`;
  ...
  if (pathname.startsWith("/api/bot/strategies")) {
    ...
    if (method === "PATCH" && patchMatch) {
      ...
      // Layer change: map the user-facing layer onto the DB mode (full-auto gate)
      // and merge params.layer without clobbering the rest of params. Requires a
      // read-modify-write so universe_tickers/entry_rules survive the toggle.
      if ("layer" in parsed && ["manual", "semi", "full"].includes(parsed.layer)) {
        ...
        updates.params = { ...(cur?.params || {}), ...(updates.params || {}), layer: parsed.layer };
      }
