...
async function _handle({ method, pathname, query, body, headers }) {
  ...
  // User-initiated downloads of their own data as CSV. JWT-gated. Three
  // endpoints, each returning text/csv with Content-Disposition: attachment
  // so the browser saves the file rather than rendering it inline.
  //
  // Throttled via the dedicated `export` rate-limit bucket (20/hr) — the
  // payload is up to 5000 rows AND for holdings/activity also fans out to
  // SnapTrade, so it costs more than a typical read.
  if (pathname.startsWith("/api/export/")) {
    ...
    const csvHeaders = (filename) => ({
      ...
      // Discourage any caching tier from holding a copy of one user's
      // personal export and serving it cross-user.
      "Cache-Control": "private, no-store",
    });

    if (pathname === "/api/export/transactions.csv") {
      try {
        // Best-effort join: pull account_name from plaid_accounts in one shot
        // and build an in-memory lookup. Falls back to "" when no row exists.
