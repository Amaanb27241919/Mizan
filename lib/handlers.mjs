...
import { isPlaidEligible, countryFromHeaders } from "./geo.mjs";
...
async function _handle({ method, pathname, query, body, headers, rawBody }) {
  ...
  if (pathname === "/api/snaptrade/all") {
    ...
    const settled = await Promise.allSettled(accounts.map(async acct => {
      ...
      // Account currency. SnapTrade nests it as {code:"USD"} and reports it on
      // the balance rows and/or the account total. Mizan's math is USD-only
      // (fmtUSD hardcodes en-US, nisab is computed in USD), so a CAD account
      // summed as if it were USD would overstate net worth by the FX rate and
      // silently push someone over the Zakat threshold — the same failure
      // class as the July nisab bug. We can't convert without an FX source, so
      // we surface the currency and let the UI refuse to quote a wrong number.
      const currency =
        balArr.find(b => b?.currency?.code || b?.currency)?.currency?.code
        || balArr.find(b => typeof b?.currency === "string")?.currency
        || acct.balance?.total?.currency?.code
        || acct.balance?.total?.currency
        || null;
      return {
        accountId:     acct.id,
  ...
  if (pathname.startsWith("/api/plaid/")) {
    ...
    // Region gate — the authoritative one. Plaid onboards US residents only,
    // so creating or completing a bank connection from outside the US is
    // refused here rather than left to fail opaquely inside Plaid Link.
    //
    // This reads the x-vercel-ip-country header the Vercel edge sets, which a
    // browser cannot forge — unlike the mz_country cookie the middleware
    // publishes, which only decides which button the SPA draws. Disconnect
    // (DELETE /item) is deliberately NOT gated: someone who linked while in
    // the US and later moved must always be able to unlink.
    const isConnectAction =
      (pathname === "/api/plaid/link-token" && method === "POST") ||
      (pathname === "/api/plaid/exchange"   && method === "POST");
    if (isConnectAction && !isPlaidEligible(countryFromHeaders(h))) {
      info("plaid.region_blocked", { pathname, country: countryFromHeaders(h) });
      return {
        status: 403,
        body: {
          error: "Bank connections are only available to US residents right now. Everything else in Mizan — Sharia screening, Zakat, and brokerage linking — works from anywhere.",
...
Output the JSON object only.`;
  ...
  if (pathname === "/api/cron/bot-signals" && (method === "GET" || method === "POST")) {
    ...
    const botHeartbeat = (status, err = null) => {
      sbAdmin.from("cron_jobs").upsert({
        job_name:    "bot_signals",
        last_run_at: new Date().toISOString(),
        last_status: status,
        last_error:  err,
        run_count:   1,
