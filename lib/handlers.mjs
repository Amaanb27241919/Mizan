...
async function _handle({ method, pathname, query, body, headers }) {
  ...
  if (pathname === "/api/user/features" && method === "GET") {
    ...
    return { status: 200, body: { trading_bot, full_auto } };
  }
...
`;
  ...
  if (pathname.startsWith("/api/bot/strategies")) {
    ...
    if (method === "GET" && pathname === "/api/bot/strategies") {
      ...
      return { status: 200, body: { strategies: data || [] } };
    }

    // POST /api/bot/strategies — create
    if (method === "POST" && pathname === "/api/bot/strategies") {
      const { ticker, account_id, strategy_type, params = {}, mode = "semi",
      ...
      if (!stop_loss_pct || stop_loss_pct <= 0) return { status: 400, body: { error: "stop_loss_pct is mandatory and must be > 0" } };
      const { data, error } = await sbAdmin.from("bot_strategies").insert({
        user_id: user.id, ticker: ticker.toUpperCase(), account_id, strategy_type, params,
      ...
      audit({ userId: user.id, action: "bot.strategy.created", target: data.id, metadata: { ticker, strategy_type, mode }, headers: h });
    ...
    if (method === "PATCH" && patchMatch) {
      ...
      const allowed = ["enabled","mode","params","capital_allocated","profit_target_pct","stop_loss_pct","max_drawdown_pct","time_horizon_days","max_trades_per_day","nl_risk_disclosed","full_auto_enabled"];
      const updates = {};
  ...
  if (pathname.startsWith("/api/bot/signals")) {
    ...
    if (method === "POST" && approveMatch) {
      ...
      audit({ userId: user.id, action: "bot.signal.approved", target: signalId, metadata: { ticker: sig.ticker, side: sig.side }, headers: h });

      const exec = await placeAlpacaOrder({ symbol: sig.ticker, qty: sig.qty, side: sig.side, type: "market" });
      if (!exec.ok) {
        // Keep status "approved" (not falsely executed); record why it didn't fill.
        await sbAdmin.from("pending_signals").update({ error_msg: String(exec.error || "execution_failed").slice(0, 300) }).eq("id", signalId);
        audit({ userId: user.id, action: "bot.signal.execute_failed", target: signalId, metadata: { ticker: sig.ticker, side: sig.side, error: exec.error }, headers: h });
      ...
      audit({ userId: user.id, action: "bot.signal.executed", target: signalId, metadata: { ticker: sig.ticker, side: sig.side, qty: sig.qty, orderId: exec.orderId, broker: "alpaca_paper" }, headers: h });
      info("bot.signal.executed", { signalId, ticker: sig.ticker, side: sig.side });
      return { status: 200, body: { ok: true, signal_id: signalId, order_id: exec.orderId, broker: "alpaca_paper" } };
    }
  ...
  if (pathname === "/api/bot/strategy/nl" && method === "POST") {
    ...
    const systemPrompt = `You are a strategy parser for the MĪZAN halal trading bot. Parse the user's description into a structured JSON object. Output ONLY valid JSON — no prose, no markdown, no explanation.

REQUIRED output fields:
- ticker (string, uppercase — extract from description or infer from context)
- account_id (string — match account names from: ${accountsStr}; if unclear use "default")
- strategy_type: one of "momentum", "ma_crossover", "breakout"
- capital_allocated (number, USD)
- profit_target_pct (number, percentage gain goal — stored as a TARGET, NOT a guarantee)
- stop_loss_pct (number, MANDATORY — if user didn't specify, default to 15)
- max_drawdown_pct (number — default 20 if not specified)
- time_horizon_days (integer — default 28 if not specified)
- max_trades_per_day (integer — default 3)
- mode: "semi" (default — user approves signals) or "full" (auto-execute)
- risk_disclosure (string — plain-English warning: profit_target_pct is a GOAL not a guarantee; strategy could lose up to stop_loss_pct% of capital_allocated; not financial advice)

REFUSE any strategy requiring: margin, leverage, options, short selling, or interest-bearing instruments.
ALWAYS include stop_loss_pct. A strategy without a stop-loss CANNOT be saved.
Keep all positions Sharia-compliant — no alcohol, tobacco, gambling, conventional finance tickers.
...
Output only the JSON object.`;
    ...
    if (!strategy.stop_loss_pct || strategy.stop_loss_pct <= 0) {
      strategy.stop_loss_pct = 15; // safety default
    }

    audit({ userId: user.id, action: "bot.strategy.nl_parsed", metadata: { strategy_type: strategy.strategy_type }, headers: h });
  ...
  if (pathname === "/api/purification/calculate") {
    ...
    const items = activities
      ...
      .map(a => {
        ...
        const fp = `${ticker}_${date}_${divAmount.toFixed(2)}`;
  ...
  if (pathname === "/api/cron/bot-signals" && (method === "GET" || method === "POST")) {
    ...
    await sbAdmin.from("pending_signals")
      ...
      .eq("status", "pending")
      .lt("expires_at", new Date().toISOString())
      .catch(e => warn("bot.signals.expire_failed", { err: e.message }));

    // Reset daily trade counts if date has changed
    const todayDate = new Date().toISOString().slice(0, 10);
    await sbAdmin.from("bot_strategies")
      .update({ trades_today: 0, trades_today_date: todayDate })
      .eq("enabled", true)
      .neq("trades_today_date", todayDate)
      .catch(e => warn("bot.signals.reset_counts_failed", { err: e.message }));
    ...
    for (const strat of strategies) {
      try {
        ...
        // Sharia gate — non-negotiable, server-side
        const tickerUp = strat.ticker.toUpperCase();
        if (HARAM_TICKERS.has(tickerUp)) {
          warn("bot.signal.sharia_blocked_cron", { ticker: tickerUp, strategy_id: strat.id });
          sharaBlocked++;
          continue;
        }

        // Simple momentum signal: fetch latest price via Finnhub
        const quoteUrl = `https://finnhub.io/api/v1/quote?symbol=${encodeURIComponent(tickerUp)}&token=${FINNHUB_KEY}`;
        const quoteRes = await fetch(quoteUrl).then(r => r.ok ? r.json() : null).catch(() => null);
        if (!quoteRes?.c || !quoteRes?.pc) continue;

        const currentPrice = quoteRes.c;
        const prevClose = quoteRes.pc;
        const dayChange = (currentPrice - prevClose) / prevClose;

        // Momentum signal: +1.5% day change → buy, -1.5% → sell (simplified)
        const BUY_THRESHOLD = 0.015, SELL_THRESHOLD = -0.015;
        let side = null;
        if (strat.strategy_type === "momentum") {
          if (dayChange >= BUY_THRESHOLD) side = "buy";
          else if (dayChange <= SELL_THRESHOLD) side = "sell";
        }
        if (!side) continue;

        // Compute qty based on capital_allocated and current price
        const qty = Math.floor((strat.capital_allocated / strat.max_trades_per_day) / currentPrice * 100) / 100;
        if (qty <= 0) continue;
        ...
        // Full-auto: execute immediately on the paper broker if opted in.
        const isFullAuto = strat.mode === "full" && !!strat.profiles?.full_auto_enabled;
        if (isFullAuto) {
          const exec = await placeAlpacaOrder({ symbol: tickerUp, qty, side, type: "market" });
          if (exec.ok) {
            ...
            audit({ userId: strat.user_id, action: "bot.signal.auto_executed", target: signal.id, metadata: { ticker: tickerUp, side, qty, price: currentPrice, orderId: exec.orderId, broker: "alpaca_paper" }, headers: {} });
            autoExecuted++;
            await sendPushToUser(sbAdmin, strat.user_id, {
              title: `Bot executed (paper): ${side.toUpperCase()} ${qty} ${tickerUp}`,
              body: `Auto-executed at ~$${currentPrice.toFixed(2)} on your Alpaca paper account.`,
              url: "/trade",
            }).catch(() => {});
          } else {
            ...
            audit({ userId: strat.user_id, action: "bot.signal.auto_execute_failed", target: signal.id, metadata: { ticker: tickerUp, side, qty, error: exec.error }, headers: {} });
            warn("bot.signal.auto_execute_failed", { strategy_id: strat.id, signal_id: signal.id, status: exec.status });
          }
        } else {
          // Semi-auto: push notification asking for approval
          audit({ userId: strat.user_id, action: "bot.signal.generated", target: signal.id, metadata: { ticker: tickerUp, side, qty, price: currentPrice }, headers: {} });
          await sendPushToUser(sbAdmin, strat.user_id, {
