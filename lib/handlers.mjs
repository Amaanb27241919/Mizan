    ...
    if (pathname === "/api/plaid/item-status" && method === "GET") {
      ...
      try {
        ...
        const { data: rows } = await sbAdmin
          ...
          .eq("user_id", user.id);
        const ownList = Array.isArray(rows) ? rows : [];
        const tok = ownList.find(r => r.item_id === itemId);
        if (!tok?.access_token) {
          // Diagnostic — keep this even after switching to in-memory match
          // so any future identity / encoding regression is visible.
