...
Output the JSON object only.`;
  ...
  // the in-app chart draws (capped at 365 entries). Vercel: 04:55 UTC daily.
    ...
    try {
      ...
      for (const u of (users || [])) {
        try {
          ...
          // One-time repair of the pre-unification past. The legacy key holds
          // bank-inclusive totals for dates the canonical key recorded as
          // brokerage-only, so where both have a date the legacy value is the
          // more complete one and wins. Dates only the legacy key has are
          // carried over. Self-healing: once merged there's nothing left to do.
          ...
          for (const e of prevArr) if (e?.date) byDate.set(e.date, { ...e });
          for (const e of (Array.isArray(legacy?.value) ? legacy.value : [])) {
            if (!e?.date) continue;
            const v = Number(e.value ?? e.total);
            if (!Number.isFinite(v)) continue;
            byDate.set(e.date, { ...(byDate.get(e.date) || {}), date: e.date, total: v });
          }
          byDate.set(today, { date: today, total });
