...
`;
  ...
  if (pathname.startsWith("/api/bot/signals")) {
    ...
    if (method === "POST" && approveMatch) {
      ...
      // Mark approved and attempt execution (same path as manual trade)
      ...
      return { status: 200, body: { ok: true, signal_id: signalId } };
    }
  ...
  if (pathname === "/api/cron/bot-signals" && (method === "GET" || method === "POST")) {
    ...
    for (const strat of strategies) {
      try {
        ...
        // Full-auto: execute immediately if profile has full_auto_enabled
        const isFullAuto = strat.mode === "full" && !!strat.profiles?.full_auto_enabled;
        if (isFullAuto) {
          // Mark as executed (real trade execution would call trade/impact + trade/place here)
          ...
          audit({ userId: strat.user_id, action: "bot.signal.auto_executed", target: signal.id, metadata: { ticker: tickerUp, side, qty, price: currentPrice }, headers: {} });
          autoExecuted++;
          await sendPushToUser(sbAdmin, strat.user_id, {
            title: `Bot executed: ${side.toUpperCase()} ${qty} ${tickerUp}`,
            body: `Auto-executed at $${currentPrice.toFixed(2)}. Check your portfolio.`,
            url: "/trade",
          }).catch(() => {});
        } else {
