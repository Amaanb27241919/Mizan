    ...
    if (pathname === "/api/plaid/item-status" && method === "GET") {
      ...
      try {
        const { data: tok } = await sbAdmin
          ...
          .maybeSingle();
        if (!tok?.access_token) {
          // Diagnostic: surface what we WERE looking for and what we found
          // for this user.id. Helps tell apart "wrong item_id" from "wrong
          // user.id" (e.g. multi-identity bug) from "row exists but
          // access_token NULL" (write-path bug). All values are scoped to
          // the authenticated user — no leakage.
          const { data: own } = await sbAdmin
            .from("plaid_tokens")
          ...
          warn("plaid.item_status.not_found", {
            ...
            owned_item_ids: ownList.map(r => r.item_id),
            owned_count: ownList.length,
            any_missing_access_token: ownList.some(r => !r.access_token),
          });
          return {
            status: 404,
            body: {
              ...
              debug: {
                ...
                owned_count: ownList.length,
                owned_item_ids: ownList.map(r => ({ id: r.item_id, institution: r.institution_name, has_token: !!r.access_token })),
              },
            },
          };
        }
