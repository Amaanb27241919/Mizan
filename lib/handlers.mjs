...
const CURATED_TTL_MS   = 90 * 24 * 3600 * 1000;   // curated (quarterly MF) rows: ~90d
...
async function getHoldingsRecord(symbol) {
  ...
  const ttl = cached?.source === "alphavantage" ? ETF_CACHE_TTL_MS : CURATED_TTL_MS;
...
`;
  ...
  if (pathname === "/api/etf/overlap" && method === "GET") {
    ...
    const rows = await Promise.all(symbols.map((s) => getHoldingsRecord(s)));
