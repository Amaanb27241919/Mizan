...
// US equity regular-session gate. The bot only ever places MARKET orders, which
// the broker rejects outside 09:30–16:00 ET ("Outside market hours", impact 403
// code 1019). Uses America/New_York wall-clock via Intl (DST-safe, no deps) plus
// a hardcoded market-holiday set.
// NOTE: update MARKET_HOLIDAYS each year, OR replace this whole helper with
// Alpaca's /v2/clock once ALPACA_KEY_ID/SECRET are set (see memory
// alpaca-data-pending). Half-day early closes (1pm ET) are intentionally NOT
// modeled — on those the 1–4pm window still reads open and a late order simply
// gets a harmless broker reject.
...
function usMarketStatus(now = new Date()) {
  ...
  const mins = hour * 60 + parseInt(p.minute, 10);
  if (p.weekday === "Sat" || p.weekday === "Sun") return { open: false, reason: "weekend" };
  if (MARKET_HOLIDAYS.has(dateKey)) return { open: false, reason: "holiday" };
  const OPEN = 9 * 60 + 30, CLOSE = 16 * 60; // 09:30–16:00 ET
  if (mins < OPEN) return { open: false, reason: "pre_market" };
  if (mins >= CLOSE) return { open: false, reason: "after_hours" };
