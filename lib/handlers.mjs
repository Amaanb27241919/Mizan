...
async function _handle({ method, pathname, query, body, headers, rawBody }) {
  ...
  if (pathname === "/api/user/features" && method === "GET") {
    ...
    try {
      const { data } = await sbAdmin.from("profiles").select("email_digest").eq("id", user.id).maybeSingle();
      if (data && typeof data.email_digest === "boolean") email_digest = data.email_digest;
    } catch (e) { warn("user.features.email_digest_read_failed", { err: e.message }); }
    return { status: 200, body: { trading_bot, full_auto, is_root, trading_bot_consented, email_digest } };
  }
...
`;
  ...
  if (pathname === "/api/admin/broadcast-audience") {
    ...
    const { data: profiles, error: profErr } = await sbAdmin
      .from("profiles")
      .select("id, email, created_at, suspended")
    ...
    async function worker() {
      while (idx < profiles.length) {
        ...
        out[i] = { id: p.id, email: p.email, created_at: p.created_at, suspended: !!p.suspended, bank, brokerage, connected };
      }
    }
  ...
  if (pathname === "/api/admin/messages" && method === "GET") {
    ...
    if (ids.length) {
      const { data: profs } = await sbAdmin.from("profiles").select("id, email").in("id", ids);
      emailById = new Map((profs || []).map((p) => [p.id, p.email]));
    }
    const list = [...threads.values()]
      .map((t) => ({ ...t, email: emailById.get(t.user_id) || null }))
