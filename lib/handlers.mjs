...
`;
  ...
  if (pathname === "/api/bot/bogleheads") {
    ...
    if (method === "POST") {
      ...
      return { status: 200, body: { strategy, sleeves: HALAL_BOGLEHEADS_SLEEVES } };
    }
    return { status: 405, body: { error: "method_not_allowed" } };
  }
  ...
  if (pathname === "/api/bot/strategy/nl" && method === "POST") {
    ...
    if (/\b(bogle\w*|three[-\s]?fund|3[-\s]?fund|lazy[-\s]?portfolio|couch[-\s]?potato)\b/i.test(description)) {
      ...
      return { status: 200, body: { strategy, description, preset: "halal_bogleheads", sleeves: HALAL_BOGLEHEADS_SLEEVES } };
    }
...
- universe_tickers (array of uppercase strings): the FULL set of candidate symbols the bot may screen and pick from. If the user named specific tickers, list them all. If the user described a theme but no explicit tickers (e.g. "halal tech", "Shariah ETFs"), propose 3–8 liquid, Sharia-screened tickers that fit. If the user named exactly one ticker, return just that one. Every entry must be Sharia-compliant. Correct obvious ticker typos to the real symbol (APPL→AAPL, NTFLX→NFLX, TESLA→TSLA, FB→META, GOOGLE→GOOGL) and KEEP the corrected name — never drop a ticker merely for a misspelling. NOTE: broad-market ETFs like SPY and QQQ are NOT Sharia-compliant (they hold conventional financials) — exclude them unless they are explicitly Shariah ETFs (e.g. SPUS, HLAL, UMMA).
