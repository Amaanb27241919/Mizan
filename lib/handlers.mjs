...
`;
  ...
  if (pathname === "/api/admin/run-cron" && method === "POST") {
    ...
    const ALLOWED_JOBS = ["sync", "cleanup", "nightly-snapshot", "dividend-check", "bill-reminders", "weekly-digest", "bot-signals"];
  ...
  if (pathname === "/api/admin/broadcast-audience") {
    ...
    if (ptErr) return { status: 500, body: { error: ptErr.message } };
    ...
    const { data: snapRows } = await sbAdmin.from("user_snaptrade").select(SNAP_SECRET_COLS);
    ...
    return { status: 200, body: { users: out, counts } };
  }
