...
async function _handle({ method, pathname, query, body, headers }) {
  ...
  if (pathname === "/api/admin/audit-log") {
    ...
    const offset = Math.max(0, parseInt(q.offset || q.page ? String((parseInt(q.page || "0", 10)) * limit) : "0", 10));
    ...
    return { status: 200, body: { rows: data || [], total: count || 0, limit, offset } };
  }
  ...
  if (pathname === "/api/admin/stats") {
    ...
    // Parallel HEAD counts — cheap.
    const [usersTotal, dau, syncsToday, aiToday] = await Promise.all([
      sbAdmin.auth.admin.listUsers({ page: 1, perPage: 1 }).then(r => r?.data?.total ?? null),
      sbAdmin.from("audit_log").select("user_id", { count: "exact", head: true })
