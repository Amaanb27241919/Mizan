...
async function _handle({ method, pathname, query, body, headers }) {
  ...
  // calculator. Proxies Finnhub OANDA forex quotes (XAU/USD and XAG/USD,
  // quoted per troy ounce), converts to USD/g, and pre-computes the two
  // nisab thresholds (87.48 g gold, 612.36 g silver). 12-hour in-memory
  // cache — precious-metals spot moves slowly enough that we don't need
  // intraday freshness, and this keeps us well under Finnhub's free-tier
  // rate cap even if every Mizan user opens Zakat at the same moment.
  // Falls back to a static body when FINNHUB_KEY is unset or the upstream
  // fetch fails so the client can still render last-known thresholds.
  if (pathname === "/api/metals/spot" && method === "GET") {
    const TROY_OZ_TO_G = 31.1034768;
    const CACHE_TTL_MS = 12 * 60 * 60 * 1000;
    // Per-instance module cache. Vercel may spin multiple instances; each
    // refreshes on its own 12 h cadence. Fine — the API budget is tiny.
    ...
    const fallback = {
      ...
      gold_usd_per_g: null,
      silver_usd_per_g: null,
      nisab_gold_usd: null,
      nisab_silver_usd: null,
    };
    if (!FINNHUB_KEY) {
      info("metals.spot.skipped", { reason: "no_finnhub_key" });
      return { status: 200, body: { ...fallback, reason: "no_finnhub_key" } };
    }
    try {
      const [goldR, silverR] = await Promise.all([
        httpsGetJson(`https://finnhub.io/api/v1/quote?symbol=OANDA:XAU_USD&token=${encodeURIComponent(FINNHUB_KEY)}`),
        httpsGetJson(`https://finnhub.io/api/v1/quote?symbol=OANDA:XAG_USD&token=${encodeURIComponent(FINNHUB_KEY)}`),
      ]);
      const goldOz   = Number(goldR.body?.c);
      const silverOz = Number(silverR.body?.c);
      if (!Number.isFinite(goldOz) || goldOz <= 0 || !Number.isFinite(silverOz) || silverOz <= 0) {
        throw new Error(`unexpected quote shape: gold=${goldOz}, silver=${silverOz}`);
      }
