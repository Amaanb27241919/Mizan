...
function plaidErrorToResponse(err) {
  ...
  const display = data.display_message || data.error_message || err?.message || "Plaid request failed";
  // Map specific codes to recovery hints and HTTP statuses.
  const map = {
      ...
      try {
        ...
        const settled = await Promise.allSettled(tokens.map(async t => {
          ...
          try {
            ...
            const mapped = plaidErrorToResponse(perItemErr);
            logError("plaid.transactions.item_failed", {
              item_id: t.item_id,
              detail: perItemErr.response?.data || perItemErr.message,
            });
            return {
        ...
        const transactions = settled.flatMap(r =>
          r.status === "fulfilled" && r.value.ok ? r.value.transactions : []);
        ...
        return { status: 200, body: { transactions, item_errors: itemErrors } };
      } catch (err) {
        logError("plaid.transactions.failed", { detail: err.response?.data || err.message });
        return plaidErrorToResponse(err);
      }
    }
