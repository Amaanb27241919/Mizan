  ...
  if (pathname === "/api/cron/bot-signals" && (method === "GET" || method === "POST")) {
    ...
    for (const strat of strategies) {
      try {
        ...
        const _eval = []; // TEMP diagnostic
        for (const cand of universe) {
          const q = await finnhubQuote(cand);
          if (!q || !q.prevClose) { _eval.push(`${cand}:NOQUOTE`); continue; }
          const score = (q.current - q.prevClose) / q.prevClose;
          _eval.push(`${cand}:${(score * 100).toFixed(2)}%@$${q.current}`);
          if (score >= BUY_THRESHOLD && (!best || score > best.score)) best = { ticker: cand, price: q.current, score };
        }
        info("bot.entry.eval", { strategy_id: strat.id, threshold_pct: BUY_THRESHOLD * 100, capital: strat.capital_allocated, size_pct: strat.params?.position_size_pct, evals: _eval, picked: best?.ticker || null }); // TEMP
        ...
        // Deployable-capital cap: a strategy's open exposure (in-flight +
        // executed buys, minus executed sells) can never exceed capital_allocated,
        // so the reserved share of buying power is never touched — in ANY mode
        // (manual / semi / full). This only ever shrinks an order, never grows it.
