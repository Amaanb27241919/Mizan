...
async function botPositionFromSignals(strategyId) {
  ...
  const { data, error } = await sbAdmin.from("pending_signals")
    .select("side, qty, suggested_price, executed_at")
    .eq("strategy_id", strategyId)
    .eq("status", "executed");
  if (error || !Array.isArray(data)) return { netQty: 0, avgEntry: null, executedCount: 0 };
  let netQty = 0, buyQty = 0, buyCost = 0;
  for (const s of data) {
    const q = Number(s.qty) || 0;
    const px = Number(s.suggested_price) || 0;
    if (s.side === "buy") { netQty += q; buyQty += q; buyCost += q * px; }
    else if (s.side === "sell") { netQty -= q; }
  }
  const avgEntry = buyQty > 0 ? buyCost / buyQty : null;
  return { netQty: Math.max(0, Math.round(netQty * 100) / 100), avgEntry, executedCount: data.length };
}
...
async function computeStrategyProgress(strat) {
  ...
  try {
    ...
    if (pos.netQty > 0) {
      const quote = await finnhubQuote(strat.ticker);
      if (quote) {
...
REFUSALS (return the {"error": ...} object — these are Sharia + risk violations, never negotiable):
...
Explain in the error which of these the request needed.

STRATEGY OBJECT — emit exactly these fields:
- ticker (string, uppercase): the symbol to trade. Extract from the description; keep it Sharia-screened (no alcohol, tobacco, gambling, weapons, adult, or conventional-finance names).
...
- params (object): { "universe": <universe>, "entry_rules": <entry_rules>, "exit_rules": <exit_rules>, "position_size_pct": <position_size_pct> } — mirror the rule text into params so it is persisted in the params jsonb column.
...
Output the JSON object only.`;
    ...
    const strategy = {
      ...
      position_size_pct,
      params:             { universe, entry_rules, exit_rules, position_size_pct },
  ...
  if (pathname === "/api/cron/bot-signals" && (method === "GET" || method === "POST")) {
    ...
    for (const strat of strategies) {
      try {
        ...
        // ── EXIT ENGINE (runs before entry) ─────────────────────────────────
        // Working-toward-the-target logic. If we hold a net-open position, check
        // stop-loss / drawdown / target / horizon against a fresh quote BEFORE
        // generating any new entry. Stop-loss and drawdown OVERRIDE the target
        // and pause the strategy; horizon closes out. All defensive — never throws.
        const pos = await botPositionFromSignals(strat.id);
        if (pos.netQty > 0 && pos.avgEntry && pos.avgEntry > 0) {
          const gainPct = ((currentPrice - pos.avgEntry) / pos.avgEntry) * 100;
          const isFullAutoExit = strat.mode === "full"
          ...
          // (semi). Sharia gate stays in the execution path via executeSnapTradeOrder.
          const exitClose = async (reason, pushTitle, pushBody) => {
            const { data: xsig, error: xErr } = await sbAdmin.from("pending_signals").insert({
              user_id: strat.user_id, strategy_id: strat.id, ticker: tickerUp,
            ...
            if (isFullAutoExit) {
              ...
              const exec = creds
                ? await executeSnapTradeOrder({ ...creds, accountId: strat.account_id, ticker: tickerUp, side: "sell", qty: pos.netQty })
                : { ok: false, status: 400, error: "broker_not_connected" };
              if (exec.ok) {
                await sbAdmin.from("pending_signals").update({ status: "executed", executed_at: new Date().toISOString() }).eq("id", xsig.id);
                autoExecuted++;
                audit({ userId: strat.user_id, action: reason, target: xsig.id, metadata: { ticker: tickerUp, qty: pos.netQty, price: currentPrice, gainPct, layer: "full", broker: "snaptrade", account: strat.account_id }, headers: {} });
              } else {
                await sbAdmin.from("pending_signals").update({ error_msg: String(exec.error || "execution_failed").slice(0, 300) }).eq("id", xsig.id);
                audit({ userId: strat.user_id, action: reason + ".execute_failed", target: xsig.id, metadata: { ticker: tickerUp, qty: pos.netQty, error: exec.error }, headers: {} });
                warn("bot.exit.execute_failed", { strategy_id: strat.id, signal_id: xsig.id, status: exec.status });
              }
            } else {
              audit({ userId: strat.user_id, action: reason, target: xsig.id, metadata: { ticker: tickerUp, qty: pos.netQty, price: currentPrice, gainPct, layer: "semi" }, headers: {} });
            }
            await sendPushToUser(sbAdmin, strat.user_id, { title: pushTitle, body: pushBody, url: "/trade" }).catch(() => {});
            return true;
          };
          ...
          if (hitStop || hitDrawdown) {
            ...
            await exitClose("bot.strategy.stopped_out",
              `Stop-loss hit on ${tickerUp} — strategy paused`,
            ...
            continue; // strategy paused — do not generate an entry this tick
          }
          if (hitHorizon) {
            await exitClose("bot.strategy.horizon_closed",
              `Horizon reached on ${tickerUp} — closed out`,
          ...
          if (hitTarget) {
            await exitClose("bot.strategy.target_hit",
              `Target reached on ${tickerUp} — sell?`,
              `Up ${gainPct.toFixed(1)}% (target ${strat.profit_target_pct}%). Locking in profit.`);
            continue; // took profit this tick — skip entry generation
          }
        }
        ...
        // Layer 3 — Full-auto: execute immediately via SnapTrade only if
