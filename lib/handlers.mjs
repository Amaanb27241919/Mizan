...
async function _handle({ method, pathname, query, body, headers }) {
  ...
  if (pathname === "/api/account-nicknames") {
    ...
    if (method === "GET") {
      try {
        ...
        logError("account_nicknames.read.failed", { err: err.message });
    ...
    if (method === "PUT") {
      ...
      try {
        ...
        logError("account_nicknames.write.failed", { err: err.message });
        return { status: 500, body: { error: err.message } };
      }
    }
  ...
  // RLS-backed but we use sbAdmin + explicit user_id scoping so we get
  // structured errors instead of silent RLS denials.
  if (pathname === "/api/budgets") {
    ...
    if (method === "GET") {
      try {
        ...
        logError("budgets.read.failed", { err: err.message });
        return { status: 500, body: { error: err.message } };
      }
    }
