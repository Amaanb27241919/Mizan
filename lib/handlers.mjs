...
import { isIpBlocked, trackAuthFailure, trackSnapTradeError, checkCronStaleness, checkNewDevice, checkDataFeeds } from "./anomaly.mjs";
...
`;
  ...
  if (pathname === "/api/admin/stats") {
    ...
    let funnel = null;
    try {
      const { users } = await buildAudience();
...
Output the JSON object only.`;
  ...
  if (pathname === "/api/cron/activation") {
    ...
    for (const u of targets) {
      ...
      const greeting = first ? `Assalamu alaikum ${first},` : "Assalamu alaikum,";
      const body = [
        greeting,
        "",
      ...
      try {
        const r = await sendUserEmail(u.email, "Your Mīzan account is waiting on one step", body, sbAdmin);
  ...
  if (pathname === "/api/cron/cleanup") {
    ...
    info("cron.cleanup.ok", { deleted_audit: deletedAudit, deleted_rate_limits: deletedRate, deleted_security_events: deletedSecEvents, feeds_unhealthy: Object.values(feeds).filter(f => !f.healthy).length });
