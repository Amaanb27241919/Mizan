...
//
// On each call we:
//   1. Read the stored cursor for this Item (NULL on first run → full backfill).
//   2. Loop /transactions/sync while has_more=true, accumulating added /
//      modified / removed.
//   3. Upsert added+modified into plaid_transactions, delete removed.
//   4. Persist the final next_cursor on the plaid_tokens row so the
//      next call only fetches the delta.
//
// Idempotent: replaying the same cursor returns the same diff, and
// transaction_id is unique so upserts are safe under concurrency.
        ...
        if (item_id && sbAdmin) {
          ...
          if (user_id) {
            audit({
              ...
              target: tok?.institution_name || null,
              metadata: { item_id, webhook_type, webhook_code, payload: parsed },
              headers: h,
            });
          }
        }
        ...
        try {
          ...
          for (const t of tokens) {
            try {
              ...
              warn("plaid.transactions.sync.item_failed", { item_id: t.item_id, detail });
              errors.push({ item_id: t.item_id, error: typeof detail === "string" ? detail : JSON.stringify(detail) });
            }
          }
          info("plaid.transactions.sync.ok", { user: user.id, items: tokens.length, ok, failed, added, modified, removed });
          return { status: 200, body: { ok: failed === 0, items: tokens.length, ok_count: ok, failed, added, modified, removed, errors } };
        } catch (err) {
          const detail = err.response?.data || err.message;
      ...
      try {
        ...
        const transactions = (data || []).map(row => ({
          ...
          personal_finance_category: row.category_primary
            ...
            : null,
          // Keep `category` populated so the legacy fallback path
          // (t.category?.[0]) continues to work for any rows whose
          // personal_finance_category was missing on ingest.
