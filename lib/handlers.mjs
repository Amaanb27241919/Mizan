...
export async function handleApiRequest({ method, pathname, query, body, headers }) {
  ...
  // hours by their poller). This endpoint pushes a manual refresh signal
  // so balances + activity catch up to what's on the brokerage UI.
  //
  // SnapTrade rate-limits this per-connection (a few per hour). On failure
  // we surface the upstream status so the UI can back off.
  if (pathname === "/api/snaptrade/refresh" && method === "POST") {
    let snap; try { snap = await getSnapForRequest(h); } catch { return { status: 404, body: { error: "No registered user" } }; }
    const { userId, userSecret, supabaseUser } = snap;
    const r = await snapReq("POST", "/snapTrade/manualRefresh", null, { userId, userSecret });
    if (r.status >= 200 && r.status < 300) {
      audit({ userId: supabaseUser?.id, action: "broker.force_refresh", headers: h });
      console.log(`  ✓ /snaptrade/refresh: refresh queued`);
      return { status: 200, body: { ok: true, queued: true, body: r.body } };
    }
    if (r.status === 429) {
      return {
        status: 429,
        body: { error: "SnapTrade is throttling refreshes for your connections. Try again in an hour." },
        headers: { "Retry-After": "3600" },
      };
    }
    console.log(`  ✗ /snaptrade/refresh: ${r.status}`);
    return { status: r.status || 500, body: { error: `refresh ${r.status}: ${JSON.stringify(r.body)}` } };
  }
