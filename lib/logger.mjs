...
/**
 * Wraps an async handler so its start, end, duration, and any thrown
 * error are all logged with the request context. Returns whatever the
 * inner function returns.
 */
export async function withTiming(event, fn) {
  const t0 = Date.now();
  try {
    const out = await fn();
    info(`${event}.done`, { durationMs: Date.now() - t0 });
    return out;
  } catch (e) {
    error(`${event}.failed`, { err: e?.message, stack: e?.stack, durationMs: Date.now() - t0 });
    throw e;
  }
}
