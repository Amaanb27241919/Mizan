...
export const PROHIBITED_PATTERNS = Object.freeze([
  ...
  {
    name: "gerund-would-benefit",
    // "trimming toward your 25% target would improve diversification",
    // "adding here would help your allocation".
    //
    // FOUND 2026-08-21 on the marketing site, which depicted the advisor saying
    // exactly the first example — and every pattern above missed it. The
    // gerund ("trimming") is not in the act-on-your-position verb list, and
    // even the base verb would have missed, because that rule requires "your"
    // to follow IMMEDIATELY and here a word intervenes ("toward your").
    //
    // Deliberately requires an explicit benefit clause rather than just a
    // gerund near "your": "your allocation is 38% technology" is a fact and
    // must stay sayable. It is the promised improvement that turns a
    // description into a recommendation.
    re: new RegExp(`\\b(?:${ACTION_ING})\\b[^.?!\\n]{0,40}\\bwould\\s+(?:improve|help|reduce|increase|lower|raise|benefit|be better|make sense|give you)\\b`, "i"),
  },
  {
    name: "best-for-you",
